...
class n {
	static patch(i) {
		!(function i(o, r) {
			o.url &&
				n.isPlatformPath(o.url, t.Platform.isWin()) &&
				(o.url = e.ParsedURL.ParsedURL.rawPathToUrlString(o.url));
			for (const e of Object.entries(o)) {
				const t = e[0],
					n = e[1],
					o = r + "." + t;
				".result.result.value" !== o &&
					null !== n &&
					"object" == typeof n &&
					i(n, o);
			}
...
class l {
	...
	registerEnum(e, t) {
		const [n, i] = s(e);
		(globalThis.Protocol[n] || (globalThis.Protocol[n] = {}),
			(globalThis.Protocol[n][i] = t),
			this.enumMap.set(e, t),
			(this.#e = !0));
	}
	registerType(e, t) {
...
class m {
	...
	constructor(e) {
		((this.#n = e),
			...
			(this.#a = new Map()),
			(this.#s = []),
			(c.deprecatedRunAfterPendingDispatches =
				this.deprecatedRunAfterPendingDispatches.bind(this)),
			(c.sendRawMessage = this.sendRawMessageForTesting.bind(this)),
			this.#n.setOnMessage(this.onMessage.bind(this)),
			this.#n.setOnDisconnect((e) => {
				const t = this.#a.get("");
	...
	sendMessage(e, t, n, i, o) {
		const r = this.nextMessageId(),
			a = { id: r, method: n };
		if (
			(i && (a.params = i),
			e && (a.sessionId = e),
			c.dumpProtocol && c.dumpProtocol("frontend: " + JSON.stringify(a)),
			c.onMessageSent)
		) {
			const o = JSON.parse(JSON.stringify(i || {}));
			c.onMessageSent(
				{ domain: t, method: n, params: o, id: r, sessionId: e },
				this.getTargetBySessionId(e)
			);
		}
		(++this.#o, u.has(n) && this.#r.add(r));
		const s = this.#a.get(e);
		s &&
			(s.callbacks.set(r, { callback: o, method: n }),
			this.#n.sendRawMessage(JSON.stringify(a)));
	}
	...
	onMessage(e) {
		if (
			(c.dumpProtocol &&
				c.dumpProtocol(
					"backend: " + ("string" == typeof e ? e : JSON.stringify(e))
				),
			c.onMessageReceived)
		) {
		...
		for (const e of this.#a.values())
			e.proxyConnection &&
				(e.proxyConnection.onMessage
					? (e.proxyConnection.onMessage(t), (i = !0))
					: l.reportProtocolError(
		...
		const o = t.sessionId || "",
			r = this.#a.get(o);
		if (r) {
			if (!r.proxyConnection)
				if (
					(r.target.getNeedsNodeJSPatching() && n.patch(t), void 0 !== t.id)
				) {
					...
					(e.callback(t.error || null, t.result || null),
						--this.#o,
						this.#r.delete(t.id),
						this.#s.length &&
					...
					if (void 0 === t.method)
						return void l.reportProtocolError(
	...
	static dispatchConnectionError(e, t) {
		const n = {
			message: `Connection is closed, can't dispatch pending call to ${t}`,
			code: a,
			data: null,
		};
	...
	static dispatchUnregisterSessionError({ callback: e, method: t }) {
		const n = {
			message: `Session is unregistering, can't dispatch pending call to ${t}`,
			code: a,
			data: null,
		};
		window.setTimeout(() => e(n, null), 0);
...
class y {
	...
	registerCommand(e, t, n, i) {
		...
		((this[e] = function (...e) {
			...
			(this.metadata[o] = { parameters: t, description: i, replyArgs: n }),
			(this["invoke_" + e] = function (e = {}) {
	...
	prepareParameters(e, t, n, i) {
		...
		for (const a of t) {
			...
			if (!n.length && !l)
				return (
					i(
						`Protocol Error: Invalid number of arguments for method '${e}' call. It must have the following arguments ${JSON.stringify(t)}'.`
					),
					null
				);
			const d = n.shift();
			if (l && void 0 === d) continue;
			if (typeof d !== ("array" === p ? "object" : p))
				return (
					i(
						`Protocol Error: Invalid type of argument '${s}' for method '${e}' call. It must be '${p}' but it is '${typeof d}'.`
					),
					null
				);
			((o[s] = d), (r = !0));
		...
		return n.length
			? (i(
					`Protocol Error: Extra ${n.length} arguments in a call to method '${e}'.`
				),
				null)
			: r
				? o
	...
	sendMessageToBackendPromise(e, t, n) {
		...
		return i
			...
			: new Promise((t) => {
							...
							if (n)
								return (
									c.suppressRequestErrors ||
										...
										n.code === a ||
										console.error(
											"Request " + e + " failed. " + JSON.stringify(n)
										),
									void t(null)
								);
					...
					i
						? i.sendMessage(this.target.sessionId, this.domain, e, s, n)
						: m.dispatchConnectionError(n, e);
				});
	}
	invoke(e, t) {
		return new Promise((n) => {
			const i = (t, i) => {
					t &&
						...
						t.code !== r &&
						t.code !== a &&
						console.error("Request " + e + " failed. " + JSON.stringify(t));
					const s = t?.message;
					n({ ...i, getError: () => s });
				},
				s = this.target.router();
			s
				? s.sendMessage(this.target.sessionId, this.domain, e, t, i)
				: m.dispatchConnectionError(i, e);
		});
	}
}
class g {
	...
	dispatch(e, t) {
		...
		if (!this.#p.has(t.method))
			return void l.reportProtocolWarning(
				`Protocol Warning: Attempted to dispatch an unspecified event '${t.method}'`,
				t
			);
		const n = { ...t.params };
...
var h = Object.freeze({
	...
	TargetBase: class {
		...
		constructor(e, t, n, i) {
			...
			let o;
			((o = n && t && t.routerInternal ? t.routerInternal : new m(i || d())),
				(this.routerInternal = o),
				o.registerSession(this, this.sessionId));
			for (const [e, t] of f.agentPrototypes) {
				const n = Object.create(t);
				((n.target = this), this.#d.set(e, n));
			}
			for (const [e, t] of f.getEventParameterNames().entries())
				this.#l.set(e, new g(t));
		}
		dispatch(e) {
			const [t, n] = s(e.method),
				i = this.#l.get(t);
			i
				? i.dispatch(n, e)
				: l.reportProtocolError(
						`Protocol Error: the message ${e.method} is for non-existing domain '${t}'`,
						e
					);
		}
		dispose(e) {
...
((self.Protocol = self.Protocol || {}),
	(function (e) {
		(e.registerEnum("Accessibility.AXValueType", {
			...
			e.registerEvent("Autofill.addressFormFilled", [
				"filledFields",
				"addressUi",
			]),
			e.registerCommand(
			...
			e.registerEvent("BackgroundService.recordingStateChanged", [
				"isRecording",
				"service",
			]),
			e.registerEvent("BackgroundService.backgroundServiceEventReceived", [
			...
			e.registerEvent("Browser.downloadWillBegin", [
				"frameId",
				"guid",
				"url",
				"suggestedFilename",
			...
			e.registerType("CSS.CSSPositionFallbackRule", [
				{
					name: "name",
					type: "object",
					optional: !1,
			...
			e.registerEvent("DOM.childNodeCountUpdated", [
				"nodeId",
				"childNodeCount",
			]),
			e.registerEvent("DOM.childNodeInserted", [
				"parentNodeId",
				"previousNodeId",
				"node",
			]),
			e.registerEvent("DOM.childNodeRemoved", ["parentNodeId", "nodeId"]),
			e.registerEvent("DOM.distributedNodesUpdated", [
				"insertionPointId",
				"distributedNodes",
			]),
			...
			e.registerEvent("DOM.pseudoElementRemoved", [
				"parentId",
				"pseudoElementId",
			]),
			...
			e.registerEnum("DOM.EnableRequestIncludeWhitespace", {
				None: "none",
				All: "all",
			}),
			e.registerCommand(
			...
			e.registerEvent("Database.addDatabase", ["database"]),
			e.registerCommand(
				"Database.disable",
				[],
				[],
			...
			e.registerType("Database.Database", [
				{
					name: "id",
					type: "string",
					optional: !1,
			...
			e.registerCommand("Memory.prepareForLeakDetection", [], [], ""),
			e.registerCommand(
			...
			e.registerEvent("Network.loadingFinished", [
				"requestId",
				"timestamp",
				"encodedDataLength",
			]),
			...
			e.registerEvent("Network.resourceChangedPriority", [
				"requestId",
				"newPriority",
				"timestamp",
			]),
			...
			e.registerEvent("Network.webSocketCreated", [
				"requestId",
				"url",
				"initiator",
			]),
			e.registerEvent("Network.webSocketFrameError", [
				"requestId",
				"timestamp",
				"errorMessage",
			]),
			e.registerEvent("Network.webSocketFrameReceived", [
				"requestId",
				"timestamp",
				"response",
			]),
			e.registerEvent("Network.webSocketFrameSent", [
				"requestId",
				"timestamp",
				"response",
			]),
			...
			e.registerEvent("Network.webTransportConnectionEstablished", [
				"transportId",
				"timestamp",
			]),
			e.registerEvent("Network.webTransportClosed", [
				"transportId",
				"timestamp",
			]),
			...
			e.registerEvent("Network.subresourceWebBundleMetadataReceived", [
				"requestId",
				"urls",
			]),
			e.registerEvent("Network.subresourceWebBundleMetadataError", [
				"requestId",
				"errorMessage",
			]),
			e.registerEvent("Network.subresourceWebBundleInnerResponseParsed", [
				"innerRequestId",
				"innerRequestURL",
				"bundleRequestId",
			]),
			e.registerEvent("Network.subresourceWebBundleInnerResponseError", [
				"innerRequestId",
				"innerRequestURL",
				"errorMessage",
				"bundleRequestId",
			...
			e.registerEvent("Network.reportingApiEndpointsChangedForOrigin", [
				"origin",
				"endpoints",
			]),
			e.registerCommand(
			...
			e.registerType("Network.BlockedCookieWithReason", [
				{
					name: "blockedReasons",
					type: "array",
					optional: !1,
			...
			e.registerEvent("Page.fileChooserOpened", [
				"frameId",
				"mode",
				"backendNodeId",
			]),
			e.registerEvent("Page.frameAttached", [
				"frameId",
				"parentFrameId",
				"stack",
			]),
			e.registerEvent("Page.frameClearedScheduledNavigation", ["frameId"]),
			e.registerEnum("Page.FrameDetachedEventReason", {
				Remove: "remove",
				Swap: "swap",
			}),
			...
			e.registerEvent("Page.frameRequestedNavigation", [
				"frameId",
				"reason",
				"url",
				"disposition",
			]),
			e.registerEvent("Page.frameScheduledNavigation", [
				"frameId",
				"delay",
				"reason",
				"url",
			...
			e.registerEvent("Page.downloadWillBegin", [
				"frameId",
				"guid",
				"url",
				"suggestedFilename",
			...
			e.registerEvent("Page.downloadProgress", [
				"guid",
				"totalBytes",
				"receivedBytes",
				"state",
			...
			e.registerEvent("Page.javascriptDialogClosed", ["result", "userInput"]),
			...
			e.registerEvent("Page.lifecycleEvent", [
				"frameId",
				"loaderId",
				"name",
				"timestamp",
			...
			e.registerEvent("Page.navigatedWithinDocument", ["frameId", "url"]),
			e.registerEvent("Page.screencastFrame", [
				"data",
				"metadata",
				"sessionId",
			]),
			e.registerEvent("Page.screencastVisibilityChanged", ["visible"]),
			e.registerEvent("Page.windowOpen", [
				"url",
				"windowName",
				"windowFeatures",
				"userGesture",
			...
			e.registerEnum("Page.StartScreencastRequestFormat", {
				Jpeg: "jpeg",
				Png: "png",
			}),
			e.registerCommand(
		...
		(e.registerCommand(
			...
			e.registerEnum("Page.SetWebLifecycleStateRequestState", {
				Frozen: "frozen",
				Active: "active",
			}),
			e.registerCommand(
			...
			e.registerEvent("Security.certificateError", [
				"eventId",
				"errorType",
				"requestURL",
			]),
			e.registerEvent("Security.visibleSecurityStateChanged", [
			...
			e.registerEvent("ServiceWorker.workerRegistrationUpdated", [
				"registrations",
			]),
			...
			e.registerEvent("Storage.cacheStorageListUpdated", [
				"origin",
				"storageKey",
				"bucketId",
			]),
			...
			e.registerEvent("Storage.indexedDBListUpdated", [
				"origin",
				"storageKey",
				"bucketId",
			]),
			...
			e.registerEvent("Storage.attributionReportingSourceRegistered", [
				"registration",
				"result",
			]),
			e.registerCommand(
			...
			e.registerType("Storage.InterestGroupAd", [
				{
					name: "renderURL",
					type: "string",
					optional: !1,
			...
			e.registerType("Storage.InterestGroupDetails", [
				{
					name: "ownerOrigin",
					type: "string",
					optional: !1,
				...
				},
				{
					...
					optional: !1,
					description: "",
					typeRef: "Storage.InterestGroupAd",
				},
				{
					name: "adComponents",
					type: "array",
			...
			e.registerEvent("Target.attachedToTarget", [
				"sessionId",
				"targetInfo",
				"waitingForDebugger",
			]),
			e.registerEvent("Target.detachedFromTarget", ["sessionId", "targetId"]),
			e.registerEvent("Target.receivedMessageFromTarget", [
				"sessionId",
				"message",
				"targetId",
			]),
			...
			e.registerEvent("Target.targetCrashed", [
				"targetId",
				"status",
				"errorCode",
			]),
			...
			e.registerEvent("Tracing.bufferUsage", [
				"percentFull",
				"eventCount",
				"value",
			]),
			...
			e.registerEvent("WebAudio.audioListenerWillBeDestroyed", [
				"contextId",
				"listenerId",
			]),
			e.registerEvent("WebAudio.audioNodeCreated", ["node"]),
			e.registerEvent("WebAudio.audioNodeWillBeDestroyed", [
				"contextId",
				"nodeId",
			]),
			e.registerEvent("WebAudio.audioParamCreated", ["param"]),
			e.registerEvent("WebAudio.audioParamWillBeDestroyed", [
				"contextId",
				"nodeId",
				"paramId",
			]),
			...
			e.registerEvent("WebAuthn.credentialAdded", [
				"authenticatorId",
				"credential",
			]),
			e.registerEvent("WebAuthn.credentialAsserted", [
				"authenticatorId",
				"credential",
			]),
			e.registerCommand(
			...
			e.registerEvent("Media.playerPropertiesChanged", [
				"playerId",
				"properties",
			]),
			...
			e.registerEvent("Media.playersCreated", ["players"]),
			...
			e.registerEvent("Debugger.breakpointResolved", [
				"breakpointId",
				"location",
			]),
			...
			e.registerEnum("Debugger.RestartFrameRequestMode", {
				StepInto: "StepInto",
			}),
			e.registerCommand(
			...
			e.registerEvent("HeapProfiler.lastSeenObjectId", [
				"lastSeenObjectId",
				"timestamp",
			]),
			e.registerEvent("HeapProfiler.reportHeapSnapshotProgress", [
				"done",
				"total",
				"finished",
			]),
			...
			e.registerEvent("Profiler.consoleProfileFinished", [
				"id",
				"location",
				"profile",
				"title",
			]),
			e.registerEvent("Profiler.consoleProfileStarted", [
				"id",
				"location",
				"title",
			]),
			e.registerEvent("Profiler.preciseCoverageDeltaUpdate", [
				"timestamp",
				"occasion",
				"result",
			]),
			...
			e.registerEvent("Runtime.bindingCalled", [
				"name",
				"payload",
				"executionContextId",
			]),
			...
			e.registerEvent("Runtime.exceptionThrown", [
				"timestamp",
				"exceptionDetails",
			]),
			...
			e.registerEvent("Runtime.inspectRequested", [
				"object",
				"hints",
				"executionContextId",
			]),
