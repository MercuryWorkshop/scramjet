...
(function (window) {
	...
	const DevToolsAPIImpl = class {
		...
		setInspectedTabId(tabId) {
			...
			if (
				window["WebInspector"] &&
				window["WebInspector"]["setInspectedTabId"]
			) {
	...
	const EnumeratedHistogram = {
		...
		NetworkPanelResponsePreviewOpened:
	...
	const InspectorFrontendHostImpl = class {
		...
		searchInPath(requestId, fileSystemPath, query) {
			DevToolsAPI.sendMessageToEmbedder(
				"searchInPath",
				[requestId, fileSystemPath, query],
				null
			);
		}
	...
	function installBackwardsCompatibility() {
		...
		if (majorVersion <= 73 && !Element.prototype.createShadowRoot) {
			...
			DOMTokenList.prototype.add = function (...tokens) {
				if (
					tokens[0].startsWith("insertion-point") ||
					tokens[0].startsWith("tabbed-pane-header")
				) {
			...
			if (!ShadowRoot.prototype.__originalShadowRootElementFromPoint) {
				...
				ShadowRoot.prototype.elementFromPoint = function (x, y) {
					const originalResult =
						ShadowRoot.prototype.__originalShadowRootElementFromPoint.apply(
							this,
							arguments
						);
	...
	function getRemoteMajorVersion() {
		try {
			const remoteVersion = new URLSearchParams(window.location.search).get(
				"remoteVersion"
			);
			if (!remoteVersion) {
				return null;
