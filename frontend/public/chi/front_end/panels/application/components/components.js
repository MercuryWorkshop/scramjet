...
import * as o from "../../../ui/components/chrome_link/chrome_link.js";
import * as i from "../../../ui/components/expandable_list/expandable_list.js";
import * as n from "../../../ui/components/helpers/helpers.js";
import * as s from "../../../ui/components/icon_button/icon_button.js";
import * as l from "../../../ui/components/legacy_wrapper/legacy_wrapper.js";
import * as c from "../../../ui/components/render_coordinator/render_coordinator.js";
import * as d from "../../../ui/components/report_view/report_view.js";
import * as h from "../../../ui/components/tree_outline/tree_outline.js";
import * as u from "../../../ui/legacy/components/utils/utils.js";
import * as m from "../../../ui/lit-html/lit-html.js";
import * as g from "../../../ui/visual_logging/visual_logging.js";
import * as p from "../../../ui/components/data_grid/data_grid.js";
import * as b from "../../../core/platform/platform.js";
import * as w from "../../../core/root/root.js";
import * as f from "../../../models/bindings/bindings.js";
import * as v from "../../../models/workspace/workspace.js";
import * as k from "../../network/forward/forward.js";
import * as y from "../../../third_party/csp_evaluator/csp_evaluator.js";
import * as T from "../../../ui/components/adorners/adorners.js";
import * as S from "../../../core/host/host.js";
import * as R from "../../../ui/components/input/input.js";
...
const x = {
		...
		schemeNotHTTPOrHTTPS:
			"Only pages whose URL scheme is HTTP / HTTPS can be cached.",
		...
		HTTPMethodNotGET:
			"Only pages loaded via a GET request are eligible for back/forward cache.",
		...
		JavaScriptExecution:
			"Chrome detected an attempt to execute JavaScript while in the cache.",
		...
		sessionRestored:
			"Chrome restarted and cleared the back/forward cache entries.",
		...
		backForwardCacheDisabledByCommandLine:
			"Back/forward cache is disabled by the command line.",
		...
		userAgentOverrideDiffers:
			"Browser has changed the user agent override header.",
		foregroundCacheLimit:
			"The page was evicted from the cache to allow another page to be cached.",
		backForwardCacheDisabledForDelegate:
			"Back/forward cache is not supported by delegate.",
		unloadHandlerExistsInMainFrame:
			"The page has an unload handler in the main frame.",
		...
		noResponseHead:
			"Pages that do not have a valid response head cannot enter back/forward cache.",
		cacheControlNoStore:
		...
		containsPlugins:
			"Pages containing plugins are not currently eligible for back/forward cache.",
		documentLoaded:
			"The document did not finish loading before navigating away.",
		...
		sharedWorker:
			"Pages that use SharedWorker are not currently eligible for back/forward cache.",
		webLocks:
			"Pages that use WebLocks are not currently eligible for back/forward cache.",
		webHID:
			"Pages that use WebHID are not currently eligible for back/forward cache.",
		webShare:
			"Pages that use WebShare are not currently eligible for back/forwad cache.",
		requestedStorageAccessGrant:
			"Pages that have requested storage access are not currently eligible for back/forward cache.",
		webNfc:
			"Pages that use WebNfc are not currently eligible for back/forwad cache.",
		...
		printing:
			"Pages that show Printing UI are not currently eligible for back/forward cache.",
		webDatabase:
			"Pages that use WebDatabase are not currently eligible for back/forward cache.",
		pictureInPicture:
		...
		idleManager:
			"Pages that use IdleManager are not currently eligible for back/forward cache.",
		paymentManager:
		...
		keyboardLock:
			"Pages that use Keyboard lock are not currently eligible for back/forward cache.",
		webOTPService:
			"Pages that use WebOTPService are not currently eligible for bfcache.",
		...
		contentFileChooser:
			"Pages that use FileChooser API are not eligible for back/forward cache.",
		contentSerial:
			"Pages that use Serial API are not eligible for back/forward cache.",
		...
		contentWebBluetooth:
			"Pages that use WebBluetooth API are not eligible for back/forward cache.",
		contentWebUSB:
			"Pages that use WebUSB API are not eligible for back/forward cache.",
		contentMediaSession:
		...
		embedderPermissionRequestManager:
			"There were permission requests upon navigating away.",
		embedderModalDialog:
		...
		fencedFramesEmbedder:
			"Pages using FencedFrames cannot be stored in bfcache.",
		keepaliveRequest:
		...
		webRTCSticky:
			"Back/forward cache is disabled because WebRTC has been used.",
		webTransportSticky:
			"Back/forward cache is disabled because WebTransport has been used.",
		webSocketSticky:
			"Back/forward cache is disabled because WebSocket has been used.",
		HTTPAuthRequired: "Undefined",
		CookieFlushed: "Undefined",
	},
	C = t.i18n.registerUIStrings(
		"panels/application/components/BackForwardCacheStrings.ts",
		x
	),
	N = t.i18n.getLazilyComputedLocalizedString.bind(void 0, C),
	I = {
		NotPrimaryMainFrame: { name: N(x.notMainFrame) },
		BackForwardCacheDisabled: { name: N(x.backForwardCacheDisabled) },
		RelatedActiveContentsExist: { name: N(x.relatedActiveContentsExist) },
		HTTPStatusNotOK: { name: N(x.HTTPStatusNotOK) },
		SchemeNotHTTPOrHTTPS: { name: N(x.schemeNotHTTPOrHTTPS) },
		Loading: { name: N(x.loading) },
		WasGrantedMediaAccess: { name: N(x.wasGrantedMediaAccess) },
		HTTPMethodNotGET: { name: N(x.HTTPMethodNotGET) },
		SubframeIsNavigating: { name: N(x.subframeIsNavigating) },
		Timeout: { name: N(x.timeout) },
		CacheLimit: { name: N(x.cacheLimit) },
		JavaScriptExecution: { name: N(x.JavaScriptExecution) },
		RendererProcessKilled: { name: N(x.rendererProcessKilled) },
		RendererProcessCrashed: { name: N(x.rendererProcessCrashed) },
		GrantedMediaStreamAccess: { name: N(x.grantedMediaStreamAccess) },
		CacheFlushed: { name: N(x.cacheFlushed) },
		ServiceWorkerVersionActivation: {
			name: N(x.serviceWorkerVersionActivation),
		},
		SessionRestored: { name: N(x.sessionRestored) },
		ServiceWorkerPostMessage: { name: N(x.serviceWorkerPostMessage) },
		EnteredBackForwardCacheBeforeServiceWorkerHostAdded: {
			name: N(x.enteredBackForwardCacheBeforeServiceWorkerHostAdded),
		},
		ServiceWorkerClaim: { name: N(x.serviceWorkerClaim) },
		HaveInnerContents: { name: N(x.haveInnerContents) },
		TimeoutPuttingInCache: { name: N(x.timeoutPuttingInCache) },
		BackForwardCacheDisabledByLowMemory: {
			name: N(x.backForwardCacheDisabledByLowMemory),
		},
		BackForwardCacheDisabledByCommandLine: {
			name: N(x.backForwardCacheDisabledByCommandLine),
		},
		NetworkRequestDatapipeDrainedAsBytesConsumer: {
			name: N(x.networkRequestDatapipeDrainedAsBytesConsumer),
		},
		NetworkRequestRedirected: { name: N(x.networkRequestRedirected) },
		NetworkRequestTimeout: { name: N(x.networkRequestTimeout) },
		NetworkExceedsBufferLimit: { name: N(x.networkExceedsBufferLimit) },
		NavigationCancelledWhileRestoring: {
			name: N(x.navigationCancelledWhileRestoring),
		},
		BackForwardCacheDisabledForPrerender: {
			name: N(x.backForwardCacheDisabledForPrerender),
		},
		UserAgentOverrideDiffers: { name: N(x.userAgentOverrideDiffers) },
		ForegroundCacheLimit: { name: N(x.foregroundCacheLimit) },
		BackForwardCacheDisabledForDelegate: {
			name: N(x.backForwardCacheDisabledForDelegate),
		},
		UnloadHandlerExistsInMainFrame: {
			name: N(x.unloadHandlerExistsInMainFrame),
		},
		UnloadHandlerExistsInSubFrame: { name: N(x.unloadHandlerExistsInSubFrame) },
		ServiceWorkerUnregistration: { name: N(x.serviceWorkerUnregistration) },
		NoResponseHead: { name: N(x.noResponseHead) },
		CacheControlNoStore: { name: N(x.cacheControlNoStore) },
		CacheControlNoStoreCookieModified: { name: N(x.cacheControlNoStore) },
		CacheControlNoStoreHTTPOnlyCookieModified: {
			name: N(x.cacheControlNoStore),
		},
		DisableForRenderFrameHostCalled: { name: N(x.ineligibleAPI) },
		BlocklistedFeatures: { name: N(x.ineligibleAPI) },
		SchedulerTrackedFeatureUsed: { name: N(x.ineligibleAPI) },
		DomainNotAllowed: { name: N(x.internalError) },
		ConflictingBrowsingInstance: { name: N(x.internalError) },
		NotMostRecentNavigationEntry: { name: N(x.internalError) },
		IgnoreEventAndEvict: { name: N(x.internalError) },
		BrowsingInstanceNotSwapped: { name: N(x.internalError) },
		ActivationNavigationsDisallowedForBug1234857: { name: N(x.internalError) },
		Unknown: { name: N(x.internalError) },
		RenderFrameHostReused_SameSite: { name: N(x.internalError) },
		RenderFrameHostReused_CrossSite: { name: N(x.internalError) },
		WebSocket: { name: N(x.webSocket) },
		WebTransport: { name: N(x.webTransport) },
		WebRTC: { name: N(x.webRTC) },
		MainResourceHasCacheControlNoStore: {
			name: N(x.mainResourceHasCacheControlNoStore),
		},
		MainResourceHasCacheControlNoCache: {
			name: N(x.mainResourceHasCacheControlNoCache),
		},
		SubresourceHasCacheControlNoStore: {
			name: N(x.subresourceHasCacheControlNoStore),
		},
		SubresourceHasCacheControlNoCache: {
			name: N(x.subresourceHasCacheControlNoCache),
		},
		ContainsPlugins: { name: N(x.containsPlugins) },
		DocumentLoaded: { name: N(x.documentLoaded) },
		DedicatedWorkerOrWorklet: { name: N(x.dedicatedWorkerOrWorklet) },
		OutstandingNetworkRequestOthers: {
			name: N(x.outstandingNetworkRequestOthers),
		},
		OutstandingIndexedDBTransaction: {
			name: N(x.outstandingIndexedDBTransaction),
		},
		RequestedNotificationsPermission: {
			name: N(x.requestedNotificationsPermission),
		},
		RequestedMIDIPermission: { name: N(x.requestedMIDIPermission) },
		RequestedAudioCapturePermission: {
			name: N(x.requestedAudioCapturePermission),
		},
		RequestedVideoCapturePermission: {
			name: N(x.requestedVideoCapturePermission),
		},
		RequestedBackForwardCacheBlockedSensors: {
			name: N(x.requestedBackForwardCacheBlockedSensors),
		},
		RequestedBackgroundWorkPermission: {
			name: N(x.requestedBackgroundWorkPermission),
		},
		BroadcastChannel: { name: N(x.broadcastChannel) },
		IndexedDBConnection: { name: N(x.indexedDBConnection) },
		WebXR: { name: N(x.webXR) },
		SharedWorker: { name: N(x.sharedWorker) },
		WebLocks: { name: N(x.webLocks) },
		WebHID: { name: N(x.webHID) },
		WebShare: { name: N(x.webShare) },
		RequestedStorageAccessGrant: { name: N(x.requestedStorageAccessGrant) },
		WebNfc: { name: N(x.webNfc) },
		OutstandingNetworkRequestFetch: {
			name: N(x.outstandingNetworkRequestFetch),
		},
		OutstandingNetworkRequestXHR: { name: N(x.outstandingNetworkRequestXHR) },
		AppBanner: { name: N(x.appBanner) },
		Printing: { name: N(x.printing) },
		WebDatabase: { name: N(x.webDatabase) },
		PictureInPicture: { name: N(x.pictureInPicture) },
		Portal: { name: N(x.portal) },
		SpeechRecognizer: { name: N(x.speechRecognizer) },
		IdleManager: { name: N(x.idleManager) },
		PaymentManager: { name: N(x.paymentManager) },
		SpeechSynthesis: { name: N(x.speechSynthesis) },
		KeyboardLock: { name: N(x.keyboardLock) },
		WebOTPService: { name: N(x.webOTPService) },
		OutstandingNetworkRequestDirectSocket: {
			name: N(x.outstandingNetworkRequestDirectSocket),
		},
		InjectedJavascript: { name: N(x.injectedJavascript) },
		InjectedStyleSheet: { name: N(x.injectedStyleSheet) },
		Dummy: { name: N(x.internalError) },
		ContentSecurityHandler: { name: N(x.contentSecurityHandler) },
		ContentWebAuthenticationAPI: { name: N(x.contentWebAuthenticationAPI) },
		ContentFileChooser: { name: N(x.contentFileChooser) },
		ContentSerial: { name: N(x.contentSerial) },
		ContentFileSystemAccess: { name: N(x.contentFileSystemAccess) },
		ContentMediaDevicesDispatcherHost: {
			name: N(x.contentMediaDevicesDispatcherHost),
		},
		ContentWebBluetooth: { name: N(x.contentWebBluetooth) },
		ContentWebUSB: { name: N(x.contentWebUSB) },
		ContentMediaSession: { name: N(x.contentMediaSession) },
		ContentMediaSessionService: { name: N(x.contentMediaSessionService) },
		ContentMediaPlay: { name: N(x.contentMediaPlay) },
		ContentScreenReader: { name: N(x.contentScreenReader) },
		EmbedderPopupBlockerTabHelper: { name: N(x.embedderPopupBlockerTabHelper) },
		EmbedderSafeBrowsingTriggeredPopupBlocker: {
			name: N(x.embedderSafeBrowsingTriggeredPopupBlocker),
		},
		EmbedderSafeBrowsingThreatDetails: {
			name: N(x.embedderSafeBrowsingThreatDetails),
		},
		EmbedderAppBannerManager: { name: N(x.embedderAppBannerManager) },
		EmbedderDomDistillerViewerSource: {
			name: N(x.embedderDomDistillerViewerSource),
		},
		EmbedderDomDistillerSelfDeletingRequestDelegate: {
			name: N(x.embedderDomDistillerSelfDeletingRequestDelegate),
		},
		EmbedderOomInterventionTabHelper: {
			name: N(x.embedderOomInterventionTabHelper),
		},
		EmbedderOfflinePage: { name: N(x.embedderOfflinePage) },
		EmbedderChromePasswordManagerClientBindCredentialManager: {
			name: N(x.embedderChromePasswordManagerClientBindCredentialManager),
		},
		EmbedderPermissionRequestManager: {
			name: N(x.embedderPermissionRequestManager),
		},
		EmbedderModalDialog: { name: N(x.embedderModalDialog) },
		EmbedderExtensions: { name: N(x.embedderExtensions) },
		EmbedderExtensionMessaging: { name: N(x.embedderExtensionMessaging) },
		EmbedderExtensionMessagingForOpenPort: {
			name: N(x.embedderExtensionMessagingForOpenPort),
		},
		EmbedderExtensionSentMessageToCachedFrame: {
			name: N(x.embedderExtensionSentMessageToCachedFrame),
		},
		ErrorDocument: { name: N(x.errorDocument) },
		FencedFramesEmbedder: { name: N(x.fencedFramesEmbedder) },
		KeepaliveRequest: { name: N(x.keepaliveRequest) },
		JsNetworkRequestReceivedCacheControlNoStoreResource: {
			name: N(x.jsNetworkRequestReceivedCacheControlNoStoreResource),
		},
		IndexedDBEvent: { name: N(x.indexedDBEvent) },
		CookieDisabled: { name: N(x.cookieDisabled) },
		WebRTCSticky: { name: N(x.webRTCSticky) },
		WebTransportSticky: { name: N(x.webTransportSticky) },
		WebSocketSticky: { name: N(x.webSocketSticky) },
		HTTPAuthRequired: { name: N(x.HTTPAuthRequired) },
		CookieFlushed: { name: N(x.CookieFlushed) },
	},
	P = new CSSStyleSheet();
P.replaceSync(
...
const D = {
	...
	M = t.i18n.registerUIStrings(
		"panels/application/components/BackForwardCacheView.ts",
		D
	),
	B = t.i18n.getLocalizedString.bind(void 0, M),
	V = c.RenderCoordinator.RenderCoordinator.instance();
class F extends l.LegacyWrapper.WrappableComponent {
	static litTagName = m.literal`devtools-resources-back-forward-cache-view`;
	...
	constructor() {
		(super(),
			...
			this.#o()?.addEventListener(
				a.ResourceTreeModel.Events.BackForwardCacheDetailsUpdated,
				this.render,
				this
			));
	}
	#o() {
		const e = a.TargetManager.TargetManager.instance().primaryPageTarget();
		return e?.model(a.ResourceTreeModel.ResourceTreeModel) || null;
	}
	#i() {
		return this.#o()?.mainFrame || null;
	}
	connectedCallback() {
		(this.parentElement?.classList.add("overflow-auto"),
			(this.#e.adoptedStyleSheets = [P]));
	}
	async render() {
		await V.write("BackForwardCacheView render", () => {
			m.render(
				m.html`
	...
	#s() {
		(a.TargetManager.TargetManager.instance().removeModelListener(
			a.ResourceTreeModel.ResourceTreeModel,
			a.ResourceTreeModel.Events.FrameNavigated,
			this.#s,
			this
		),
			(this.#t = "Result"),
	...
	async #c(e) {
		const t = a.TargetManager.TargetManager.instance().primaryPageTarget(),
			r = t?.model(a.ResourceTreeModel.ResourceTreeModel),
			o = await r?.navigationHistory();
		r &&
			o &&
			(o.currentIndex === this.#r
				? window.setTimeout(this.#c.bind(this, 2 * e), e)
				: (a.TargetManager.TargetManager.instance().addModelListener(
					...
					r.navigateToHistoryEntry(o.entries[o.currentIndex - 1])));
	}
	async #d() {
		const e = a.TargetManager.TargetManager.instance().primaryPageTarget(),
			t = e?.model(a.ResourceTreeModel.ResourceTreeModel),
			r = await t?.navigationHistory();
		t &&
			r &&
			((this.#r = r.currentIndex),
			(this.#t = "Running"),
			this.render(),
			a.TargetManager.TargetManager.instance().addModelListener(
				a.ResourceTreeModel.ResourceTreeModel,
	...
	#n() {
      ...
      `;
		const a = "Running" === this.#t,
			o = e.ParsedURL.schemeIs(t.url, "devtools:");
		return m.html`
	...
	#u(e) {
		if (!e || (0 === e.explanations.length && 0 === e.children.length))
			return m.nothing;
		const t = this.#g(e, { blankCount: 1 });
		t.node.treeNodeData.iconName = "frame";
		let a = "";
		a =
			1 === t.frameCount
				? B(D.issuesInSingleFrame, { n: t.issueCount })
				: B(D.issuesInMultipleFrames, { n: t.issueCount, m: t.frameCount });
		const r = {
			treeNodeData: { text: a },
			id: "root",
			children: () => Promise.resolve([t.node]),
		};
		return m.html`
      <div class="report-line"
      jslog=${g.section().context("frames")}>
        ...
        <div class="report-value">
          <${h.TreeOutline.TreeOutline.litTagName} .data=${{
						tree: [r],
						defaultRenderer: function (e) {
							return m.html`
        <div class="text-ellipsis">
          ${
						e.treeNodeData.iconName
							? m.html`
	...
	#g(e, t) {
		...
		e.url.length
			? (i = e.url)
			: ((i = B(D.blankURLTitle, { PH1: t.blankCount })), (t.blankCount += 1));
		for (const t of e.explanations) {
			const e = { treeNodeData: { text: t.reason }, id: String(this.#a++) };
			((r += 1), o.push(e));
		}
		for (const i of e.children) {
			const e = this.#g(i, t);
			e.issueCount > 0 &&
				(o.push(e.node), (r += e.issueCount), (a += e.frameCount));
		}
		let n = { treeNodeData: { text: `(${r}) ${i}` }, id: String(this.#a++) };
		return (
			o.length
				? ((n = { ...n, children: () => Promise.resolve(o) }),
					(n.treeNodeData.iconName = "iframe"))
				: e.url.length || (t.blankCount -= 1),
			{ node: n, frameCount: a, issueCount: r }
		);
	}
	#h(e) {
		switch (e) {
	...
	#p(e, t, a) {
		let r = e.url;
		(0 === r.length &&
			((r = B(D.blankURLTitle, { PH1: t.blankCount })), (t.blankCount += 1)),
			e.explanations.forEach((e) => {
				let t = a.get(e.reason);
				void 0 === t ? ((t = [r]), a.set(e.reason, t)) : t.push(r);
			}),
			e.children.map((e) => {
				this.#p(e, t, a);
			}));
	...
	#m(e, t) {
		if (0 === e.length) return m.nothing;
		const a = e.filter((e) => "PageSupportNeeded" === e.type),
			r = e.filter((e) => "SupportPending" === e.type),
			o = e.filter((e) => "Circumstantial" === e.type),
			i = new Map();
		return (
			t && this.#p(t, { blankCount: 1 }, i),
			m.html`
	...
	#f(e) {
		if ("EmbedderExtensionSentMessageToCachedFrame" === e.reason && e.context) {
			const t = "chrome://extensions/?id=" + e.context;
			return m.html`${B(D.blockingExtensionId)}
      <${o.ChromeLink.ChromeLink.litTagName} .href=${t}>${e.context}</${o.ChromeLink.ChromeLink.litTagName}>`;
		}
		return m.nothing;
	}
	#v(e) {
		if (void 0 === e || 0 === e.length) return m.nothing;
		const t = [m.html`<div>${B(D.framesPerIssue, { n: e.length })}</div>`];
		return (
			t.push(
				...e.map(
					(e) => m.html`<div class="text-ellipsis" title=${e}
    jslog=${g.treeItem()}>${e}</div>`
				)
			),
			m.html`
	...
	#k(e) {
		return "UnloadHandlerExistsInMainFrame" === e.reason ||
			"UnloadHandlerExistsInSubFrame" === e.reason
			? m.html`
	...
	#y(e) {
		if (void 0 === e || 0 === e.length) return m.nothing;
		const t = new u.Linkifier.Linkifier(50),
			a = [m.html`<div>${B(D.filesPerIssue, { n: e.length })}</div>`];
		return (
			a.push(
				...e.map(
					(e) =>
						m.html`${t.linkifyScriptLocation(null, null, e.url, e.lineNumber, { columnNumber: e.columnNumber, showColumnNumber: !0, inlineFrameIndex: 0 })}`
				)
			),
			m.html`
      <div class="details-list">
...
n.CustomElements.defineComponent(
	"devtools-resources-back-forward-cache-view",
	F
);
var E = Object.freeze({ __proto__: null, BackForwardCacheView: F });
...
const A = {
	...
	H = t.i18n.registerUIStrings(
		"panels/application/components/BounceTrackingMitigationsView.ts",
		A
	),
	O = t.i18n.getLocalizedString.bind(void 0, H);
class W extends l.LegacyWrapper.WrappableComponent {
	static litTagName = m.literal`devtools-bounce-tracking-mitigations-view`;
	#e = this.attachShadow({ mode: "open" });
	#T = [];
	#t = "Result";
	#S = !1;
	#R = !1;
	connectedCallback() {
		((this.#e.adoptedStyleSheets = [L]), this.#$());
	}
	async #$() {
		m.render(
			m.html`
	...
	async #n() {
		if ((this.#S || (await this.#x()), "Disabled" === this.#t)) {
			const e = new o.ChromeLink.ChromeLink();
			return (
				(e.href = "chrome://flags/#bounce-tracking-mitigations"),
				(e.textContent = O(A.featureFlag)),
				m.html`
        <${d.ReportView.ReportSection.litTagName}>
	...
	#N() {
      ...
      `;
		const e = {
			...
			rows: this.#P(),
			initialSort: { columnId: "sites", direction: "ASC" },
		};
		return m.html`
      <${d.ReportView.ReportSection.litTagName}>
	...
	async #I() {
		const e = a.TargetManager.TargetManager.instance().primaryPageTarget();
		if (!e) return;
		((this.#R = !0), (this.#t = "Running"), this.#$());
		const t = await e.storageAgent().invoke_runBounceTrackingMitigations();
		((this.#T = []),
			t.deletedSites.forEach((e) => {
				this.#T.push(e);
			}),
			this.#D());
	}
	...
	#P() {
		return this.#T.map((e) => ({ cells: [{ columnId: "sites", value: e }] }));
	}
	async #x() {
		this.#S = !0;
		const e = a.TargetManager.TargetManager.instance().primaryPageTarget();
		e &&
			((await e.systemInfo().invoke_getFeatureState({ featureState: "DIPS" }))
				.featureEnabled ||
				(this.#t = "Disabled"));
	}
}
...
var U = Object.freeze({
	__proto__: null,
	i18nString: O,
	BounceTrackingMitigationsView: W,
});
...
const _ = { noEndpointsToDisplay: "No endpoints to display" },
	z = t.i18n.registerUIStrings(
		"panels/application/components/EndpointsGrid.ts",
		_
	),
	G = t.i18n.getLocalizedString.bind(void 0, z),
	{ render: j, html: K } = m;
class J extends HTMLElement {
	static litTagName = m.literal`devtools-resources-endpoints-grid`;
	#e = this.attachShadow({ mode: "open" });
	#M = new Map();
	connectedCallback() {
		((this.#e.adoptedStyleSheets = [q]), this.#$());
	}
	set data(e) {
		((this.#M = e.endpoints), this.#$());
	}
	#$() {
		const e = {
			columns: [
				{
					id: "origin",
					title: t.i18n.lockedString("Origin"),
					widthWeighting: 30,
					hideable: !1,
					visible: !0,
				},
				{
					id: "name",
					title: t.i18n.lockedString("Name"),
					widthWeighting: 20,
					hideable: !1,
					visible: !0,
				},
	...
	#B() {
		return Array.from(this.#M)
			.map(([e, t]) =>
				t.map((t) => ({
					cells: [
						{ columnId: "origin", value: e },
						{ columnId: "name", value: t.groupName },
						{ columnId: "url", value: t.url },
					],
				}))
			)
			.flat();
...
n.CustomElements.defineComponent("devtools-resources-endpoints-grid", J);
var X = Object.freeze({ __proto__: null, i18nString: G, EndpointsGrid: J });
...
const te = {
	...
	ae = t.i18n.registerUIStrings(
		"panels/application/components/OriginTrialTreeView.ts",
		te
	),
	re = t.i18n.getLocalizedString.bind(void 0, ae);
class oe extends HTMLElement {
	static litTagName = m.literal`devtools-resources-origin-trial-tree-view-badge`;
	#e = this.attachShadow({ mode: "open" });
	#V = new T.Adorner.Adorner();
	set data(e) {
		this.#$(e);
	}
	connectedCallback() {
		this.#e.adoptedStyleSheets = [Q];
	}
	#$(e) {
		const t = document.createElement("span");
		((t.textContent = e.badgeContent),
			(this.#V.data = { name: "badge", content: t }),
			this.#V.classList.add(`badge-${e.style}`),
			m.render(
				m.html`
      ${this.#V}
    `,
...
function ie(e) {
	return {
		treeNodeData: e,
		id: "OriginTrialTreeNode#" + e.trialName,
		children: async () =>
			e.tokensWithStatus.length > 1
				? e.tokensWithStatus.map(ne)
				: le(e.tokensWithStatus[0]),
		renderer: (e) => {
			const t = e.treeNodeData,
				a = m.html`
...
function ne(e) {
	return {
		treeNodeData: e.status,
		id: "TokenNode#" + e.rawTokenText,
		children: async () => le(e),
		renderer: (e, t) => {
			const a = e.treeNodeData,
				r = m.html`
        <${oe.litTagName} .data=${{ badgeContent: a, style: "Success" === a ? "success" : "error" }}></${oe.litTagName}>
      `;
			return m.html`${re(te.token)} ${t.isExpanded ? m.nothing : r}`;
		},
	};
}
function se(e) {
...
function le(e) {
	return [
		{ treeNodeData: e, id: "TokenDetailsNode#" + e.rawTokenText, renderer: se },
		((t = e.rawTokenText),
		{
			treeNodeData: re(te.rawTokenText),
			id: "TokenRawTextContainerNode#" + t,
			children: async () => [
				{
					treeNodeData: t,
					id: "TokenRawTextNode#" + t,
					renderer: (e) => {
						const t = e.treeNodeData;
						return m.html`
...
function ce(e) {
	return m.html`${String(e.treeNodeData)}`;
}
n.CustomElements.defineComponent(
...
class de extends HTMLElement {
	static litTagName = m.literal`devtools-resources-origin-trial-token-rows`;
	#e = this.attachShadow({ mode: "open" });
	#F = null;
	#E = [];
	...
	set data(e) {
		((this.#F = e.node.treeNodeData), this.#A());
	}
	connectedCallback() {
		((this.#e.adoptedStyleSheets = [Z]), this.#$());
	}
	#H = (e, t) => m.html`
			...
			((this.#E = [
				{
					name: re(te.origin),
					value: this.#H(
						this.#F.parsedToken.origin,
						"WrongOrigin" === this.#F.status
					),
				},
				{
					name: re(te.expiryTime),
					value: this.#H(
						this.#L.format(1e3 * this.#F.parsedToken.expiryTime),
						"Expired" === this.#F.status
					),
				},
				{
					name: re(te.usageRestriction),
					value: this.#H(this.#F.parsedToken.usageRestriction),
				},
				{
					name: re(te.isThirdParty),
					value: this.#H(this.#F.parsedToken.isThirdParty.toString()),
				},
				{
					name: re(te.matchSubDomains),
					value: this.#H(this.#F.parsedToken.matchSubDomains.toString()),
				},
			]),
			"UnknownTrial" === this.#F.status &&
				(this.#E = [
					{
						name: re(te.trialName),
						value: this.#H(this.#F.parsedToken.trialName),
					},
					...this.#E,
				]));
	}
		...
		const e = [
			{
				name: re(te.status),
				value: m.html`
          <${oe.litTagName} .data=${{ badgeContent: this.#F.status, style: "Success" === this.#F.status ? "success" : "error" }}></${oe.litTagName}>`,
			},
			...this.#E,
		].map(
			(e) => m.html`
...
class he extends HTMLElement {
	static litTagName = m.literal`devtools-resources-origin-trial-tree-view`;
	#e = this.attachShadow({ mode: "open" });
	set data(e) {
		this.#$(e.trials);
	}
	connectedCallback() {
		this.#e.adoptedStyleSheets = [ee];
	}
	#$(e) {
		e.length
			? m.render(
...
var ue = Object.freeze({
	__proto__: null,
	Badge: oe,
	OriginTrialTokenRows: de,
	OriginTrialTreeView: he,
});
const me = new CSSStyleSheet();
me.replaceSync(
...
const ge = {
	...
	pe = t.i18n.registerUIStrings(
		"panels/application/components/PermissionsPolicySection.ts",
		ge
	),
	be = t.i18n.getLocalizedString.bind(void 0, pe),
	we = c.RenderCoordinator.RenderCoordinator.instance();
function fe(e, t, a) {
	return m.html`
...
class ve extends HTMLElement {
	static litTagName = m.literal`devtools-resources-permissions-policy-section`;
	#e = this.attachShadow({ mode: "open" });
	#O = { policies: [], showDetails: !1 };
	set data(e) {
		((this.#O = e), this.#$());
	}
	connectedCallback() {
		this.#e.adoptedStyleSheets = [me];
	}
	#W() {
		((this.#O.showDetails = !this.#O.showDetails), this.#$());
	}
	#U() {
		const e = this.#O.policies
			.filter((e) => e.allowed)
			.map((e) => e.feature)
			.sort();
		return e.length
	...
	async #q() {
		const t = this.#O.policies
			.filter((e) => !e.allowed)
			.sort((e, t) => e.feature.localeCompare(t.feature));
		if (!t.length) return m.nothing;
		if (!this.#O.showDetails)
			return m.html`
      ...
      `;
		const r = a.FrameManager.FrameManager.instance(),
			o = await Promise.all(
				t.map(async (t) => {
					const a = t.locator ? r.getFrame(t.locator.frameId) : null,
						o = t.locator?.blockReason,
						i = await ("IframeAttribute" === o &&
							a &&
							a.getOwnerDOMNodeOrDocument()),
						n = a && a.resourceForURL(a.url),
						l = "Header" === o && n && n.request,
						c = (() => {
							switch (o) {
								case "IframeAttribute":
									return be(ge.disabledByIframe);
								case "Header":
									return be(ge.disabledByHeader);
								case "InFencedFrameTree":
									return be(ge.disabledByFencedFrame);
								default:
									return "";
							}
						})();
            ...
            ${
							l
								? fe(
										...
										be(ge.clickToShowHeader),
										async () => {
											if (!l) return;
											const t = l.responseHeaderValue("permissions-policy")
													? "permissions-policy"
													: "feature-policy",
												a =
													k.UIRequestLocation.UIRequestLocation.responseHeaderMatch(
														l,
														{ name: t, value: "" }
													);
											await e.Revealer.reveal(a);
										}
									)
								: m.nothing
						}
	...
	async #$() {
		await we.write("PermissionsPolicySection render", () => {
			m.render(
				m.html`
...
const Te = {
		...
		showSMoreFrames:
			"{n, plural, =1 {Show # more frame} other {Show # more frames}}",
		showLess: "Show less",
	},
	Se = t.i18n.registerUIStrings(
		"panels/application/components/StackTrace.ts",
		Te
	),
	Re = t.i18n.getLocalizedString.bind(void 0, Se);
class $e extends HTMLElement {
	static litTagName = m.literal`devtools-stack-trace-row`;
	#e = this.attachShadow({ mode: "open" });
	#_ = null;
	set data(e) {
		((this.#_ = e.stackTraceRowItem), this.#$());
	}
	connectedCallback() {
		this.#e.adoptedStyleSheets = [ke];
	}
	#$() {
		this.#_ &&
			m.render(
...
class xe extends HTMLElement {
	static litTagName = m.literal`devtools-stack-trace-link-button`;
	#e = this.attachShadow({ mode: "open" });
	#z = () => {};
	#G = null;
	#j = !1;
	set data(e) {
		((this.#z = e.onShowAllClick),
			(this.#G = e.hiddenCallFramesCount),
			(this.#j = e.expandedView),
			this.#$());
	}
	connectedCallback() {
		this.#e.adoptedStyleSheets = [ye];
	}
	#$() {
		if (!this.#G) return;
		const e = this.#j
			? Re(Te.showLess)
			: Re(Te.showSMoreFrames, { n: this.#G });
		m.render(
			m.html`
      <div class="stack-trace-row">
...
class Ce extends HTMLElement {
	static litTagName = m.literal`devtools-resources-stack-trace`;
	#e = this.attachShadow({ mode: "open" });
	#K = new u.Linkifier.Linkifier();
	#J = [];
	#X = !1;
	set data(e) {
		const t = e.frame,
			{ creationStackTrace: a, creationStackTraceTarget: r } =
				t.getCreationStackTraceData();
		(a &&
			(this.#J = e.buildStackTraceRows(a, r, this.#K, !0, this.#Y.bind(this))),
			this.#$());
	}
	#Y(e) {
		((this.#J = e), this.#$());
	...
	createRowTemplates() {
		...
		for (const a of this.#J)
			((!this.#X && a.ignoreListHide) ||
				("functionName" in a &&
					e.push(m.html`
          <${$e.litTagName} data-stack-trace-row .data=${{ stackTraceRowItem: a }}></${$e.litTagName}>`),
				"asyncDescription" in a &&
					e.push(m.html`
            <div>${a.asyncDescription}</div>
          `)),
				"functionName" in a && a.ignoreListHide && t++);
		return (
			t &&
				e.push(m.html`
	...
	#$() {
		...
		const e = this.createRowTemplates();
		m.render(
			m.html`
...
(n.CustomElements.defineComponent("devtools-stack-trace-row", $e),
	n.CustomElements.defineComponent("devtools-stack-trace-link-button", xe),
	n.CustomElements.defineComponent("devtools-resources-stack-trace", Ce));
var Ne = Object.freeze({
	__proto__: null,
	StackTraceRow: $e,
	StackTraceLinkButton: xe,
	StackTrace: Ce,
});
const Ie = {
		...
		clickToRevealInNetworkPanelMight:
			"Click to reveal in Network panel (might require page reload)",
		origin: "Origin",
		ownerElement: "Owner Element",
		clickToRevealInElementsPanel: "Click to reveal in Elements panel",
		adStatus: "Ad Status",
		rootDescription:
			"This frame has been identified as the root frame of an ad",
		root: "root",
		childDescription:
			"This frame has been identified as a child frame of an ad",
		child: "child",
		...
		available: "available",
		thePerformanceAPI:
			"The `performance.measureUserAgentSpecificMemory()` API is available",
		...
		none: "None",
		originTrialsExplanation:
			"Origin trials give you access to a new or experimental feature.",
	},
	Pe = t.i18n.registerUIStrings(
		"panels/application/components/FrameDetailsView.ts",
		Ie
	),
	De = t.i18n.getLocalizedString.bind(void 0, Pe),
	Me = c.RenderCoordinator.RenderCoordinator.instance();
class Be extends l.LegacyWrapper.WrappableComponent {
	static litTagName = m.literal`devtools-resources-frame-details-view`;
	#e = this.attachShadow({ mode: "open" });
	#Z;
	#ee;
	#te = !1;
	...
	#oe = null;
	constructor(e) {
		(super(), (this.#Z = e), this.render());
	}
	connectedCallback() {
		(this.parentElement?.classList.add("overflow-auto"),
			(this.#te = w.Runtime.experiments.isEnabled("protocolMonitor")),
			(this.#e.adoptedStyleSheets = [Y]));
	}
	async render() {
		this.#oe =
			(await this.#Z?.parentFrame()?.getAdScriptId(this.#Z?.id)) || null;
		const e = this.#oe?.debuggerId
			? await a.DebuggerModel.DebuggerModel.modelForDebuggerId(
					this.#oe?.debuggerId
				)
			: null;
		((this.#ee = e?.target()),
			!this.#ae && this.#Z && (this.#ae = this.#Z.getPermissionsPolicyState()),
			await Me.write("FrameDetailsView render", () => {
				this.#Z &&
					m.render(
						m.html`
	...
	#le() {
		return this.#Z
			? (this.#re.classList.add("span-cols"),
				this.#Z.getOriginTrials().then((e) => {
					this.#re.data = { trials: e };
				}),
				m.html`
	...
	#de() {
		if (!this.#Z || this.#Z.unreachableUrl()) return m.nothing;
		const t = this.#we(this.#Z);
		return fe("breakpoint-circle", De(Ie.clickToRevealInSourcesPanel), () =>
			e.Revealer.reveal(t)
		);
	}
	#he() {
		if (this.#Z) {
			const t = this.#Z.resourceForURL(this.#Z.url);
			if (t && t.request) {
				const a = t.request;
				return fe(
					"arrow-up-down-circle",
					De(Ie.clickToRevealInNetworkPanel),
					() => {
						const t = k.UIRequestLocation.UIRequestLocation.tab(
							a,
							k.UIRequestLocation.UIRequestTabs.HeadersComponent
						);
						return e.Revealer.reveal(t);
					}
				);
			}
		}
	...
	#we(e) {
		for (const t of v.Workspace.WorkspaceImpl.instance().projects()) {
			const a = f.NetworkProject.NetworkProject.getTargetForProject(t);
			if (a && a === e.resourceTreeModel().target()) {
				const a = t.uiSourceCodeForURL(e.url);
				if (a) return a;
			}
		}
		return null;
	...
	#ue() {
		return this.#Z && this.#Z.unreachableUrl()
			? m.html`
	...
	#fe() {
		if (this.#Z) {
			const t = e.ParsedURL.ParsedURL.fromString(this.#Z.unreachableUrl());
			if (t)
				return fe(
					"arrow-up-down-circle",
					De(Ie.clickToRevealInNetworkPanelMight),
					() => {
						e.Revealer.reveal(
							k.UIFilter.UIRequestFilter.filters([
								{
									filterType: k.UIFilter.FilterType.Domain,
									filterValue: t.domain(),
								},
								{ filterType: null, filterValue: t.path },
							])
						);
					}
				);
	...
	#me() {
		return this.#Z && this.#Z.securityOrigin && "://" !== this.#Z.securityOrigin
			? m.html`
	...
	async #ge() {
		if (this.#Z) {
			const t = await this.#Z.getOwnerDOMNodeOrDocument();
			if (t)
				return m.html`
	...
	#pe() {
		const e = this.#Z?.getCreationStackTraceData();
		return e && e.creationStackTrace
			? m.html`
	...
	#ve(e) {
		switch (e) {
			case "child":
				return { value: De(Ie.child), description: De(Ie.childDescription) };
			case "root":
				return { value: De(Ie.root), description: De(Ie.rootDescription) };
		}
	}
	#ke(e) {
		switch (e) {
			case "CreatedByAdScript":
				return De(Ie.createdByAdScriptExplanation);
			case "MatchedBlockingRule":
				return De(Ie.matchedBlockingRuleExplanation);
			case "ParentIsAd":
				return De(Ie.parentIsAdExplanation);
		}
	}
	#be() {
		if (!this.#Z) return m.nothing;
		const e = this.#Z.adFrameType();
		if ("none" === e) return m.nothing;
		const t = this.#ve(e),
			a = [m.html`<div title=${t.description}>${t.value}</div>`];
		for (const e of this.#Z.adFrameStatus()?.explanations || [])
			a.push(m.html`<div>${this.#ke(e)}</div>`);
		const r = this.#ee
			? this.#K.linkifyScriptLocation(
					this.#ee,
					this.#oe?.scriptId || null,
					b.DevToolsPath.EmptyUrlString,
					void 0,
					void 0
				)
			: null;
	...
	#ye() {
		const e = this.#Se();
		return e ? m.html`<span class="inline-comment">${e}</span>` : m.nothing;
	}
	#Se() {
		switch (this.#Z?.getSecureContextType()) {
			case "Secure":
				return null;
			case "SecureLocalhost":
				return De(Ie.localhostIsAlwaysASecureContext);
			case "InsecureAncestor":
				return De(Ie.aFrameAncestorIsAnInsecure);
			case "InsecureScheme":
				return De(Ie.theFramesSchemeIsInsecure);
		}
		return null;
	}
	async #Te() {
		if (this.#Z) {
			const e = this.#Z
					.resourceTreeModel()
					.target()
					.model(a.NetworkManager.NetworkManager),
				r = e && (await e.getSecurityIsolationStatus(this.#Z.id));
			if (r)
				return m.html`
	...
	#Re(e, t, a) {
		if (!e) return m.nothing;
		const r = e.value !== a,
			o = !r && e.reportOnlyValue !== a,
			i = r ? e.reportingEndpoint : e.reportOnlyReportingEndpoint;
		return m.html`
	...
	#xe(e) {
		const t = new y.CspParser.CspParser(e).csp.directives,
			a = [];
		for (const e in t)
			a.push(
				m.html`<div><span class="bold">${e}</span>${": " + t[e]?.join(", ")}</div>`
			);
		return a;
	}
	#Ce(e) {
      ...
      <${d.ReportView.ReportKey.litTagName}>${
				e.isEnforced
					...
					: m.html`${t.i18n.lockedString("Content-Security-Policy-Report-Only")}<x-link
            class="link"
	...
	#Ne() {
		if (this.#Z) {
			const e = this.#Z.getGatedAPIFeatures();
			if (e) {
				const t = e.includes("SharedArrayBuffers"),
					a = t && e.includes("SharedArrayBuffersTransferAllowed"),
					r = De(
						a
							? Ie.availableTransferable
							: t
								? Ie.availableNotTransferable
								: Ie.unavailable
					),
					o = a
						? De(Ie.sharedarraybufferConstructorIs)
						: t
							? De(Ie.sharedarraybufferConstructorIsAvailable)
							: "";
				function i(e) {
					switch (e.getCrossOriginIsolatedContextType()) {
						case "Isolated":
							return m.nothing;
						case "NotIsolated":
							return t
								? m.html`<span class="inline-comment">${De(Ie.willRequireCrossoriginIsolated)}</span>`
								: m.html`<span class="inline-comment">${De(Ie.requiresCrossoriginIsolated)}</span>`;
						case "NotIsolatedFeatureDisabled":
							if (!a)
								return m.html`<span class="inline-comment">${De(Ie.transferRequiresCrossoriginIsolatedPermission)} <code>cross-origin-isolated</code></span>`;
					}
					return m.nothing;
				}
				return m.html`
	...
	#Ie() {
		if (this.#Z) {
			const e = this.#Z.isCrossOriginIsolated(),
				t = De(e ? Ie.available : Ie.unavailable),
				a = De(
					e
						? Ie.thePerformanceAPI
						: Ie.thePerformancemeasureuseragentspecificmemory
				);
			return m.html`
...
n.CustomElements.defineComponent("devtools-resources-frame-details-view", Be);
var Ve = Object.freeze({ __proto__: null, FrameDetailsReportView: Be });
...
const Ee = {
	...
	Le = t.i18n.registerUIStrings(
		"panels/application/components/InterestGroupAccessGrid.ts",
		Ee
	),
	Ae = t.i18n.getLocalizedString.bind(void 0, Le);
class He extends HTMLElement {
	static litTagName = m.literal`devtools-interest-group-access-grid`;
	#e = this.attachShadow({ mode: "open" });
	#Pe = [];
	connectedCallback() {
		((this.#e.adoptedStyleSheets = [Fe]), this.#$());
	}
	set data(e) {
		((this.#Pe = e), this.#$());
	}
	...
	#De() {
		if (0 === this.#Pe.length)
			return m.html`<div class="no-events-message">${Ae(Ee.noEvents)}</div>`;
		const e = {
			columns: [
				{
					id: "event-time",
					title: Ae(Ee.eventTime),
					widthWeighting: 10,
					hideable: !1,
					visible: !0,
					sortable: !0,
			...
			rows: this.#Me(),
			initialSort: { columnId: "event-time", direction: "ASC" },
		};
		return m.html`
	...
	#Me() {
		return this.#Pe.map((e) => ({
			cells: [
				{
					columnId: "event-time",
					value: e.accessTime,
					renderer: this.#Be.bind(this),
				},
				{ columnId: "event-type", value: e.type },
				{ columnId: "event-group-owner", value: e.ownerOrigin },
				{ columnId: "event-group-name", value: e.name },
			],
		}));
	}
	#Be(e) {
		const t = new Date(1e3 * e);
		return m.html`${t.toLocaleString()}`;
	}
}
n.CustomElements.defineComponent("devtools-interest-group-access-grid", He);
var Oe = Object.freeze({
	__proto__: null,
	i18nString: Ae,
	InterestGroupAccessGrid: He,
});
const We = new CSSStyleSheet();
We.replaceSync(
...
const qe = {
	...
	_e = t.i18n.registerUIStrings(
		"panels/application/components/ProtocolHandlersView.ts",
		qe
	),
	ze = t.i18n.getLocalizedString.bind(void 0, _e);
class Ge extends HTMLElement {
	static litTagName = m.literal`devtools-protocol-handlers-view`;
	#e = this.attachShadow({ mode: "open" });
	#Ve = [];
	#Fe = b.DevToolsPath.EmptyUrlString;
	#Ee = "";
	#Le = "";
	set data(e) {
		const t = this.#Fe !== e.manifestLink;
		((this.#Ve = e.protocolHandlers),
			(this.#Fe = e.manifestLink),
			t && this.#Ae());
	}
	#Ae() {
		((this.#Le = ""), (this.#Ee = this.#Ve[0]?.protocol ?? ""), this.#$());
	}
	#He() {
		const e = $.XLink.XLink.create(
				this.#Fe,
				ze(qe.manifest),
				void 0,
				void 0,
			...
			a = this.#Ve.length > 0 ? qe.protocolDetected : qe.protocolNotDetected,
			r =
				this.#Ve.length > 0
					? {
	...
	#Oe() {
		if (0 === this.#Ve.length) return m.nothing;
		const e = this.#Ve
			.filter((e) => e.protocol)
			.map(
				(e) => m.html`<option value=${e.protocol}>${e.protocol}://</option>`
			);
		return m.html`
	...
	#We = (e) => {
		this.#Ee = e.target.value;
	};
	#Ue = (e) => {
		((this.#Le = e.target.value), this.#$());
	};
	#qe = () => {
		const e = `${this.#Ee}://${this.#Le}`;
		(S.InspectorFrontendHost.InspectorFrontendHostInstance.openInNewTab(e),
			S.userMetrics.actionTaken(
				S.UserMetrics.Action.CaptureTestProtocolClicked
			));
	};
	connectedCallback() {
		this.#e.adoptedStyleSheets = [Ue, We, R.textInputStyles];
	}
	#$() {
		const e = $.XLink.XLink.create(
			"https://web.dev/url-protocol-handler/",
			ze(qe.protocolHandlerRegistrations),
			void 0,
			void 0,
			"learn-more"
		);
...
n.CustomElements.defineComponent("devtools-protocol-handlers-view", Ge);
var je = Object.freeze({ __proto__: null, ProtocolHandlersView: Ge });
const Ke = {
	...
	Je = t.i18n.registerUIStrings(
		"panels/application/components/ReportsGrid.ts",
		Ke
	),
	Xe = t.i18n.getLocalizedString.bind(void 0, Je),
	{ render: Ye, html: Qe } = m;
class Ze extends HTMLElement {
	static litTagName = m.literal`devtools-resources-reports-grid-status-header`;
	#e = this.attachShadow({ mode: "open" });
	connectedCallback() {
		((this.#e.adoptedStyleSheets = [q]), this.#$());
	}
...
class et extends HTMLElement {
	static litTagName = m.literal`devtools-resources-reports-grid`;
	#e = this.attachShadow({ mode: "open" });
	#_e = [];
	#te = !1;
	connectedCallback() {
	...
	#$() {
		const e = {
			columns: [
				...
				{
					id: "type",
					title: t.i18n.lockedString("Type"),
					widthWeighting: 20,
					hideable: !1,
					visible: !0,
				},
          ...
          `,
				...
				{
					id: "body",
					title: t.i18n.lockedString("Body"),
					widthWeighting: 20,
					hideable: !1,
					visible: !0,
				},
	...
	#B() {
		return this.#_e.map((e) => ({
			cells: [
				{ columnId: "id", value: e.id },
				{ columnId: "url", value: e.initiatorUrl },
				{ columnId: "type", value: e.type },
				{ columnId: "status", value: e.status },
				{ columnId: "destination", value: e.destination },
				{
					columnId: "timestamp",
					value: new Date(1e3 * e.timestamp).toLocaleString(),
				},
				{ columnId: "body", value: JSON.stringify(e.body) },
			],
		}));
	}
}
(n.CustomElements.defineComponent(
	...
	n.CustomElements.defineComponent("devtools-resources-reports-grid", et));
var tt = Object.freeze({
	__proto__: null,
	i18nString: Xe,
	ReportsGridStatusHeader: Ze,
	ReportsGrid: et,
});
...
const { html: rt, render: ot } = m;
class it extends l.LegacyWrapper.WrappableComponent {
	static litTagName = m.literal`devtools-service-worker-router-view`;
	#e = this.attachShadow({ mode: "open" });
	#ze = [];
	connectedCallback() {
		this.#e.adoptedStyleSheets = [at];
	}
	update(e) {
		((this.#ze = e), this.#ze.length > 0 && this.#$());
	}
	#$() {
		ot(
			rt`
...
n.CustomElements.defineComponent("devtools-service-worker-router-view", it);
var nt = Object.freeze({ __proto__: null, ServiceWorkerRouterView: it });
...
const lt = {
	...
	ct = t.i18n.registerUIStrings(
		"panels/application/components/SharedStorageAccessGrid.ts",
		lt
	),
	dt = t.i18n.getLocalizedString.bind(void 0, ct);
class ht extends HTMLElement {
	static litTagName = m.literal`devtools-shared-storage-access-grid`;
	#e = this.attachShadow({ mode: "open" });
	#Pe = [];
	connectedCallback() {
		((this.#e.adoptedStyleSheets = [st]), this.#$());
	}
	set data(e) {
		((this.#Pe = e), this.#$());
	}
	...
	#De() {
		if (0 === this.#Pe.length)
			return m.html`<div
        class="no-events-message">${dt(lt.noEvents)}</div>`;
		const e = {
			columns: [
				{
					id: "event-main-frame-id",
					title: dt(lt.mainFrameId),
					widthWeighting: 10,
					hideable: !1,
					visible: !1,
					sortable: !1,
	...
	#Me() {
		return this.#Pe.map((e) => ({
			cells: [
				{ columnId: "event-main-frame-id", value: e.mainFrameId },
				{
					columnId: "event-time",
					value: e.accessTime,
					renderer: this.#Be.bind(this),
				...
				{ columnId: "event-type", value: e.type },
				{ columnId: "event-owner-origin", value: e.ownerOrigin },
				{ columnId: "event-params", value: JSON.stringify(e.params) },
			],
		}));
	}
	#Be(e) {
...
n.CustomElements.defineComponent("devtools-shared-storage-access-grid", ht);
var ut = Object.freeze({
	__proto__: null,
	i18nString: dt,
	SharedStorageAccessGrid: ht,
});
...
const gt = {
		...
		yesBecauseOriginNotInTopLevelSite:
	...
	pt = t.i18n.registerUIStrings(
		"panels/application/components/StorageMetadataView.ts",
		gt
	),
	bt = t.i18n.getLocalizedString.bind(void 0, pt),
	wt = c.RenderCoordinator.RenderCoordinator.instance();
class ft extends l.LegacyWrapper.WrappableComponent {
	static litTagName = m.literal`devtools-storage-metadata-view`;
	#e = this.attachShadow({ mode: "open" });
	#je;
	#Ke = null;
	#Je = null;
	...
	setStorageKey(e) {
		((this.#Ke = a.StorageKeyManager.parseStorageKey(e)), this.render());
	}
	setStorageBucket(e) {
		((this.#Je = e), this.setStorageKey(e.bucket.storageKey));
	}
	enableStorageBucketControls(e) {
		((this.#je = e), this.#Ke && this.render());
	}
	render() {
		return wt.write("StorageMetadataView render", async () => {
			m.render(
				m.html`
	...
	getTitle() {
		if (!this.#Ke) return;
		const e = this.#Ke.origin,
			t = this.#Je?.bucket.name || bt(gt.defaultBucket);
		return this.#je ? `${t} - ${e}` : e;
	}
	key(e) {
		return m.html`<${d.ReportView.ReportKey.litTagName}>${e}</${d.ReportView.ReportKey.litTagName}>`;
	}
	value(e) {
		return m.html`<${d.ReportView.ReportValue.litTagName}>${e}</${d.ReportView.ReportValue.litTagName}>`;
	}
	async renderReportContent() {
		if (!this.#Ke) return m.nothing;
		const e = this.#Ke.origin,
			t = Boolean(this.#Ke.components.get("3")),
			a = Boolean(this.#Ke.components.get("1")),
			r = Boolean(this.#Ke.components.get("4")),
			o = this.#Ke.components.get("0"),
			i = t
				? bt(gt.yesBecauseAncestorChainHasCrossSite)
				: a
					? bt(gt.yesBecauseKeyIsOpaque)
					: r
						? bt(gt.yesBecauseTopLevelIsOpaque)
						: o && e !== o
							? bt(gt.yesBecauseOriginNotInTopLevelSite)
							: null;
		return m.html`
        ...
        ${this.#je ? this.#Ye() : m.nothing}`;
	}
	#Xe() {
		if (!this.#Je)
			throw new Error(
				"Should not call #renderStorageBucketInfo if #bucket is null."
			);
		const {
			bucket: { name: e },
			persistent: t,
	...
	#Qe() {
		if (!this.#Je)
			throw new Error(
				"Should not call #getExpirationString if #bucket is null."
			);
		const { expiration: e } = this.#Je;
		return 0 === e ? bt(gt.none) : new Date(1e3 * e).toLocaleString();
	}
	#Ye() {
		return m.html`
      <${d.ReportView.ReportSection.litTagName}>
	...
	async #Ze() {
		...
		(await $.UIUtils.ConfirmDialog.show(
			bt(gt.confirmBucketDeletion, { PH1: this.#Je.bucket.name || "" }),
			this
		)) && this.#je.deleteBucket(this.#Je.bucket);
	}
}
n.CustomElements.defineComponent("devtools-storage-metadata-view", ft);
var vt = Object.freeze({ __proto__: null, StorageMetadataView: ft });
const kt = {
		sharedStorage: "Shared storage",
	...
	yt = t.i18n.registerUIStrings(
		"panels/application/components/SharedStorageMetadataView.ts",
		kt
	),
	Tt = t.i18n.getLocalizedString.bind(void 0, yt);
class St extends ft {
	static litTagName = m.literal`devtools-shared-storage-metadata-view`;
	#et;
	#tt = null;
	#at = 0;
	#rt = 0;
	constructor(e, t) {
		(super(),
			...
			this.setStorageKey(t));
	}
	async #ot() {
		(await this.#et.resetBudget(), await this.render());
	}
	connectedCallback() {
		this.getShadow().adoptedStyleSheets = [mt];
	}
	getTitle() {
		return Tt(kt.sharedStorage);
	}
	async renderReportContent() {
		const e = await this.#et.getMetadata();
		return (
			(this.#tt = e?.creationTime ?? null),
			(this.#at = e?.length ?? 0),
			(this.#rt = e?.remainingBudget ?? 0),
			m.html`
      ...
      ${this.value(m.html`${this.#rt}${this.#nt()}`)}`
		);
	}
	#it() {
		if (!this.#tt) return m.html`${Tt(kt.notYetCreated)}`;
		const e = new Date(1e3 * this.#tt);
		return m.html`${e.toLocaleString()}`;
	}
	#nt() {
		return m.html`
...
n.CustomElements.defineComponent("devtools-shared-storage-metadata-view", St);
var Rt = Object.freeze({ __proto__: null, SharedStorageMetadataView: St });
...
const xt = {
	...
	Nt = t.i18n.getLocalizedString.bind(void 0, Ct),
	It = c.RenderCoordinator.RenderCoordinator.instance();
class Pt extends l.LegacyWrapper.WrappableComponent {
	static litTagName = m.literal`devtools-trust-tokens-storage-view`;
	#e = this.attachShadow({ mode: "open" });
	#st(e) {
		const t = a.TargetManager.TargetManager.instance().primaryPageTarget();
		t?.storageAgent().invoke_clearTrustTokens({ issuerOrigin: e });
	}
	connectedCallback() {
		(this.wrapper?.contentElement.classList.add("vbox"),
			(this.#e.adoptedStyleSheets = [$t]),
			this.render());
	}
	async render() {
		const e = a.TargetManager.TargetManager.instance().primaryPageTarget();
		if (!e) return;
		const { tokens: t } = await e.storageAgent().invoke_getTrustTokens();
		await It.write("Render TrustTokensView", () => {
			(m.render(
				m.html`
        <div>
      ...
      `,
				...
				this.isConnected && setTimeout(() => this.render(), 1e3));
		});
	}
	#De(e) {
		if (0 === e.length)
			return m.html`<div class="no-tt-message">${Nt(xt.noTrustTokensStored)}</div>`;
		const t = {
			columns: [
				{
					id: "issuer",
			...
			rows: this.#lt(e),
			initialSort: { columnId: "issuer", direction: "ASC" },
		};
		return m.html`
				...
				cells: [
					{
						...
						value: Dt(e.issuerOrigin),
						renderer: this.#ct.bind(this),
					},
					{ columnId: "issuer", value: Dt(e.issuerOrigin) },
					{ columnId: "count", value: e.count },
				],
			}));
	}
	#ct(e) {
...
n.CustomElements.defineComponent("devtools-trust-tokens-storage-view", Pt);
...
export {
	E as BackForwardCacheView,
	U as BounceTrackingMitigationsView,
	X as EndpointsGrid,
	Ve as FrameDetailsView,
	Oe as InterestGroupAccessGrid,
	ue as OriginTrialTreeView,
	je as ProtocolHandlersView,
	tt as ReportsGrid,
	nt as ServiceWorkerRouterView,
	ut as SharedStorageAccessGrid,
	Rt as SharedStorageMetadataView,
	Ne as StackTrace,
	vt as StorageMetadataView,
	Mt as TrustTokensView,
};
