...
const r = {
	...
	n = i.i18n.registerUIStrings(
		"panels/browser_debugger/browser_debugger-meta.ts",
		r
	),
	o = i.i18n.getLazilyComputedLocalizedString.bind(void 0, n);
let a, s;
async function p() {
	return (a || (a = await import("./browser_debugger.js")), a);
}
async function c() {
	return (s || (s = await import("../sources/sources.js")), s);
}
(t.ViewManager.registerViewExtension({
	...
	t.ViewManager.registerViewExtension({
		...
		persistence: "permanent",
		loadView: async () =>
			(await c()).SourcesNavigator.NetworkNavigatorView.instance(),
	}),
	t.ViewManager.registerViewExtension({
		location: "navigator-view",
		...
		persistence: "permanent",
		loadView: async () =>
			(await c()).SourcesNavigator.OverridesNavigatorView.instance(),
	}),
	t.ViewManager.registerViewExtension({
		location: "navigator-view",
		...
		persistence: "permanent",
		loadView: async () =>
			new (await c()).SourcesNavigator.ContentScriptsNavigatorView(),
	}),
	t.ContextMenu.registerProvider({
		contextTypes: () => [e.DOMModel.DOMNode],
		loadProvider: async () =>
			new (await p()).DOMBreakpointsSidebarPane.ContextMenuProvider(),
		experiment: void 0,
	}),
