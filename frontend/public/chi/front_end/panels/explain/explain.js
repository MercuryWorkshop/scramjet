...
import * as n from "../../third_party/marked/marked.js";
import * as o from "../../ui/components/buttons/buttons.js";
import * as s from "../../ui/components/helpers/helpers.js";
import * as i from "../../ui/components/icon_button/icon_button.js";
import * as r from "../../ui/components/markdown_view/markdown_view.js";
...
import * as u from "../../models/formatter/formatter.js";
import * as p from "../../models/logs/logs.js";
import * as m from "../../ui/legacy/components/utils/utils.js";
import * as g from "../console/console.js";
const f = 1e3;
var v;
!(function (e) {
	((e.MESSAGE = "message"),
		...
		(e.RELATED_CODE = "relatedCode"),
		(e.SEARCH_ANSWERS = "searchAnswers"));
})(v || (v = {}));
class w {
	#e;
	#t;
	...
	async getSearchAnswers() {
		...
		const e = l.Runtime.Runtime.queryParam("aidaApiKey");
		if (!e) return "";
		const t = this.#e.consoleMessage().messageText,
			n = await fetch(
				`https://customsearch.googleapis.com/customsearch/v1?cx=f499de4cd70e644b1&key=${e}&q="${encodeURIComponent(t)}"`
			),
			o = await n.json(),
			s = [];
		for (const e of o.items || [])
			if (e.pagemap?.question?.length && e.pagemap?.answer) {
				for (let t = 0; t < Math.min(e.pagemap?.answer?.length, 3); ++t)
					s.push("  * " + e.pagemap?.answer[t].text);
				if (s.length >= 4) break;
			}
		return ((this.#t = s.join("\n")), this.#t);
	}
	async getNetworkRequest() {
		const e = this.#e.consoleMessage().getAffectedResources()?.requestId;
		if (!e) return;
		return p.NetworkLog.NetworkLog.instance().requestsForId(e)[0];
	}
	async getMessageSourceCode() {
		const e = this.#e.consoleMessage().stackTrace?.callFrames[0],
			t = this.#e.consoleMessage().runtimeModel(),
			n = t?.debuggerModel();
		if (!n || !t || !e) return { text: "", columnNumber: 0, lineNumber: 0 };
		const o = new d.DebuggerModel.Location(
				n,
				e.scriptId,
				e.lineNumber,
				e.columnNumber
			...
			s =
				await h.DebuggerWorkspaceBinding.DebuggerWorkspaceBinding.instance().rawLocationToUILocation(
					o
				),
			i = await s?.uiSourceCode.requestContent(),
			r = !i?.isEncoded && i?.content ? i.content : "",
			a = r.indexOf("\n");
		if (r.length > f && (a < 0 || a > f)) {
			const { formattedContent: e, formattedMapping: t } =
					await u.ScriptFormatter.formatScriptContent(
						s?.uiSourceCode.mimeType() ?? "text/javascript",
						r
					),
				[n, o] = t.originalToFormatted(
					s?.lineNumber ?? 0,
					s?.columnNumber ?? 0
				);
			return { text: e, columnNumber: o, lineNumber: n };
		}
		return {
			text: r,
	...
	async buildPrompt(e = Object.values(v)) {
		const [t, n, o] = await Promise.all([
				e.includes(v.RELATED_CODE) ? this.getMessageSourceCode() : void 0,
				e.includes(v.NETWORK_REQUEST) ? this.getNetworkRequest() : void 0,
				e.includes(v.SEARCH_ANSWERS) ? this.getSearchAnswers() : "",
			]),
			s = t?.text ? b(t) : "",
			i = n ? R(n) : "",
			r = e.includes(v.STACKTRACE) ? E(this.#e) : "",
			a = k(this.#e),
			c = this.formatPrompt({
				message: [a, r].join("\n").trim(),
				relatedCode: s,
			...
			l = [{ type: v.MESSAGE, value: a }];
		return (
			r && l.push({ type: v.STACKTRACE, value: r }),
			s && l.push({ type: v.RELATED_CODE, value: s }),
			i && l.push({ type: v.NETWORK_REQUEST, value: i }),
			o && l.push({ type: v.SEARCH_ANSWERS, value: o }),
			{ prompt: c, sources: l }
		);
	}
	formatPrompt({
		...
		const s = (e) => {
			const s = [];
			return (
				s.push("### Console message:", e.message),
				t &&
					s.push(
				...
				n &&
					e.relatedRequest &&
					s.push("### Related network request:", e.relatedRequest),
				o && s.push("### Suggestions:", e.searchAnswers),
				s.push("### Summary:", e.explanation),
				s.join("\n")
			);
		};
		return `\nYou are an expert software engineer looking at a console message in DevTools.\n\nYou will follow these rules strictly:\n- Answer the question as truthfully as possible using the provided context\n- if you don't have the answer, say "I don't know" and suggest looking for this information\n  elsewhere\n- Start with the explanation immediately without repeating the given console message.\n- Always wrap code with three backticks (\`\`\`)\n\n${[
			...
			.join(
				"\n\n"
			)}\n\n${s({ message: e, relatedCode: t, relatedRequest: n, searchAnswers: o, explanation: "" })}`;
	}
}
function y(e) {
	const t = e.name.toLowerCase().trim();
	return (
		!t.startsWith("x-") &&
		"cookie" !== t &&
		"set-cookie" !== t &&
...
function b({ text: e, columnNumber: t, lineNumber: n }, o = 1e3) {
	const s = e.split("\n");
	if (s[n].length >= o / 2) {
		const e = Math.max(t - o / 2, 0),
			i = Math.min(t + o / 2, s[n].length);
		return s[n].substring(e, i);
	}
	let i = 0,
		r = n,
		a = x(s[n]);
	const c = new Map();
	for (; void 0 !== s[r] && i + s[r].length <= o / 2; ) {
		const e = x(s[r]);
		(null === e ||
			null === a ||
			(e !== a && e.startsWith(a)) ||
			(/^\s*[\}\)\]]/.exec(s[r]) || c.set(e, r), (a = e)),
			(i += s[r].length + 1),
			r--);
	}
	r = n + 1;
	let l = n,
		d = n;
	for (a = x(s[n]); void 0 !== s[r] && i + s[r].length <= o; ) {
		i += s[r].length;
		const e = x(s[r]);
		if (null !== e && null !== a && (e === a || !e.startsWith(a))) {
			const t = s[r + 1],
				n = t ? x(t) : null;
			((n && n !== e && n.startsWith(e)) ||
				(c.has(e) && ((l = c.get(e) ?? 0), (d = r))),
				(a = e));
		}
		r++;
	}
	return s.slice(l, d + 1).join("\n");
}
function R(e) {
	return `Request: ${e.url()}\n\nRequest headers:\n${e
		...
		.filter(y)
		.map((e) => `${e.name}: ${e.value}`)
		.join("\n")}\n\nResponse status: ${e.statusCode} ${e.statusText}`;
}
function k(e) {
	return e.toMessageTextString();
}
function E(e) {
	const t = e.contentElement().querySelector(".stack-preview-container");
	if (!t) return "";
	const n = t.shadowRoot?.querySelector(".stack-preview-container");
	return n
		.childTextNodes()
		.filter(
			(e) =>
				!e.parentElement?.closest(".show-all-link,.show-less-link,.hidden-row")
		)
		.map(m.Linkifier.Linkifier.untruncatedNodeText)
		.join("")
		.trim();
}
...
const I = {
	...
	N = t.i18n.registerUIStrings(
		"panels/explain/components/ConsoleInsight.ts",
		I
	),
	$ = t.i18n.getLocalizedString.bind(void 0, N),
	C = t.i18n.getLazilyComputedLocalizedString.bind(void 0, N),
	{ render: M, html: A, Directives: j } = c;
class L extends Event {
	static eventName = "close";
	constructor() {
		super(L.eventName, { composed: !0, bubbles: !0 });
	}
}
const B = [
	["inaccurate", C(I.inaccurate)],
	["irrelevant", C(I.irrelevant)],
	["inapproprate", C(I.inappropriate)],
	["not-helpful", C(I.notHelpful)],
	["other", C(I.other)],
];
function O(e) {
	switch (e) {
		case v.MESSAGE:
			return $(I.consoleMessage);
		case v.STACKTRACE:
			return $(I.stackTrace);
		case v.NETWORK_REQUEST:
			return $(I.networkRequest);
		case v.RELATED_CODE:
			return $(I.relatedCode);
		case v.SEARCH_ANSWERS:
			return $(I.searchAnswers);
	}
}
var F;
!(function (e) {
...
class H extends HTMLElement {
	static litTagName = c.literal`devtools-console-insight`;
	...
	constructor(e, t) {
		(super(),
			...
			(this.#p = D++),
			this.focus(),
			(this.#u = new a.PopoverHelper.PopoverHelper(this, this.#f.bind(this))),
			this.#u.setTimeout(300),
			this.#u.setHasPadding(!0),
	...
	#f(e) {
		...
		if (!t || !t.isSelfOrDescendant(this.#n.querySelector(".info")))
			return null;
		const n = this.#m || "mousemove" !== e.type;
		return {
			box: t.boxInWindow(),
			show: async (e) => {
				const { sources: t } = await this.#i.buildPrompt(),
					o = `dialog-${this.#p}`,
				...
				((s.style.display = "flex"),
					...
					n &&
						s.addEventListener("keydown", (t) => {
							...
							if ("Tab" === n.key) {
								const n = (function (e) {
										const t = e.contentElement.getComponentRoot();
										if (!(t instanceof ShadowRoot))
											throw new Error("Expected a shadow root");
										const n = ["x-link", "[role=document]"],
											o = [];
										function s(e) {
									...
									o = (function (e) {
										...
										for (; n && n.shadowRoot?.activeElement; )
								...
								if (o) {
									const e = n.indexOf(o);
									t.shiftKey && 0 === e
										? (n.at(-1)?.focus(), t.preventDefault())
										: t.shiftKey ||
											e !== n.length - 1 ||
											(n.at(0)?.focus(), t.preventDefault());
								}
							} else
								"Escape" === n.key &&
									(t.consume(!0),
									this.#u.hidePopover(),
									this.#n.querySelector(".info")?.focus());
						}));
				const i = document.createElement("div");
				((i.role = "document"),
					(i.tabIndex = 0),
					i.setAttribute("aria-describedby", o),
					s.append(i));
				const r = document.createElement("p");
				((r.id = o),
					(r.innerText = $(I.refineButtonHint)),
					(r.style.margin = "0"),
					i.append(r));
				...
				if (
					...
					e.show = (n) => {
						(t.call(e, n),
							e.contentElement.querySelector("[role=document]")?.focus());
					};
					const n = e.hide;
					e.hide = () => {
						(n.call(e), this.#n.querySelector(".info")?.focus());
					};
				}
				return !0;
			},
	...
	connectedCallback() {
		((this.#n.adoptedStyleSheets = [S]), this.classList.add("opening"));
	}
	disonnectedCallback() {
		this.#u.dispose();
	}
	...
	#v(e) {
		const t = this.#c;
		((this.#c = e),
			e.type !== t.type && t.type === F.LOADING && this.classList.add("loaded"),
			this.#g());
	}
	async update(t = this.#s) {
		this.#v(
			this.#c.type === F.INSIGHT
				? { ...this.#c, type: F.REFINING }
				: { type: F.LOADING }
		);
		try {
			const e = t ? void 0 : [v.MESSAGE],
				{ prompt: o, sources: s } = await this.#i.buildPrompt(e),
				i = await this.#r.getInsights(o);
			this.#v({
				type: F.INSIGHT,
				tokens: n.Marked.lexer(i),
				explanation: i,
				sources: s,
				refined: t,
			});
		} catch (t) {
			(e.userMetrics.actionTaken(e.UserMetrics.Action.InsightErrored),
				this.#v({ type: F.ERROR, error: t.message }));
		}
	}
	#w() {
		(this.dispatchEvent(new L()), this.classList.add("closing"));
	}
	#y() {
		((this.#l = !1), (this.#d = void 0), this.#h.clear(), this.#g());
	}
	#x() {
		(this.#o && this.#b(), this.#y());
	}
	#b() {
		if (this.#c.type !== F.INSIGHT) throw new Error("Unexpected state");
		const t = (function (e, t, n, o, s, i, r) {
			const a =
				"Negative" === e
					? {
							"entry.1465663861": e,
			...
			return `http://go/console-insights-experiment-rating?usp=pp_url&${Object.keys(
				a
			)
				.map((e) => `${e}=${encodeURIComponent(a[e])}`)
				.join("&")}`;
		})(
			this.#d ? "Positive" : "Negative",
			this.#n.querySelector("textarea")?.value,
			this.#c.explanation,
			this.#c.sources
				.filter((e) => e.type === v.MESSAGE)
				.map((e) => e.value)
				.join("\n"),
			this.#c.sources
				.filter((e) => e.type === v.STACKTRACE)
				.map((e) => e.value)
				.join("\n"),
			this.#c.sources
				.filter((e) => e.type === v.RELATED_CODE)
				.map((e) => e.value)
				.join("\n"),
			this.#c.sources
				.filter((e) => e.type === v.NETWORK_REQUEST)
				.map((e) => e.value)
				.join("\n")
		);
		e.InspectorFrontendHost.InspectorFrontendHostInstance.openInNewTab(t);
	}
	#R(t) {
		((this.#d = "true" === t.target.dataset.rating),
			this.#d
				? e.userMetrics.actionTaken(e.UserMetrics.Action.InsightRatedPositive)
				: e.userMetrics.actionTaken(e.UserMetrics.Action.InsightRatedNegative),
			this.#o ? this.#b() : ((this.#l = !0), this.#g()));
	}
	#k(e) {
		const t = e.target;
		(t.active
			? this.#h.delete(t.dataset.reason)
			: this.#h.add(t.dataset.reason),
			this.#g());
	}
	#E() {
		if (this.#c.type !== F.INSIGHT) throw new Error("Unexpected state");
		(e.userMetrics.actionTaken(e.UserMetrics.Action.InsightRefined),
			this.update(!0));
	}
	#S(e) {
		if (e instanceof KeyboardEvent)
			switch (e.key) {
				case "Escape":
					(e.consume(!0), this.#u.hidePopover());
					break;
				case "Enter":
				case " ":
					(e.consume(!0), (this.#m = !0));
					try {
	...
	#g() {
		const e = j.classMap({ wrapper: !0, top: this.#l }),
			t = j.classMap({ wrapper: !0, bottom: this.#l });
		M(
			A`
      <div class=${e}>
        <header>
...
class q extends HTMLElement {
	static litTagName = c.literal`devtools-console-insight-sources-list`;
	#n = this.attachShadow({ mode: "open" });
	#$ = [];
	constructor() {
		(super(), (this.#n.adoptedStyleSheets = [T]));
	}
	#g() {
		M(
			A`
        ...
        ${j.repeat(
					...
					(e) => {
						...
						return (
							...
							A`<li><x-link class="link" title="${O(e.type)} ${$(I.opensInNewTab)}" href=${`data:text/plain,${encodeURIComponent(e.value)}`}>${O(e.type)}${t}</x-link></li>`
						);
					}
				)}
      </ul>
...
(s.CustomElements.defineComponent("devtools-console-insight", H),
	s.CustomElements.defineComponent("devtools-console-insight-sources-list", q));
class P extends r.MarkdownView.MarkdownLitRenderer {
	renderToken(e) {
		const t = this.templateForToken(e);
		return null === t
			? (console.warn(`Markdown token type '${e.type}' not supported.`),
				c.html``)
			: t;
	}
	templateForToken(e) {
		switch (e.type) {
			case "heading":
				return A`<strong>${this.renderText(e)}</strong>`;
			case "link":
			case "image":
				return c.html`${a.XLink.XLink.create(e.href, e.text, void 0, void 0, "token")}`;
		}
		return super.templateForToken(e);
	}
}
class U {
	static buildApiRequest(e) {
		const t = { input: e, client: "CHROME_DEVTOOLS" },
			n = parseFloat(l.Runtime.Runtime.queryParam("aidaTemperature") || "");
		return (isNaN(n) || (t.options = { temperature: n }), t);
	}
	async getInsights(t) {
		return new Promise((n, o) => {
			if (
				...
				return o(new Error("doAidaConversation is not available"));
			(console.time("request"),
				e.InspectorFrontendHost.InspectorFrontendHostInstance.doAidaConversation(
					JSON.stringify(U.buildApiRequest(t)),
					(e) => {
						console.timeEnd("request");
						try {
							const t = JSON.parse(e.response)
								.map((e) => {
									if ("textChunk" in e) return e.textChunk.text;
									if ("codeChunk" in e)
										return "\n`````\n" + e.codeChunk.code + "\n`````\n";
									if ("error" in e) throw new Error(`${e.error}: ${e.detail}`);
	...
	handleAction(t, n) {
		switch (n) {
			...
			case "explain.consoleMessage:hover": {
				const o = t.flavor(g.ConsoleViewMessage.ConsoleViewMessage);
				if (o) {
					n.startsWith("explain.consoleMessage:context")
						? e.userMetrics.actionTaken(
								e.UserMetrics.Action.InsightRequestedViaContextMenu
							)
						: "explain.consoleMessage:hover" === n &&
							e.userMetrics.actionTaken(
								e.UserMetrics.Action.InsightRequestedViaHoverButton
							);
					const t = new H(new w(o), new U());
					return (o.setInsight(t), t.update(), !0);
				}
				return !1;
			}
		}
...
export {
	...
	L as CloseEvent,
	H as ConsoleInsight,
	U as InsightProvider,
	P as MarkdownRenderer,
	w as PromptBuilder,
	v as SourceType,
	y as allowHeader,
	k as formatConsoleMessage,
	R as formatNetworkRequest,
	b as formatRelatedCode,
	E as formatStackTrace,
	x as lineWhitespace,
};
