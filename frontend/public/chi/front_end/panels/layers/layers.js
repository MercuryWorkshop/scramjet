import * as e from "../layer_viewer/layer_viewer.js";
...
class n extends t.SplitWidget.SplitWidget {
	logTreeView;
	paintProfilerView;
	constructor(t) {
		(super(!0, !1),
			(this.logTreeView =
				new e.PaintProfilerView.PaintProfilerCommandLogView()),
			this.setSidebarWidget(this.logTreeView),
			(this.paintProfilerView = new e.PaintProfilerView.PaintProfilerView(t)),
			this.setMainWidget(this.paintProfilerView),
			this.paintProfilerView.addEventListener(
				e.PaintProfilerView.Events.WindowChanged,
...
var s,
	l = Object.freeze({ __proto__: null, LayerPaintProfilerView: n });
class o extends a.SDKModel.SDKModel {
	layerTreeAgent;
	paintProfilerModel;
	layerTreeInternal;
	throttler;
	...
	constructor(e) {
		(super(e),
			...
			e.registerLayerTreeDispatcher(new y(this)),
			(this.paintProfilerModel = e.model(a.PaintProfiler.PaintProfilerModel)));
		const t = e.model(a.ResourceTreeModel.ResourceTreeModel);
		(t &&
			t.addEventListener(
			...
			(this.throttler = new r.Throttler.Throttler(20)));
	}
	async disable() {
		this.enabled &&
			((this.enabled = !1), await this.layerTreeAgent.invoke_disable());
	}
	enable() {
	...
	async forceEnable() {
		((this.lastPaintRectByLayerId = new Map()),
			this.layerTreeInternal || (this.layerTreeInternal = new d(this)),
	...
	async innerSetLayers(e) {
		...
		((this.lastPaintRectByLayerId = new Map()),
			this.dispatchEventToListeners(s.LayerTreeChanged));
	}
	layerPainted(e, t) {
		...
		if (!r)
			return (
				this.lastPaintRectByLayerId ||
					(this.lastPaintRectByLayerId = new Map()),
				void this.lastPaintRectByLayerId.set(e, t)
			);
		(r.didPaint(t), this.dispatchEventToListeners(s.LayerPainted, r));
	}
	onPrimaryPageChanged() {
		((this.layerTreeInternal = null), this.enabled && this.forceEnable());
	}
}
(a.SDKModel.SDKModel.register(o, {
...
class d extends a.LayerTreeBase.LayerTreeBase {
	layerTreeModel;
	constructor(e) {
		(super(e.target()), (this.layerTreeModel = e));
	}
	async setLayers(e) {
		if (!e) return void this.innerSetLayers(e);
		const t = new Set();
		for (let r = 0; r < e.length; ++r) {
			const i = e[r].backendNodeId;
			i && !this.backendNodeIdToNode().has(i) && t.add(i);
		}
		(await this.resolveBackendNodeIds(t), this.innerSetLayers(e));
	}
	innerSetLayers(e) {
		...
		for (let i = 0; i < e.length; ++i) {
			const a = e[i].layerId;
			let n = r.get(a);
			(n ? n.reset(e[i]) : (n = new h(this.layerTreeModel, e[i])),
				this.layersById.set(a, n));
			const s = e[i].backendNodeId;
			(s && n.setNode(this.backendNodeIdToNode().get(s) || null),
				!this.contentRoot() && n.drawsContent() && this.setContentRoot(n));
			const l = n.parentId();
			if (l) {
				const e = this.layersById.get(l);
				if (!e) throw new Error(`Missing parent ${l} for layer ${a}`);
				e.addChild(n);
			} else (t && console.assert(!1, "Multiple root layers"), (t = n));
		}
...
class h {
	...
	nodeForSelfOrAncestor() {
		let e = this;
		for (; e; e = e.parentInternal) if (e.nodeInternal) return e.nodeInternal;
		return null;
	}
	offsetX() {
	...
	async requestCompositingReasons() {
		return (
			(
				await this.layerTreeModel.layerTreeAgent.invoke_compositingReasons({
					layerId: this.id(),
				})
			).compositingReasons || []
		);
	...
	reset(e) {
		((this.nodeInternal = null),
			...
			(this.stickyPositionConstraintInternal = this.layerPayload
				.stickyPositionConstraint
				? new a.LayerTreeBase.StickyPositionConstraint(
	...
	calculateTransformToViewport(e) {
		let r = new WebKitCSSMatrix().translate(
			this.layerPayload.offsetX,
			this.layerPayload.offsetY
		);
		if (this.layerPayload.transform) {
			const e = this.matrixFromArray(this.layerPayload.transform),
				i = new t.Geometry.Vector(
					this.layerPayload.width * this.anchorPoint()[0],
					this.layerPayload.height * this.anchorPoint()[1],
					this.anchorPoint()[2]
				),
				a = t.Geometry.multiplyVectorByMatrixAndNormalize(i, r),
				n = new WebKitCSSMatrix().translate(-a.x, -a.y, -a.z);
			r = n.inverse().multiply(e.multiply(n.multiply(r)));
		}
		return ((r = e.multiply(r)), r);
	}
	createVertexArrayForRect(e, t) {
		return [0, 0, 0, e, 0, 0, e, t, 0, 0, t, 0];
	}
	calculateQuad(e) {
		const r = this.calculateTransformToViewport(e);
		this.quadInternal = [];
		const i = this.createVertexArrayForRect(
			this.layerPayload.width,
			this.layerPayload.height
		);
		for (let e = 0; e < 4; ++e) {
			const a = t.Geometry.multiplyVectorByMatrixAndNormalize(
				new t.Geometry.Vector(i[3 * e], i[3 * e + 1], i[3 * e + 2]),
				r
			);
...
const u = { details: "Details", profiler: "Profiler" },
	P = i.i18n.registerUIStrings("panels/layers/LayersPanel.ts", u),
	p = i.i18n.getLocalizedString.bind(void 0, P);
let w;
class g extends t.Panel.PanelWithSidebar {
	model;
	layerViewHost;
	layerTreeOutline;
	rightSplitWidget;
	...
	constructor() {
		(super("layers", 225),
			...
			(this.layerViewHost = new e.LayerViewHost.LayerViewHost()),
			(this.layerTreeOutline = new e.LayerTreeOutline.LayerTreeOutline(
				this.layerViewHost
			)),
			...
			(this.rightSplitWidget = new t.SplitWidget.SplitWidget(
				!1,
				!0,
				"layerDetailsSplitViewState"
			)),
			this.splitWidget().setMainWidget(this.rightSplitWidget),
			(this.layers3DView = new e.Layers3DView.Layers3DView(this.layerViewHost)),
			...
			this.layers3DView.addEventListener(
				e.Layers3DView.Events.ScaleChanged,
				this.onScaleChanged,
				this
			),
			(this.tabbedPane = new t.TabbedPane.TabbedPane()),
			this.rightSplitWidget.setSidebarWidget(this.tabbedPane),
			(this.layerDetailsView = new e.LayerDetailsView.LayerDetailsView(
				this.layerViewHost
			)),
			this.layerDetailsView.addEventListener(
				e.LayerDetailsView.Events.PaintProfilerRequested,
				this.onPaintProfileRequested,
				this
			),
			this.tabbedPane.appendTab(f.Details, p(u.details), this.layerDetailsView),
			...
			(this.updateThrottler = new r.Throttler.Throttler(100)));
	}
	static instance(e) {
		return ((w && !e?.forceNew) || (w = new g()), w);
	}
	focus() {
	...
	targetAdded(e) {
		e === e.outermostTarget() &&
			((this.model = e.model(o)),
			this.model &&
				(this.model.addEventListener(
					s.LayerTreeChanged,
					this.onLayerTreeUpdated,
					this
				),
				this.model.addEventListener(s.LayerPainted, this.onLayerPainted, this),
	...
	targetRemoved(e) {
		this.model &&
			...
			this.model.removeEventListener(s.LayerPainted, this.onLayerPainted, this),
			this.model.disable(),
			(this.model = null));
	}
	onLayerTreeUpdated() {
	...
	onPaintProfileRequested({ data: e }) {
		this.layers3DView.snapshotForSelection(e).then((t) => {
			t &&
				...
				this.tabbedPane.hasTab(f.Profiler) ||
				...
				this.tabbedPane.selectTab(f.Profiler),
				this.paintProfilerView.profile(t.snapshot));
		});
	}
	onTabClosed(e) {
		e.data.tabId === f.Profiler &&
	...
	showImage(e) {
		this.layerBeingProfiled &&
			this.layers3DView.showImageForLayer(this.layerBeingProfiled, e);
	}
	onScaleChanged(e) {
		this.paintProfilerView.setScale(e.data);
	}
}
const f = { Details: "details", Profiler: "profiler" };
var T = Object.freeze({ __proto__: null, LayersPanel: g, DetailsViewTabs: f });
export { l as LayerPaintProfilerView, c as LayerTreeModel, T as LayersPanel };
