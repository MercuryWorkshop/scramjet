...
import * as a from "../../../ui/components/input/input.js";
...
class m extends Event {
	...
	data;
	constructor(e) {
		(super(m.eventName), (this.data = e));
	}
}
class y extends HTMLElement {
	static litTagName = r.literal`devtools-linear-memory-highlight-chip-list`;
	#e = this.attachShadow({ mode: "open" });
	#t = [];
	#o;
	connectedCallback() {
		this.#e.adoptedStyleSheets = [l];
	}
	set data(e) {
		((this.#t = e.highlightInfos),
			(this.#o = e.focusedMemoryHighlight),
			this.#r());
	}
	#r() {
		const e = [];
		for (const t of this.#t) e.push(this.#s(t));
		const t = p`
            <div class="highlight-chip-list">
              ${e}
            </div>
        `;
		g(t, this.#e, { host: this });
	}
	#s(e) {
		const t = e.name || "<anonymous>",
			n = e.type,
			i = { focused: e === this.#o, "highlight-chip": !0 };
		return p`
      <div class=${r.Directives.classMap(i)}>
        <button class="jump-to-highlight-button" title=${h(c.jumpToAddress)}
            jslog=${s.action().track({ click: !0 }).context("linear-memory-inspector.jump-to-highlight")}
            @click=${() => this.#n(e.startAddress)}>
          <span class="source-code">
            <span class="value">${t}</span><span class="separator">: </span><span>${n}</span>
          </span>
        </button>
        <div class="delete-highlight-container">
          <button class="delete-highlight-button" title=${h(c.deleteHighlight)}
              jslog=${s.action().track({ click: !0 }).context("linear-memory-inspector.delete-highlight")}
              @click=${() => this.#i(e)}>
            <${o.Icon.Icon.litTagName} .data=${{ iconName: "cross", color: "var(--icon-default-hover)", width: "16px" }}>
...
t.CustomElements.defineComponent(
	"devtools-linear-memory-highlight-chip-list",
	y
);
var v = Object.freeze({
	__proto__: null,
	DeleteMemoryHighlightEvent: u,
	JumpToHighlightedMemoryEvent: m,
	LinearMemoryHighlightChipList: y,
});
...
const w = {
	...
	x = e.i18n.registerUIStrings(
		"panels/linear_memory_inspector/components/LinearMemoryNavigator.ts",
		w
	),
	k = e.i18n.getLocalizedString.bind(void 0, x),
	{ render: S, html: $ } = r;
class T extends Event {
	static eventName = "addressinputchanged";
	data;
	constructor(e, t) {
		(super(T.eventName), (this.data = { address: e, mode: t }));
	}
}
...
class N extends HTMLElement {
	static litTagName = r.literal`devtools-linear-memory-inspector-navigator`;
	#e = this.attachShadow({ mode: "open" });
	#a = "0";
	#l = void 0;
	#c = !0;
	...
	#h = !1;
	connectedCallback() {
		this.#e.adoptedStyleSheets = [b];
	}
	set data(e) {
		((this.#a = e.address),
			(this.#l = e.error),
			(this.#c = e.valid),
			(this.#d = e.canGoBackInHistory),
			(this.#h = e.canGoForwardInHistory),
			this.#r());
		const t = this.#e.querySelector(".address-input");
		t &&
			("Submitted" === e.mode
				? t.blur()
				: "InvalidSubmit" === e.mode && t.select());
	}
	#r() {
		const e = $`
      <div class="navigator">
        <div class="navigator-item">
          ${this.#g({ icon: "undo", title: k(w.goBackInAddressHistory), event: new M("Backward"), enabled: this.#d, jslogContext: "linear-memory-inspector.history-back" })}
          ${this.#g({ icon: "redo", title: k(w.goForwardInAddressHistory), event: new M("Forward"), enabled: this.#h, jslogContext: "linear-memory-inspector.history-forward" })}
        </div>
        <div class="navigator-item">
          ${this.#g({ icon: "chevron-left", title: k(w.previousPage), event: new I("Backward"), enabled: !0, jslogContext: "linear-memory-inspector.previous-page" })}
          ${this.#p()}
          ${this.#g({ icon: "chevron-right", title: k(w.nextPage), event: new I("Forward"), enabled: !0, jslogContext: "linear-memory-inspector.next-page" })}
        </div>
        ${this.#g({ icon: "refresh", title: k(w.refresh), event: new E(), enabled: !0, jslogContext: "linear-memory-inspector.refresh" })}
      </div>
      `;
		S(e, this.#e, { host: this });
	}
	#p() {
		const e = { "address-input": !0, invalid: !this.#c };
		return $`
      <input class=${r.Directives.classMap(e)} data-input="true" .value=${this.#a}
        jslog=${s.textField().track({ keydown: !0 }).context("linear-memory-inspector.address")}
        title=${this.#c ? k(w.enterAddress) : this.#l} @change=${this.#u.bind(this, "Submitted")} @input=${this.#u.bind(this, "Edit")}/>`;
	}
	#u(e, t) {
		const o = t.target;
		this.dispatchEvent(new T(o.value, e));
	}
	#g(e) {
		return $`
      <button class="navigator-button" ?disabled=${!e.enabled}
        jslog=${s.action().track({ click: !0, keydown: "Enter" }).context(e.jslogContext)}
        data-button=${e.event.type} title=${e.title}
        @click=${this.dispatchEvent.bind(this, e.event)}>
        <${o.Icon.Icon.litTagName} name=${e.icon}></${o.Icon.Icon.litTagName}>
      </button>`;
	}
}
t.CustomElements.defineComponent(
...
var A = Object.freeze({
	__proto__: null,
	AddressInputChangedEvent: T,
	PageNavigationEvent: I,
	HistoryNavigationEvent: M,
	RefreshRequestedEvent: E,
	LinearMemoryNavigator: N,
});
...
function V(e, t) {
	switch (e) {
		...
		case "Pointer 64-bit":
			return "hex" === t;
		default:
			return i.assertNever(e, `Unknown value type: ${e}`);
	}
}
function U(e) {
	switch (e) {
...
function F(e, t, o) {
	if (!D(e))
		return (
			console.error(`Requesting address of a non-pointer type: ${e}.\n`),
			NaN
		);
	try {
		const r = new DataView(t),
			s = "Little Endian" === o;
		return "Pointer 32-bit" === e ? r.getUint32(0, s) : r.getBigUint64(0, s);
	} catch (e) {
		return NaN;
	}
}
function D(e) {
	return "Pointer 32-bit" === e || "Pointer 64-bit" === e;
}
function q(e) {
	if (!e.mode)
		return (
			console.error(`No known way of showing value for ${e.type}`),
			H(R.notApplicable)
		);
	const t = new DataView(e.buffer),
		o = "Little Endian" === e.endianness;
	let r;
	try {
		switch (e.type) {
			case "Integer 8-bit":
				return ((r = e.signed ? t.getInt8(0) : t.getUint8(0)), J(r, e.mode));
			case "Integer 16-bit":
				return (
					(r = e.signed ? t.getInt16(0, o) : t.getUint16(0, o)),
					J(r, e.mode)
				);
			case "Integer 32-bit":
				return (
					(r = e.signed ? t.getInt32(0, o) : t.getUint32(0, o)),
					J(r, e.mode)
				);
			case "Integer 64-bit":
				return (
					(r = e.signed ? t.getBigInt64(0, o) : t.getBigUint64(0, o)),
					J(r, e.mode)
				);
			case "Float 32-bit":
				return ((r = t.getFloat32(0, o)), G(r, e.mode));
			case "Float 64-bit":
				return ((r = t.getFloat64(0, o)), G(r, e.mode));
			case "Pointer 32-bit":
				return ((r = t.getUint32(0, o)), J(r, "hex"));
			case "Pointer 64-bit":
				return ((r = t.getBigUint64(0, o)), J(r, "hex"));
			default:
				return i.assertNever(e.type, `Unknown value type: ${e.type}`);
		}
	} catch (e) {
		return H(R.notApplicable);
	}
}
function G(e, t) {
	switch (t) {
		...
		case "sci":
			return e.toExponential(2).toString();
		default:
			throw new Error(`Unknown mode for floats: ${t}.`);
	}
}
function J(e, t) {
	switch (t) {
		case "dec":
			return e.toString();
		case "hex":
			return e < 0 ? H(R.notApplicable) : "0x" + e.toString(16).toUpperCase();
		case "oct":
			return e < 0 ? H(R.notApplicable) : e.toString(8);
		default:
			throw new Error(`Unknown mode for integers: ${t}.`);
	}
}
var K = Object.freeze({
	__proto__: null,
	VALUE_INTEPRETER_MAX_NUM_BYTES: 8,
	getDefaultValueTypeMapping: B,
	VALUE_TYPE_MODE_LIST: O,
	valueTypeToLocalizedString: P,
	isValidMode: V,
	isNumber: U,
	getPointerAddress: F,
	isPointer: D,
...
const X = {
	...
	W = e.i18n.registerUIStrings(
		"panels/linear_memory_inspector/components/ValueInterpreterDisplay.ts",
		X
	),
	Y = e.i18n.getLocalizedString.bind(void 0, W),
	{ render: Q, html: Z } = r,
	ee = Array.from(B().keys());
class te extends Event {
	static eventName = "valuetypemodechanged";
	data;
	constructor(e, t) {
		(super(te.eventName, { composed: !0 }), (this.data = { type: e, mode: t }));
	}
}
class oe extends Event {
	...
	data;
	constructor(e) {
		(super(oe.eventName, { composed: !0 }), (this.data = e));
	}
}
class re extends HTMLElement {
	static litTagName = r.literal`devtools-linear-memory-inspector-interpreter-display`;
	#e = this.attachShadow({ mode: "open" });
	#m = "Little Endian";
	#y = new ArrayBuffer(0);
	#v = new Set();
	...
	#b = 0;
	constructor() {
		(super(), (this.#e.adoptedStyleSheets = [z]));
	}
	connectedCallback() {
		this.#e.adoptedStyleSheets = [L];
	}
	set data(e) {
		((this.#y = e.buffer),
			(this.#m = e.endianness),
			(this.#v = e.valueTypes),
			(this.#b = e.memoryLength),
			e.valueTypeModes &&
				e.valueTypeModes.forEach((e, t) => {
					V(t, e) && this.#f.set(t, e);
				}),
			this.#r());
	}
	#r() {
      ...
      <div class="value-types">
        ${ee.map((e) => (this.#v.has(e) ? this.#w(e) : ""))}
      </div>
    `,
			this.#e,
			{ host: this }
	...
	#w(e) {
		if (U(e)) return this.#x(e);
		if (D(e)) return this.#k(e);
		throw new Error(`No known way to format ${e}`);
	}
	#k(t) {
		const r = this.#S({ type: t, signed: !1 }),
			n = F(t, this.#y, this.#m),
			i = Number.isNaN(n) || BigInt(n) >= BigInt(this.#b),
			a = Y(i ? X.addressOutOfRange : X.jumpToPointer),
			l = i ? "var(--icon-default)" : "var(--icon-link)";
		return Z`
      ...
      <div class="value-type-cell">
        ...
        <span class="selectable-text">${r}</span>
          ${Z`
              <button class="jump-to-button" data-jump="true" title=${a} ?disabled=${i}
                jslog=${s.action().track({ click: !0 }).context("linear-memory-inspector.jump-to-address")}
                @click=${this.#$.bind(this, Number(n))}>
                <${o.Icon.Icon.litTagName} .data=${{ iconName: "open-externally", color: l, width: "16px" }}>
                </${o.Icon.Icon.litTagName}>
              </button>`}
        </div>
      </div>
	...
	#I(e) {
		const t = this.#S({ type: e, signed: !1 }),
			o = this.#S({ type: e, signed: !0 }),
			r = this.#f.get(e),
			s = o !== t && "hex" !== r && "oct" !== r,
			n = Z`<span class="value-type-cell selectable-text"  title=${Y(X.unsignedValue)} data-value="true">${t}</span>`;
		if (!s) return n;
		const i = "Integer 32-bit" === e || "Integer 64-bit" === e,
			a = Z`<span class="selectable-text" data-value="true" title=${Y(X.signedValue)}>${o}</span>`;
		return i
			? Z`
        <div class="value-type-cell">
          ${n}
	...
	#T(e, t) {
		...
		this.dispatchEvent(new te(e, o));
	}
	#S(e) {
		const t = this.#f.get(e.type);
		return q({
			buffer: this.#y,
			type: e.type,
			endianness: this.#m,
...
var se = Object.freeze({
	__proto__: null,
	ValueTypeModeChangedEvent: te,
	JumpToPointerAddressEvent: oe,
	ValueInterpreterDisplay: re,
});
...
const { render: ie, html: ae } = r,
	le = { otherGroup: "Other" },
	ce = e.i18n.registerUIStrings(
		"panels/linear_memory_inspector/components/ValueInterpreterSettings.ts",
		le
	),
	de = e.i18n.getLocalizedString.bind(void 0, ce),
	he = new Map([
		[
			"Integer",
			["Integer 8-bit", "Integer 16-bit", "Integer 32-bit", "Integer 64-bit"],
		],
...
class ge extends Event {
	...
	data;
	constructor(e, t) {
		(super(ge.eventName), (this.data = { type: e, checked: t }));
	}
}
class pe extends HTMLElement {
	static litTagName = r.literal`devtools-linear-memory-inspector-interpreter-settings`;
	#e = this.attachShadow({ mode: "open" });
	#v = new Set();
	connectedCallback() {
		this.#e.adoptedStyleSheets = [a.checkboxStyles, ne];
	}
	set data(e) {
		((this.#v = e.valueTypes), this.#r());
	}
	#r() {
		ie(
			ae`
      <div class="settings" jslog=${s.section().context("settings")}>
       ${[...he.keys()].map(
					(e) => ae`
          <div class="value-types-selection">
            <span class="group">${(function (e) {
							return "Other" === e ? de(le.otherGroup) : e;
						})(e)}</span>
            ${this.#M(e)}
          </div>
        `
	...
	#M(e) {
		const t = he.get(e);
		if (!t) throw new Error(`Unknown group ${e}`);
		return ae`
      ${t.map(
				(e) => ae`
          <label class="type-label" title=${P(e)}>
            <input data-input="true" type="checkbox" .checked=${this.#v.has(e)} @change=${(t) => this.#E(e, t)} jslog=${s.toggle().track({ change: !0 }).context(e)}>
            <span data-title="true">${P(e)}</span>
          </label>
     `
			)}`;
	}
	#E(e, t) {
		const o = t.target;
		this.dispatchEvent(new ge(e, o.checked));
	}
}
t.CustomElements.defineComponent(
...
class xe extends Event {
	...
	data;
	constructor(e, t) {
		(super(xe.eventName), (this.data = { type: e, checked: t }));
	}
}
class ke extends HTMLElement {
	static litTagName = r.literal`devtools-linear-memory-inspector-interpreter`;
	#e = this.attachShadow({ mode: "open" });
	#m = "Little Endian";
	#y = new ArrayBuffer(0);
	#v = new Set();
	...
	set data(e) {
		((this.#m = e.endianness),
			(this.#y = e.value),
			(this.#v = e.valueTypes),
			(this.#f = e.valueTypeModes || new Map()),
			(this.#b = e.memoryLength),
			this.#r());
	}
	#r() {
      ...
      <div class="value-interpreter">
        <div class="settings-toolbar">
          ...
          <button data-settings="true" class="settings-toolbar-button ${this.#N ? "active" : ""}"
              ...
              jslog=${s.action().track({ click: !0 }).context("linear-memory-inspector.toggle-value-settings")}>
	...
	#z(e) {
		...
		this.dispatchEvent(new we(t));
	}
	#A() {
		const t = this.#z.bind(this);
		return be`
    <label data-endianness-setting="true" title=${ve(me.changeEndianness)}>
      <select class="chrome-select"
        jslog=${s.toggle().track({ change: !0 }).context("linear-memory-inspector.endianess")}
	...
	#E(e) {
		this.dispatchEvent(new xe(e.data.type, e.data.checked));
	}
}
t.CustomElements.defineComponent(
...
var Se = Object.freeze({
	__proto__: null,
	EndiannessChangedEvent: we,
	ValueTypeToggledEvent: xe,
	LinearMemoryValueInterpreter: ke,
});
const $e = /^0x[a-fA-F0-9]+$/,
	Te = /^0$|[1-9]\d*$/;
function Ie(e) {
	const t = e.number.toString(16).padStart(e.pad, "0").toUpperCase();
	return e.prefix ? "0x" + t : t;
}
function Me(e) {
	return Ie({ number: e, pad: 8, prefix: !0 });
}
function Ee(e) {
	const t = e.match($e),
		o = e.match(Te);
	let r;
	return (
		t && t[0].length === e.length
			? (r = parseInt(e, 16))
			: o && o[0].length === e.length && (r = parseInt(e, 10)),
		r
	);
}
var Ne = Object.freeze({
	__proto__: null,
	HEXADECIMAL_REGEXP: $e,
	DECIMAL_REGEXP: Te,
	toHexString: Ie,
	formatAddress: Me,
	parseAddress: Ee,
...
const { render: Ce, html: ze } = r;
class Le extends Event {
	static eventName = "byteselected";
	data;
	constructor(e) {
		(super(Le.eventName), (this.data = e));
	}
}
class Re extends Event {
	...
	data;
	constructor(e) {
		(super(Re.eventName), (this.data = e));
	}
}
class je extends HTMLElement {
	static litTagName = r.literal`devtools-linear-memory-inspector-viewer`;
	#e = this.attachShadow({ mode: "open" });
	#L = new ResizeObserver(() => this.#R());
	#j = !1;
	#H = new Uint8Array();
	#a = 0;
	#B = 0;
	...
	set data(e) {
		...
		((this.#H = e.memory),
			(this.#a = e.address),
			(this.#_ = e.highlightInfo),
			(this.#o = e.focusedMemoryHighlight),
			(this.#B = e.memoryOffset),
			(this.#V = e.focus),
			this.#F());
	}
	connectedCallback() {
		(t.SetCSSProperty.set(this, "--byte-group-margin", "8px"),
			(this.#e.adoptedStyleSheets = [Ae]));
	}
	disconnectedCallback() {
		((this.#j = !1), this.#L.disconnect());
	}
	#F() {
		(this.#D(), this.#r(), this.#q(), this.#G());
	}
	#q() {
		if (this.#V) {
			const e = this.#e.querySelector(".view");
			e && e.focus();
		}
	}
	#R() {
		(this.#F(), this.dispatchEvent(new Re(this.#P * this.#O)));
	}
	#D() {
		if (0 === this.clientWidth || 0 === this.clientHeight || !this.shadowRoot)
			return ((this.#P = 4), void (this.#O = 1));
		const e = this.shadowRoot.querySelector(".byte-cell"),
			t = this.shadowRoot.querySelector(".text-cell"),
			o = this.shadowRoot.querySelector(".divider"),
			r = this.shadowRoot.querySelector(".row"),
			s = this.shadowRoot.querySelector(".address");
		if (!(e && t && o && r && s)) return ((this.#P = 4), void (this.#O = 1));
		const n =
				4 *
					(e.getBoundingClientRect().width + t.getBoundingClientRect().width) +
				8,
			i = o.getBoundingClientRect().width,
			a = e.getBoundingClientRect().left - s.getBoundingClientRect().left,
			l = this.clientWidth - 1 - a - i;
		if (l < n) return ((this.#P = 4), void (this.#O = 1));
		((this.#P = 4 * Math.floor(l / n)),
			(this.#O = Math.floor(this.clientHeight / r.clientHeight)));
	}
	#G() {
		this.#L && !this.#j && (this.#L.observe(this), (this.#j = !0));
	}
	#r() {
		const e = s
			.section()
			...
			: "ArrowDown" === t.code
				...
				: "ArrowLeft" === t.code
					...
					: "ArrowRight" === t.code
						? (o = this.#a + 1)
						: "PageUp" === t.code
							? (o = this.#a - this.#P * this.#O)
							: "PageDown" === t.code && (o = this.#a + this.#P * this.#O),
			void 0 !== o &&
				o !== this.#U &&
				((this.#U = o), this.dispatchEvent(new Le(o))));
	}
	#K() {
		const e = [];
		for (let t = 0; t < this.#O; ++t) e.push(this.#X(t));
		return ze`${e}`;
	}
	#X(e) {
		const { startIndex: t, endIndex: o } = {
				startIndex: e * this.#P,
				endIndex: (e + 1) * this.#P,
			},
			s = {
				address: !0,
				selected: Math.floor((this.#a - this.#B) / this.#P) === e,
			};
		return ze`
    <div class="row">
      <span class=${r.Directives.classMap(s)}>${Ie({ number: t + this.#B, pad: 8, prefix: !1 })}</span>
      <span class="divider"></span>
      ${this.#W(t, o)}
      <span class="divider"></span>
      ${this.#Y(t, o)}
		...
		for (let n = e; n < t; ++n) {
			const t = n + this.#B,
				i = {
					cell: !0,
					"byte-cell": !0,
					"byte-group-margin": n !== e && (n - e) % 4 == 0,
					selected: n === this.#a - this.#B,
					"highlight-area": this.#Q(t),
					"focused-area": this.#Z(t),
				},
				a = n < this.#H.length,
				l = a ? ze`${Ie({ number: this.#H[n], pad: 2, prefix: !1 })}` : "",
				c = a ? this.#ee.bind(this, t) : "",
				d = s
					.tableCell()
					.track({ click: !0 })
					.context("linear-memory-inspector.byte-cell");
			o.push(
				ze`<span class=${r.Directives.classMap(i)} @click=${c} jslog=${d}>${l}</span>`
			);
		}
		return ze`${o}`;
	}
		...
		for (let n = e; n < t; ++n) {
			const e = n + this.#B,
				...
				a = {
					cell: !0,
					"text-cell": !0,
					selected: this.#a - this.#B === n,
					"highlight-area": t,
					"focused-area": i,
				},
				l = n < this.#H.length,
				c = l ? ze`${this.#te(this.#H[n])}` : "",
				d = l ? this.#ee.bind(this, n + this.#B) : "",
				h = s
					.tableCell()
					.track({ click: !0 })
					.context("linear-memory-inspector.text-cell");
			o.push(
				ze`<span class=${r.Directives.classMap(a)} @click=${d} jslog=${h}>${c}</span>`
			);
		}
		return ze`${o}`;
	}
	...
	#ee(e) {
		this.dispatchEvent(new Le(e));
	}
	#Q(e) {
		return (
			void 0 !== this.#_ &&
			this.#_.startAddress <= e &&
			e < this.#_.startAddress + this.#_.size
		);
	}
	#Z(e) {
		return (
			!!this.#o &&
			this.#o.startAddress <= e &&
			e < this.#o.startAddress + this.#o.size
		);
	}
}
t.CustomElements.defineComponent("devtools-linear-memory-inspector-viewer", je);
var He = Object.freeze({
	__proto__: null,
	ByteSelectedEvent: Le,
	ResizeEvent: Re,
	LinearMemoryViewer: je,
});
const { render: Be, html: _e } = r,
	Oe = {
		addressHasToBeANumberBetweenSAnd:
			"Address has to be a number between {PH1} and {PH2}",
	},
	Pe = e.i18n.registerUIStrings(
		"panels/linear_memory_inspector/components/LinearMemoryInspector.ts",
		Oe
	),
	Ve = e.i18n.getLocalizedString.bind(void 0, Pe);
class Ue extends Event {
	static eventName = "memoryrequest";
	data;
	constructor(e, t, o) {
		(super(Ue.eventName), (this.data = { start: e, end: t, address: o }));
	}
}
...
class De extends Event {
	...
	data;
	constructor(e) {
		(super(De.eventName), (this.data = e));
	}
}
class qe {
	#a = 0;
...
class Ge extends HTMLElement {
	static litTagName = r.literal`devtools-linear-memory-inspector-inspector`;
	#e = this.attachShadow({ mode: "open" });
	#re = new n.SimpleHistoryManager.SimpleHistoryManager(10);
	#H = new Uint8Array();
	#B = 0;
	#se = 0;
	#a = -1;
	...
	set data(e) {
		...
		if (e.memoryOffset < 0)
			throw new Error("Memory offset has to be greater or equal to zero.");
		if (e.highlightInfo) {
			if (e.highlightInfo.size < 0)
				throw new Error("Object size has to be greater than or equal to zero");
			if (
				e.highlightInfo.startAddress < 0 ||
				e.highlightInfo.startAddress >= e.outerMemoryLength
			)
				throw new Error("Object start address is out of bounds.");
		}
		((this.#H = e.memory),
			(this.#B = e.memoryOffset),
			(this.#se = e.outerMemoryLength),
			(this.#le = e.valueTypeModes || this.#le),
			(this.#v = e.valueTypes || this.#v),
			(this.#m = e.endianness || this.#m),
			(this.#_ = e.highlightInfo),
			this.#ce(e.address),
			this.#r());
	}
	#r() {
		const { start: e, end: t } = this.#de(this.#a, this.#ae),
			o = "Submitted" === this.#ne ? Me(this.#a) : this.#ie,
			r = this.#he(o),
			s = Ve(Oe.addressHasToBeANumberBetweenSAnd, {
				PH1: Me(0),
				PH2: Me(this.#se),
			}),
			n = r ? void 0 : s,
			i = this.#re.canRollback(),
			a = this.#re.canRollover(),
			l = this.#_ ? [this.#_] : [],
			c = this.#ge(l, this.#a);
		Be(
			_e`
      <div class="view">
        <${N.litTagName}
          ...
          @historynavigation=${this.#me}></${N.litTagName}>
          <${y.litTagName}
          .data=${{ highlightInfos: l, focusedMemoryHighlight: c }}
          @jumptohighlightedmemory=${this.#ye}>
          </${y.litTagName}>
        <${je.litTagName}
          .data=${{ memory: this.#H.slice(e - this.#B, t - this.#B), address: this.#a, memoryOffset: e, focus: "Submitted" === this.#ne, highlightInfo: this.#_, focusedMemoryHighlight: c }}
      ...
      <div class="value-interpreter">
        <${ke.litTagName}
          .data=${{ value: this.#H.slice(this.#a - this.#B, this.#a + 8).buffer, valueTypes: this.#v, valueTypeModes: this.#le, endianness: this.#m, memoryLength: this.#se }}
	...
	#ye(e) {
		(e.stopPropagation(), (this.#ne = "Submitted"));
		const t = Math.max(0, Math.min(e.data, this.#se - 1));
		this.#xe(t);
	}
	#pe() {
		const { start: e, end: t } = this.#de(this.#a, this.#ae);
		this.dispatchEvent(new Ue(e, t, this.#a));
	}
	#ve(e) {
		this.#ne = "Submitted";
		const t = Math.max(0, Math.min(e.data, this.#se - 1));
	...
	#ke() {
		return { valueTypes: this.#v, modes: this.#le, endianness: this.#m };
	}
	#we(e) {
		((this.#m = e.data), this.dispatchEvent(new De(this.#ke())), this.#r());
	}
	#he(e) {
		const t = Ee(e);
		return void 0 !== t && t >= 0 && t < this.#se;
	}
	#u(e) {
		const { address: t, mode: o } = e.data,
			r = this.#he(t),
			s = Ee(t);
		if (((this.#ie = t), void 0 !== s && r))
			return ((this.#ne = o), void this.#xe(s));
		((this.#ne = "Submitted" !== o || r ? "Edit" : "InvalidSubmit"), this.#r());
	}
	#fe(e) {
		const { type: t, checked: o } = e.data;
		(o ? this.#v.add(t) : this.#v.delete(t),
			this.dispatchEvent(new De(this.#ke())),
			this.#r());
	}
	#be(e) {
		e.stopImmediatePropagation();
		const { type: t, mode: o } = e.data;
		(this.#le.set(t, o), this.dispatchEvent(new De(this.#ke())), this.#r());
	}
	#me(e) {
		return "Forward" === e.data ? this.#re.rollover() : this.#re.rollback();
	}
	#ue(e) {
		const t = "Forward" === e.data ? this.#a + this.#ae : this.#a - this.#ae,
			o = Math.max(0, Math.min(t, this.#se - 1));
		this.#xe(o);
	}
	#xe(e) {
		e < 0 || e >= this.#se
	...
	#de(e, t) {
		const o = Math.floor(e / t) * t;
		return { start: o, end: Math.min(o + t, this.#se) };
	}
	#R(e) {
		((this.#ae = e.data), this.#F());
	}
	#F() {
		const { start: e, end: t } = this.#de(this.#a, this.#ae);
		e < this.#B || t > this.#B + this.#H.length
			? this.dispatchEvent(new Ue(e, t, this.#a))
			: this.#r();
	}
	#ce(e) {
		if (this.#a === e) return;
		const t = new qe(e, () => this.#xe(e));
		(this.#re.push(t), (this.#a = e), this.dispatchEvent(new Fe(this.#a)));
	}
	#ge(e, t) {
		let o;
		for (const r of e)
			r.startAddress <= t &&
				t < r.startAddress + r.size &&
				(o ? r.size < o.size && (o = r) : (o = r));
		return o;
	}
}
t.CustomElements.defineComponent(
...
var Je = Object.freeze({
		__proto__: null,
		MemoryRequestEvent: Ue,
		AddressChangedEvent: Fe,
		SettingsChangedEvent: De,
		LinearMemoryInspector: Ge,
	}),
	Ke = Object.freeze({ __proto__: null });
export {
	v as LinearMemoryHighlightChipList,
	Je as LinearMemoryInspector,
	Ne as LinearMemoryInspectorUtils,
	A as LinearMemoryNavigator,
	Se as LinearMemoryValueInterpreter,
	He as LinearMemoryViewer,
	Ke as LinearMemoryViewerUtils,
	se as ValueInterpreterDisplay,
	K as ValueInterpreterDisplayUtils,
	ue as ValueInterpreterSettings,
};
