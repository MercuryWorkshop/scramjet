import * as e from "../../../ui/lit-html/lit-html.js";
import * as t from "../../../ui/legacy/legacy.js";
import * as i from "../../../core/i18n/i18n.js";
import * as s from "../../../ui/components/buttons/buttons.js";
import * as r from "../../../ui/components/helpers/helpers.js";
import * as o from "../../../ui/components/icon_button/icon_button.js";
import * as n from "../../../ui/components/input/input.js";
...
import * as h from "../../../ui/components/code_highlighter/code_highlighter.js";
import "../../../ui/components/dialogs/dialogs.js";
import * as u from "../../../ui/components/menus/menus.js";
import * as g from "../../../ui/components/split_view/split_view.js";
import * as m from "../../../ui/components/text_editor/text_editor.js";
import * as v from "../extensions/extensions.js";
import * as b from "../../../ui/components/panel_feedback/panel_feedback.js";
import * as f from "../../../ui/components/panel_introduction_steps/panel_introduction_steps.js";
import * as y from "../../../ui/components/suggestion_input/suggestion_input.js";
import * as w from "../controllers/controllers.js";
import * as S from "../util/util.js";
...
const { html: k, Decorators: T, LitElement: E } = e,
	{ customElement: R, property: C } = T;
let N = class extends E {
	static styles = [x];
	constructor() {
		(super(), (this.label = ""), (this.shape = "square"), (this.disabled = !1));
	}
	#e = (e) => {
...
($([C()], N.prototype, "label", void 0),
	$([C()], N.prototype, "shape", void 0),
	$([C()], N.prototype, "disabled", void 0),
	(N = $([R("devtools-control-button")], N)));
var I = Object.freeze({
	__proto__: null,
	get ControlButton() {
		return N;
...
const B = {
	...
	A = i.i18n.registerUIStrings(
		"panels/recorder/components/CreateRecordingView.ts",
		B
	),
	P = i.i18n.getLocalizedString.bind(void 0, A);
class O extends Event {
	...
	constructor(e, t, i) {
		(super(O.eventName, {}),
			(this.name = e),
			(this.selectorAttribute = i || void 0),
			(this.selectorTypesToRecord = t));
	}
}
...
class F extends HTMLElement {
	static litTagName = e.literal`devtools-create-recording-view`;
	#t = this.attachShadow({ mode: "open" });
	#i = "";
	#s;
	#r;
	connectedCallback() {
		((this.#t.adoptedStyleSheets = [M, n.textInputStyles, n.checkboxStyles]),
			this.#o(),
			this.#t.querySelector("input")?.focus());
	}
	set data(e) {
		((this.#r = e.recorderSettings), (this.#i = this.#r.defaultTitle));
	}
	#n(e) {
		this.#s && ((this.#s = void 0), this.#o());
		"Enter" === e.key &&
			(this.startRecording(), e.stopPropagation(), e.preventDefault());
	}
	startRecording() {
		const e = this.#t.querySelector("#user-flow-name");
		if (!e) throw new Error("input#user-flow-name not found");
		if (!this.#r) throw new Error("settings not set");
		if (!e.value.trim())
			return (
				(this.#s = new Error(P(B.recordingNameIsRequired))),
				void this.#o()
			);
		const t = this.#t.querySelectorAll(".selector-type input[type=checkbox]"),
			i = [];
		for (const e of t) {
			const t = e,
				s = t.value;
		...
		if (
			!i.includes(a.Schema.SelectorType.CSS) &&
			!i.includes(a.Schema.SelectorType.XPath) &&
			!i.includes(a.Schema.SelectorType.Pierce)
		)
			return (
				(this.#s = new Error(P(B.includeNecessarySelectors))),
				void this.#o()
			);
		for (const e of Object.values(a.Schema.SelectorType))
			this.#r.setSelectorByType(e, i.includes(e));
		const s = this.#t.querySelector("#selector-attribute").value.trim();
		((this.#r.selectorAttribute = s),
			this.dispatchEvent(new O(e.value.trim(), i, s)));
	}
	#a() {
		this.dispatchEvent(new L());
	}
	#l = () => {
		this.#t.querySelector("#user-flow-name")?.select();
	};
	#o() {
		const t = new Map([
			[a.Schema.SelectorType.ARIA, P(B.selectorTypeARIA)],
			[a.Schema.SelectorType.CSS, P(B.selectorTypeCSS)],
			[a.Schema.SelectorType.Text, P(B.selectorTypeText)],
			[a.Schema.SelectorType.XPath, P(B.selectorTypeXPath)],
			[a.Schema.SelectorType.Pierce, P(B.selectorTypePierce)],
		]);
		e.render(
			e.html`
        <div class="wrapper">
          ...
          <div class="checkbox-container">
            ${Object.values(a.Schema.SelectorType).map((i) => {
							const s = this.#r?.getSelectorByType(i);
							return e.html`
...
r.CustomElements.defineComponent("devtools-create-recording-view", F);
var j = Object.freeze({
	__proto__: null,
	RecordingStartedEvent: O,
	RecordingCancelledEvent: L,
	CreateRecordingView: F,
});
...
const z = {
	...
	_ = i.i18n.registerUIStrings(
		"panels/recorder/components/RecordingListView.ts",
		z
	),
	V = i.i18n.getLocalizedString.bind(void 0, _);
...
class G extends Event {
	...
	constructor(e) {
		(super(G.eventName), (this.storageName = e));
	}
}
class q extends Event {
	...
	constructor(e) {
		(super(q.eventName), (this.storageName = e));
	}
}
class K extends Event {
	...
	constructor(e) {
		(super(K.eventName), (this.storageName = e));
	}
}
class H extends HTMLElement {
	static litTagName = e.literal`devtools-recording-list-view`;
	#t = this.attachShadow({ mode: "open" });
	#d = { recordings: [], replayAllowed: !0 };
	connectedCallback() {
		((this.#t.adoptedStyleSheets = [D]),
			r.ScheduledRender.scheduleRender(this, this.#o));
	}
	set recordings(e) {
		((this.#d.recordings = e), r.ScheduledRender.scheduleRender(this, this.#o));
	}
	set replayAllowed(e) {
		((this.#d.replayAllowed = e),
			r.ScheduledRender.scheduleRender(this, this.#o));
	}
	#c() {
	...
	#p(e, t) {
		(t.stopPropagation(), this.dispatchEvent(new G(e)));
	}
	#h(e, t) {
		(t.stopPropagation(), this.dispatchEvent(new q(e)));
	}
	#u(e, t) {
		(t.stopPropagation(), this.dispatchEvent(new K(e)));
	}
	#n(e, t) {
		"Enter" === t.key && this.#h(e, t);
	}
	#g(e) {
		e.stopPropagation();
	}
...
r.CustomElements.defineComponent("devtools-recording-list-view", H);
var W = Object.freeze({
	__proto__: null,
	CreateRecordingEvent: U,
	DeleteRecordingEvent: G,
	OpenRecordingEvent: q,
	PlayRecordingEvent: K,
	RecordingListView: H,
});
const X = new CSSStyleSheet();
X.replaceSync(
...
class Z extends Event {
	...
	constructor() {
		super(Z.eventName, { bubbles: !0, composed: !0 });
	}
}
class ee extends HTMLElement {
	static litTagName = e.literal`devtools-recorder-extension-view`;
	#t = this.attachShadow({ mode: "open" });
	#m;
	connectedCallback() {
		((this.#t.adoptedStyleSheets = [X]), this.#o());
	}
	disconnectedCallback() {
		this.#m &&
			v.ExtensionManager.ExtensionManager.instance().getView(this.#m.id).hide();
	}
	set descriptor(e) {
		((this.#m = e),
			this.#o(),
			v.ExtensionManager.ExtensionManager.instance().getView(e.id).show());
	}
	#v() {
	...
	#o() {
		if (!this.#m) return;
		const t = v.ExtensionManager.ExtensionManager.instance()
			.getView(this.#m.id)
			.frame();
		e.render(
			e.html`
...
r.CustomElements.defineComponent("devtools-recorder-extension-view", ee);
...
class re extends HTMLElement {
	...
	set disabled(e) {
		((this.#d.disabled = e), r.ScheduledRender.scheduleRender(this, this.#o));
	}
	get items() {
		return this.#d.items;
	}
	set items(e) {
		((this.#d.items = e), r.ScheduledRender.scheduleRender(this, this.#o));
	}
	set groups(e) {
		((this.#d.groups = e), r.ScheduledRender.scheduleRender(this, this.#o));
	}
	get value() {
		return this.#d.value;
	}
	set value(e) {
		((this.#d.value = e), r.ScheduledRender.scheduleRender(this, this.#o));
	}
	get variant() {
		return this.#d.variant;
	}
	set variant(e) {
		((this.#d.variant = e), r.ScheduledRender.scheduleRender(this, this.#o));
	}
	set action(e) {
		((this.#d.action = e), r.ScheduledRender.scheduleRender(this, this.#o));
	}
	#b(e) {
		(e.stopPropagation(), this.dispatchEvent(new se(this.#d.value)));
	}
	#f(e) {
		(this.dispatchEvent(new se(e.itemValue)),
			r.ScheduledRender.scheduleRender(this, this.#o));
	}
	#y(t, i) {
		return e.html`
	...
	#S(e) {
		return this.#d.action
			? a.Tooltip.getTooltipForActions(e, this.#d.action)
			: "";
	}
	#o = () => {
		const t = Boolean(this.#d.groups.length),
			i = t ? this.#d.groups.flatMap((e) => e.items) : this.#d.items,
			r = i.find((e) => e.value === this.#d.value) || i[0];
		if (!r) return;
		const o = {
				primary: "primary" === this.#d.variant,
				secondary: "secondary" === this.#d.variant,
			},
			n = "secondary" === this.#d.variant ? "secondary" : "primary",
			a = r.buttonLabel ? r.buttonLabel() : r.label();
		e.render(
			e.html`
...
r.CustomElements.defineComponent("devtools-select-button", re);
var oe = Object.freeze({
	__proto__: null,
	SelectButtonClickEvent: se,
	SelectButton: re,
});
const ne = {
	...
	ae = [
		{
			value: "normal",
			buttonIconName: "play",
			buttonLabel: () => ce(ne.ReplayNormalButtonLabel),
			label: () => ce(ne.ReplayNormalItemLabel),
		},
		{
			value: "slow",
			buttonIconName: "play",
			buttonLabel: () => ce(ne.ReplaySlowButtonLabel),
			label: () => ce(ne.ReplaySlowItemLabel),
		},
		{
			value: "very_slow",
			buttonIconName: "play",
			buttonLabel: () => ce(ne.ReplayVerySlowButtonLabel),
			label: () => ce(ne.ReplayVerySlowItemLabel),
		},
		{
			value: "extremely_slow",
			buttonIconName: "play",
			buttonLabel: () => ce(ne.ReplayExtremelySlowButtonLabel),
			label: () => ce(ne.ReplayExtremelySlowItemLabel),
		},
	],
	le = {
		normal: l.UserMetrics.RecordingReplaySpeed.Normal,
		slow: l.UserMetrics.RecordingReplaySpeed.Slow,
		very_slow: l.UserMetrics.RecordingReplaySpeed.VerySlow,
		extremely_slow: l.UserMetrics.RecordingReplaySpeed.ExtremelySlow,
	},
	de = i.i18n.registerUIStrings(
		"panels/recorder/components/ReplayButton.ts",
		ne
	),
	ce = i.i18n.getLocalizedString.bind(void 0, de);
class pe extends Event {
	speed;
	extension;
	static eventName = "startreplay";
	constructor(e, t) {
		(super(pe.eventName, { bubbles: !0, composed: !0 }),
			(this.speed = e),
			(this.extension = t));
	}
}
...
class ue extends HTMLElement {
	static litTagName = e.literal`devtools-replay-button`;
	#t = this.attachShadow({ mode: "open" });
	#x = this.#o.bind(this);
	#d = { disabled: !1 };
	#$;
	#k = [];
	set data(e) {
		((this.#$ = e.settings), (this.#k = e.replayExtensions));
	}
	get disabled() {
		return this.#d.disabled;
	}
	set disabled(e) {
		((this.#d.disabled = e), r.ScheduledRender.scheduleRender(this, this.#x));
	}
	connectedCallback() {
		r.ScheduledRender.scheduleRender(this, this.#x);
	}
	#T(e) {
		if ((e.stopPropagation(), e.value.startsWith(he))) {
			this.#$ && (this.#$.replayExtension = e.value);
			const t = Number(e.value.substring(9));
			return (
				this.dispatchEvent(new pe("normal", this.#k[t])),
				void r.ScheduledRender.scheduleRender(this, this.#x)
			);
		}
		const t = e.value;
		(this.#$ && ((this.#$.speed = t), (this.#$.replayExtension = "")),
			l.userMetrics.recordingReplaySpeed(le[t]),
			this.dispatchEvent(new pe(e.value)),
			r.ScheduledRender.scheduleRender(this, this.#x));
	}
	#o() {
		const t = [{ name: ce(ne.speedGroup), items: ae }];
		(this.#k.length &&
			t.push({
				name: ce(ne.extensionGroup),
				items: this.#k.map((e, t) => ({
					value: he + t,
					buttonIconName: "play",
					buttonLabel: () => e.getName(),
					label: () => e.getName(),
				})),
			}),
			e.render(
...
r.CustomElements.defineComponent("devtools-replay-button", ue);
...
class be extends HTMLElement {
	static litTagName = e.literal`devtools-timeline-section`;
	#E = !1;
	#R = !1;
	#C = !1;
	#N = !1;
	...
	constructor() {
		super();
		this.attachShadow({ mode: "open" }).adoptedStyleSheets = [ve];
	}
	set data(e) {
		((this.#C = e.isFirstSection),
			(this.#N = e.isLastSection),
			(this.#E = e.isEndOfGroup),
			(this.#R = e.isStartOfGroup),
			(this.#I = e.isSelected),
			this.#o());
	}
	connectedCallback() {
	...
	#o() {
		const t = {
			"timeline-section": !0,
			"is-end-of-group": this.#E,
			"is-start-of-group": this.#R,
			"is-first-section": this.#C,
			"is-last-section": this.#N,
			"is-selected": this.#I,
		};
		e.render(
			e.html`
...
r.CustomElements.defineComponent("devtools-timeline-section", be);
var fe = Object.freeze({ __proto__: null, TimelineSection: be });
const ye = {
	...
	we = i.i18n.registerUIStrings("panels/recorder/components/StepView.ts", ye),
	Se = i.i18n.getLocalizedString.bind(void 0, we);
class xe extends Event {
	static eventName = "captureselectors";
	data;
	constructor(e) {
		(super(xe.eventName, { bubbles: !0, composed: !0 }), (this.data = e));
	}
}
class $e extends Event {
	static eventName = "stopselectorscapture";
	constructor() {
		super($e.eventName, { bubbles: !0, composed: !0 });
	}
}
class ke extends Event {
	...
	step;
	constructor(e) {
		(super(ke.eventName, { bubbles: !0, composed: !0 }), (this.step = e));
	}
}
class Te extends Event {
	...
	newStep;
	constructor(e, t) {
		(super(Te.eventName, { bubbles: !0, composed: !0 }),
			(this.currentStep = e),
			(this.newStep = t));
	}
}
class Ee extends Event {
	...
	stepOrSection;
	constructor(e, t) {
		(super(Ee.eventName, { bubbles: !0, composed: !0 }),
...
class Re extends Event {
	...
	step;
	constructor(e) {
		(super(Re.eventName, { bubbles: !0, composed: !0 }), (this.step = e));
	}
}
class Ce extends Event {
	...
	index;
	constructor(e) {
		(super(Ce.eventName, { bubbles: !0, composed: !0 }), (this.index = e));
	}
}
class Ne extends Event {
	...
	index;
	constructor(e) {
		(super(Ne.eventName, { bubbles: !0, composed: !0 }), (this.index = e));
	}
}
const Ie = "copy-step-as-";
class Me extends HTMLElement {
	static litTagName = e.literal`devtools-step-view`;
	#t = this.attachShadow({ mode: "open" });
	#M;
	#B;
	#A = "default";
	...
	set data(e) {
		...
		((this.#M = e.step),
			...
			(this.#R = e.isStartOfGroup),
			(this.#O = e.stepIndex),
			(this.#L = e.sectionIndex),
			(this.#C = e.isFirstSection),
			(this.#N = e.isLastSection),
			(this.#F = e.isRecording),
			(this.#j = e.isPlaying),
			(this.#U = e.hasBreakpoint),
			(this.#G = e.removable),
			(this.#q = e.builtInConverters),
			(this.#K = e.extensionConverters),
			(this.#I = e.isSelected),
			(this.#r = e.recorderSettings),
			this.#o(),
			this.#A === t ||
				"current" !== this.#A ||
				this.#_ ||
	...
	connectedCallback() {
		((this.#t.adoptedStyleSheets = [me]), this.#V.observe(this), this.#o());
	}
	disconnectedCallback() {
		this.#V.unobserve(this);
	}
	#H() {
	...
	#W(e) {
		const t = e;
		("Enter" !== t.key && " " !== t.key) ||
			(this.#H(), e.stopPropagation(), e.preventDefault());
	}
	#X() {
		if (this.#B)
			return this.#B.title
				? this.#B.title
				: e.html`<span class="fallback">(No Title)</span>`;
		if (!this.#M) throw new Error("Missing both step and section");
		switch (this.#M.type) {
			case a.Schema.StepType.CustomStep:
				return Se(ye.customStepTitle);
			case a.Schema.StepType.SetViewport:
				return Se(ye.setViewportClickTitle);
			case a.Schema.StepType.Click:
				return Se(ye.clickStepTitle);
			case a.Schema.StepType.DoubleClick:
				return Se(ye.doubleClickStepTitle);
			case a.Schema.StepType.Hover:
				return Se(ye.hoverStepTitle);
			case a.Schema.StepType.EmulateNetworkConditions:
				return Se(ye.emulateNetworkConditionsStepTitle);
			case a.Schema.StepType.Change:
				return Se(ye.changeStepTitle);
			case a.Schema.StepType.Close:
				return Se(ye.closeStepTitle);
			case a.Schema.StepType.Scroll:
				return Se(ye.scrollStepTitle);
			case a.Schema.StepType.KeyUp:
				return Se(ye.keyUpStepTitle);
			case a.Schema.StepType.KeyDown:
				return Se(ye.keyDownStepTitle);
			case a.Schema.StepType.WaitForElement:
				return Se(ye.waitForElementStepTitle);
			case a.Schema.StepType.WaitForExpression:
				return Se(ye.waitForExpressionStepTitle);
			case a.Schema.StepType.Navigate:
				return Se(ye.navigateStepTitle);
		}
	}
	#Y(e) {
		switch (e) {
			case "button":
				return Se(ye.elementRoleButton);
			case "input":
				return Se(ye.elementRoleInput);
			default:
				return Se(ye.elementRoleFallback);
		}
	}
	#J() {
		if (!this.#M || !("selectors" in this.#M)) return "";
		const e = this.#M.selectors.flat().find((e) => e.startsWith("aria/"));
		if (!e) return "";
		const t = e.match(/^aria\/(.+?)(\[role="(.+)"\])?$/);
		return t ? `${this.#Y(t[3])} "${t[1]}"` : "";
	}
	#Q() {
		return this.#B ? this.#B.url : "";
	}
	#Z(e) {
		const t = this.#M || this.#B?.causingStep;
		if (!t) throw new Error("Expected step.");
		this.dispatchEvent(new Te(t, e.data));
	}
	#ee(e) {
		switch (e.itemValue) {
			case "add-step-before": {
				const e = this.#M || this.#B;
				if (!e) throw new Error("Expected step or section.");
				this.dispatchEvent(new Ee(e, "before"));
				break;
			}
			case "add-step-after": {
				const e = this.#M || this.#B;
				if (!e) throw new Error("Expected step or section.");
				this.dispatchEvent(new Ee(e, "after"));
				break;
			}
			case "remove-step": {
				const e = this.#B?.causingStep;
				if (!this.#M && !e) throw new Error("Expected step.");
				this.dispatchEvent(new Re(this.#M || e));
				break;
			}
			case "add-breakpoint":
				if (!this.#M) throw new Error("Expected step");
				this.dispatchEvent(new Ce(this.#O));
				break;
			case "remove-breakpoint":
				if (!this.#M) throw new Error("Expected step");
				this.dispatchEvent(new Ne(this.#O));
				break;
			default: {
				const t = e.itemValue;
				if (!t.startsWith(Ie)) throw new Error("Unknown step action.");
				const i = this.#M || this.#B?.causingStep;
				if (!i) throw new Error("Step not found.");
				const s = t.substring(13);
				(this.#r && (this.#r.preferredCopyFormat = s),
					this.dispatchEvent(new ke(structuredClone(i))));
			}
		}
	}
	#te(e) {
		(e.stopPropagation(), e.preventDefault(), (this.#z = !this.#z), this.#o());
	}
	#ie() {
		((this.#z = !1), this.#o());
	}
	...
	#re() {
		if (!this.#D) throw new Error("Missing actionsMenuButton");
		return this.#D;
	}
	#oe = () => {
		const e = [];
		if (
			(this.#j ||
				(this.#M &&
					e.push({
						id: "add-step-before",
						label: Se(ye.addStepBefore),
						group: "stepManagement",
						groupTitle: Se(ye.stepManagement),
					}),
				e.push({
					id: "add-step-after",
					label: Se(ye.addStepAfter),
					group: "stepManagement",
					groupTitle: Se(ye.stepManagement),
				}),
				this.#G &&
			...
			this.#M &&
				...
				(this.#U
					? e.push({
							...
							label: Se(ye.removeBreakpoint),
							group: "breakPointManagement",
							groupTitle: Se(ye.breakpoints),
						})
					: e.push({
							id: "add-breakpoint",
							label: Se(ye.addBreakpoint),
							group: "breakPointManagement",
							groupTitle: Se(ye.breakpoints),
						})),
			this.#M)
	...
	#ne() {
		const t = this.#oe(),
			i = new Map();
		for (const e of t) {
			const t = i.get(e.group);
			t ? t.push(e) : i.set(e.group, [e]);
		}
		const o = [];
		for (const [e, t] of i)
			o.push({ group: e, groupTitle: t[0].groupTitle, actions: t });
		return e.html`
      <${s.Button.Button.litTagName}
        class="step-actions"
        ...
        on-render=${r.Directives.nodeRenderedCallback((e) => {
					this.#D = e;
				})}
        ...
        ${e.Directives.repeat(
            ...
            >
              ${e.Directives.repeat(
								t.actions,
								(e) => e.id,
								(t) => e.html`<${u.Menu.MenuItem.litTagName}
                      .value=${t.id}
                    >
                      ${t.label}
                    </${u.Menu.MenuItem.litTagName}>
	...
	#ae = (e) => {
		...
		const i = new t.ContextMenu.ContextMenu(e),
			s = this.#oe(),
			r = s.filter((e) => e.id.startsWith(Ie)),
			o = s.filter((e) => !e.id.startsWith(Ie));
		for (const e of o) {
			i.section(e.group).appendItem(e.label, () => {
				this.#ee(new u.Menu.MenuItemSelectedEvent(e.id));
			});
		}
		const n = r.find((e) => e.id === Ie + this.#r?.preferredCopyFormat);
		if (
			(n &&
				i.section("copy").appendItem(n.label, () => {
					this.#ee(new u.Menu.MenuItemSelectedEvent(n.id));
				}),
			r.length)
		) {
			const e = i.section("copy").appendSubMenuItem(Se(ye.copyAs));
			for (const t of r)
				t !== n &&
					e.section(t.group).appendItem(t.label, () => {
						this.#ee(new u.Menu.MenuItemSelectedEvent(t.id));
					});
		}
		i.show();
	};
	#o() {
		if (!this.#M && !this.#B) return;
		const t = {
				step: !0,
				expanded: this.#P,
				"is-success": "success" === this.#A,
				"is-current": "current" === this.#A,
				"is-outstanding": "outstanding" === this.#A,
				"is-error": "error" === this.#A,
				"is-stopped": "stopped" === this.#A,
				"is-start-of-group": this.#R,
				"is-first-section": this.#C,
				"has-breakpoint": this.#U,
			},
			i = Boolean(this.#M),
			s = this.#X(),
			r = this.#M ? this.#J() : this.#Q();
		e.render(
			e.html`
      <${be.litTagName} .data=${{ isFirstSection: this.#C, isLastSection: this.#N, isStartOfGroup: this.#R, isEndOfGroup: this.#E, isSelected: this.#I }} @contextmenu=${this.#ae} data-step-index=${this.#O} data-section-index=${this.#L} class=${e.Directives.classMap(t)}>
        ...
        <div class="details">
          ${
						this.#M &&
						e.html`<devtools-recorder-step-editor
...
r.CustomElements.defineComponent("devtools-step-view", Me);
var Be = Object.freeze({
	__proto__: null,
	CaptureSelectorsEvent: xe,
	StopSelectorsCaptureEvent: $e,
	CopyStepEvent: ke,
	StepChanged: Te,
	AddStep: Ee,
	RemoveStep: Re,
	AddBreakpointEvent: Ce,
	RemoveBreakpointEvent: Ne,
	StepView: Me,
});
const Ae = {
		mobile: "Mobile",
	...
	Pe = i.i18n.registerUIStrings(
		"panels/recorder/components/RecordingView.ts",
		Ae
	),
	Oe = i.i18n.getLocalizedString.bind(void 0, Pe);
...
class Fe extends Event {
	static eventName = "playrecording";
	data;
	constructor(e = { targetPanel: "chrome_recorder", speed: "normal" }) {
		(super(Fe.eventName), (this.data = e));
	}
}
...
class De extends Event {
	...
	data;
	constructor(e, t) {
		(super(De.eventName), (this.data = { currentStep: e, newStep: t }));
	}
}
...
class _e extends Event {
	...
	title;
	constructor(e) {
		(super(_e.eventName, {}), (this.title = e));
	}
}
class Ve extends Event {
	...
	data;
	constructor(e) {
		(super(Ve.eventName, { composed: !0, bubbles: !0 }), (this.data = e));
	}
}
class Ue extends Event {
	...
	data;
	constructor(e) {
		(super(Ue.eventName, { composed: !0, bubbles: !0 }), (this.data = e));
	}
}
const Ge = [
	c.NetworkManager.NoThrottlingConditions,
	c.NetworkManager.OfflineConditions,
	c.NetworkManager.Slow3GConditions,
	c.NetworkManager.Fast3GConditions,
];
class qe extends HTMLElement {
	static litTagName = e.literal`devtools-recording-view`;
	#t = this.attachShadow({ mode: "open" });
	#le = { isPlaying: !1, isPausedOnBreakpoint: !1 };
	#de = null;
	#F = !1;
	#ce = !1;
	#pe = !1;
	...
	#Te;
	#Ee;
	#Re = this.#Ce.bind(this);
	set data(e) {
		((this.#F = e.isRecording),
			(this.#le = e.replayState),
			(this.#ce = e.recordingTogglingInProgress),
			(this.#he = e.currentStep),
			(this.#de = e.recording),
			(this.#ue = this.#de.steps),
			(this.#me = e.sections),
			(this.#$ = e.settings),
			(this.#r = e.recorderSettings),
			(this.#ge = e.currentError),
			(this.#ve = e.lastReplayResult),
			(this.#we = e.replayAllowed),
			(this.#pe = !1),
			(this.#be = e.breakpointIndexes),
			(this.#q = e.builtInConverters),
			(this.#K = e.extensionConverters),
			(this.#k = e.replayExtensions),
			(this.#Ee = e.extensionDescriptor),
			(this.#$e =
				this.#r?.preferredCopyFormat ?? e.builtInConverters[0]?.getId()),
			this.#Ne(),
			this.#o());
	}
	connectedCallback() {
		((this.#t.adoptedStyleSheets = [te, n.textInputStyles]),
			document.addEventListener("copy", this.#Re),
			this.#o());
	}
	disconnectedCallback() {
		document.removeEventListener("copy", this.#Re);
	}
	scrollToBottom() {
		const e = this.shadowRoot?.querySelector(".sections");
	...
	#Pe(e) {
		if (!this.#he) return "default";
		if (e === this.#he)
			return this.#ge
				? "error"
				: this.#le.isPlaying
					? this.#le.isPausedOnBreakpoint
						? "stopped"
						: "current"
					: "success";
		const t = this.#ue.indexOf(this.#he);
		if (-1 === t) return "default";
		return this.#ue.indexOf(e) < t ? "success" : "outstanding";
	}
	#Oe(e) {
		const t = this.#he;
		if (!t) return "default";
		const i = this.#me.find((e) => e.steps.includes(t));
		if (!i && this.#ge) return "error";
		if (e === i) return "success";
		return this.#me.indexOf(i) >= this.#me.indexOf(e)
			? "success"
			: "outstanding";
	}
	#Le(t, i, s) {
		const r = this.#ue.indexOf(i);
		return e.html`
      <${Me.litTagName}
      @click=${this.#Fe}
      @mouseover=${this.#je}
	...
	#je = (e) => {
		const t = e.target,
			i = t.step || t.section?.causingStep;
		i && !this.#fe && this.#ze(i);
	};
	#Fe(e) {
		e.stopPropagation();
		const t = e.target,
			i = t.step || t.section?.causingStep || null;
		this.#fe !== i && ((this.#fe = i), this.#o(), i && this.#ze(i, !0));
	}
	#_e() {
		void 0 !== this.#fe && ((this.#fe = void 0), this.#o());
	}
	#Ve(e) {
		"Enter" === e.key && (e.preventDefault(), this.#Ue(e));
	}
	#Ue(e) {
		(e.stopPropagation(), (this.#ye = !this.#ye), this.#o());
	}
	#Ge(e) {
		const t = Ge.find((t) => t.i18nTitleKey === e.itemValue);
		this.dispatchEvent(
			new Ve(
				t?.i18nTitleKey === c.NetworkManager.NoThrottlingConditions.i18nTitleKey
					? void 0
					: t
			)
		);
	...
	#qe(e) {
		const t = e.target;
		t.checkValidity()
			? this.dispatchEvent(new Ue(Number(t.value)))
			: t.reportValidity();
	}
	#Ke = (e) => {
		const t = e.target.innerText.trim();
		if (!t) return ((this.#pe = !0), void this.#o());
		this.dispatchEvent(new _e(t));
	};
	#He = (e) => {
		switch (e.code) {
			case "Escape":
	...
	#We = () => {
		const e = this.#t.getElementById("title-input");
		e.focus();
	...
	#Xe = (e) => {
		const t = e.target;
		t.matches(".wrapping-label") &&
			t.querySelector("devtools-select-menu")?.click();
	};
	async #Ye(e) {
		let t = [...this.#q, ...this.#K].find(
			(e) => e.getId() === this.#r?.preferredCopyFormat
		);
		if ((t || (t = this.#q[0]), !t))
			throw new Error("No default converter found");
		let i = "";
		(e
			? (i = await t.stringifyStep(e))
			: this.#de && ([i] = await t.stringify(this.#de)),
			l.InspectorFrontendHost.InspectorFrontendHostInstance.copyText(i));
		const s = e
			? (function (e) {
					switch (e) {
						case "puppeteer":
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedStepWithPuppeteer;
						case "json":
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedStepWithJSON;
						case "@puppeteer/replay":
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedStepWithReplay;
						default:
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedStepWithExtension;
					}
				})(t.getId())
			: (function (e) {
					switch (e) {
						case "puppeteer":
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedRecordingWithPuppeteer;
						case "json":
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedRecordingWithJSON;
						case "@puppeteer/replay":
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedRecordingWithReplay;
						default:
							return l.UserMetrics.RecordingCopiedToClipboard
								.CopiedRecordingWithExtension;
					}
				})(t.getId());
		l.userMetrics.recordingCopiedToClipboard(s);
	}
	#De(e) {
		(e.stopPropagation(), this.#Ye(e.step));
	}
	async #Ce(e) {
		e.target === document.body &&
			(e.preventDefault(),
			await this.#Ye(this.#fe),
			l.userMetrics.keyboardShortcutFired(
				"chrome_recorder.copy-recording-or-step"
			));
	}
	#Je() {
		if (!this.#$) return e.html``;
		const t = [];
		this.#$.viewportSettings &&
			(t.push(
				e.html`<div>${this.#$.viewportSettings.isMobile ? Oe(Ae.mobile) : Oe(Ae.desktop)}</div>`
			),
			t.push(e.html`<div class="separator"></div>`),
			t.push(
				e.html`<div>${this.#$.viewportSettings.width}×${this.#$.viewportSettings.height} px</div>`
			));
		const i = [];
		if (this.#ye) {
			const t =
					this.#$.networkConditionsSettings?.i18nTitleKey ||
					c.NetworkManager.NoThrottlingConditions.i18nTitleKey,
				s = Ge.find((e) => e.i18nTitleKey === t);
			let r = "";
			(s && (r = s.title instanceof Function ? s.title() : s.title),
				i.push(e.html`<div class="editable-setting">
        <label class="wrapping-label" @click=${this.#Xe}>
          ...
          >
            ${Ge.map(
							(i) => e.html`<${u.Menu.MenuItem.litTagName}
        ...
        <label class="wrapping-label" title=${Oe(Ae.timeoutExplanation)}>
          ...
          <input
			...
			(this.#$.networkConditionsSettings
				? this.#$.networkConditionsSettings.title
					? i.push(
							e.html`<div>${this.#$.networkConditionsSettings.title}</div>`
						)
					: i.push(e.html`<div>
            ...
            ${Oe(Ae.latency, { value: this.#$.networkConditionsSettings.latency })}
				...
				: i.push(
						e.html`<div>${c.NetworkManager.NoThrottlingConditions.title instanceof Function ? c.NetworkManager.NoThrottlingConditions.title() : c.NetworkManager.NoThrottlingConditions.title}</div>`
					),
				i.push(e.html`<div class="separator"></div>`),
				i.push(
					e.html`<div>${Oe(Ae.timeout, { value: this.#$.timeout || a.RecordingPlayer.defaultTimeout })}</div>`
				));
		const s = !this.#F && !this.#le.isPlaying,
			r = { "settings-title": !0, expanded: this.#ye },
			n = { expanded: this.#ye, settings: !0 };
		return e.html`
	...
	#rt(e) {
		(e.stopPropagation(),
			this.dispatchEvent(new Fe({ targetPanel: "timeline", speed: "normal" })));
	}
	showCodeToggle = () => {
		((this.#Se = !this.#Se),
			l.userMetrics.recordingCodeToggled(
				this.#Se
					? l.UserMetrics.RecordingCodeToggled.CodeShown
					: l.UserMetrics.RecordingCodeToggled.CodeHidden
			),
			this.#Ne());
	};
	#Ne = async () => {
		...
		const e = this.#Qe();
		if (!e) return;
		const [t, i] = await e.stringify(this.#de);
		((this.#xe = t), (this.#Te = i), this.#Te?.shift());
		const s = e.getMediaType(),
			r = s ? await h.CodeHighlighter.languageFromMIME(s) : null;
		((this.#ke = p.EditorState.create({
			doc: this.#xe,
			extensions: [
				m.Config.baseConfiguration(this.#xe),
				p.EditorState.readOnly.of(!0),
	...
	#ze = (e, t = !1) => {
		if (!this.#Te) return;
		const i = this.#ue.indexOf(e);
		if (-1 === i) return;
		const s = this.#t.querySelector("devtools-text-editor");
		if (!s) return;
		const r = s.editor;
		if (!r) return;
		const o = this.#Te[2 * i],
			n = this.#Te[2 * i + 1];
		let a = s.createSelection(
			{ lineNumber: o + n, columnNumber: 0 },
			{ lineNumber: o, columnNumber: 0 }
		);
		const l = s.state.doc.lineAt(a.main.anchor);
		((a = s.createSelection(
			{ lineNumber: o + n - 1, columnNumber: l.length + 1 },
			{ lineNumber: o, columnNumber: 0 }
		)),
			r.dispatch({
				selection: a,
				effects: t
					? [p.EditorView.scrollIntoView(a.main, { y: "nearest" })]
					: void 0,
			}));
	};
	#tt = (e) => {
		((this.#$e = e.itemValue),
			this.#r && (this.#r.preferredCopyFormat = e.itemValue),
			this.#Ne());
	};
	#et() {
		return e.html`
      ...
      ${
				this.#Se
					? ""
					: e.html`<div class="section-toolbar">
        <${s.Button.Button.litTagName}
          @click=${this.showCodeToggle}
          class="show-code"
            ...
            <div class="section">
              ...
              <div class="content">
                <div class="steps">
                  ...
                  ${
										!this.#ce && this.#F && i === this.#me.length - 1
											? e.html`<devtools-button
                    class="step add-assertion-button"
                    .data=${{ variant: "secondary", title: Oe(Ae.addAssertion) }}
                    @click=${this.#Ie}
                  >${Oe(Ae.addAssertion)}</devtools-button>`
											: void 0
									}
	...
	#ot() {
		if (!this.#de) return "";
		const { title: t } = this.#de,
			i = !this.#le.isPlaying && !this.#F;
		return e.html`
      <div class="header">
        <div class="header-title-wrapper">
          ...
          ${
						this.#pe
							? e.html`<div class="title-input-error-text">
	...
	#nt() {
		if (!this.#F) return "";
		const t = this.#ce ? Oe(Ae.recordingIsBeingStopped) : Oe(Ae.endRecording);
		return e.html`
      <div class="footer">
        <div class="controls">
          <devtools-control-button
	...
	#o() {
		const t = {
			wrapper: !0,
			"is-recording": this.#F,
			"is-playing": this.#le.isPlaying,
			"was-successful": "Success" === this.#ve,
			"was-failure": "Failure" === this.#ve,
		};
		e.render(
			e.html`
...
r.CustomElements.defineComponent("devtools-recording-view", qe);
var Ke = Object.freeze({
	__proto__: null,
	RecordingFinishedEvent: Le,
	PlayRecordingEvent: Fe,
	AbortReplayEvent: je,
	RecordingChangedEvent: De,
	AddAssertionEvent: ze,
	RecordingTitleChangedEvent: _e,
	NetworkConditionsChanged: Ve,
	TimeoutChanged: Ue,
	RecordingView: qe,
});
...
const We = {
		...
		step3:
			"Generate a detailed performance trace or export a Puppeteer script for testing",
		createRecording: "Create a new recording",
		quickStart: "Quick start: learn the new Recorder panel in DevTools",
	},
	Xe = i.i18n.registerUIStrings("panels/recorder/components/StartView.ts", We),
	Ye = i.i18n.getLocalizedString.bind(void 0, Xe),
	Je = "https://goo.gle/recorder-feedback";
...
class Ze extends HTMLElement {
	static litTagName = e.literal`devtools-start-view`;
	#t = this.attachShadow({ mode: "open" });
	connectedCallback() {
		((this.#t.adoptedStyleSheets = [He]),
			r.ScheduledRender.scheduleRender(this, this.#o));
	}
	#at() {
		this.dispatchEvent(new Qe());
	}
	#o = () => {
		e.render(
			e.html`
...
r.CustomElements.defineComponent("devtools-start-view", Ze);
var et = Object.freeze({
	__proto__: null,
	FEEDBACK_URL: Je,
	CreateRecordingEvent: Qe,
	StartView: Ze,
});
...
function it(e, t = "Assertion failed!") {
	if (!e) throw new Error(t);
}
const st = (e) => {
	for (const t of Reflect.ownKeys(e)) {
		const i = e[t];
		((i && "object" == typeof i) || "function" == typeof i) && st(i);
	}
...
const nt = (e, t) => {
	if (t instanceof ot) {
		it(Array.isArray(e), `Expected an array. Got ${typeof e}.`);
		const i = [...e],
			s = Object.keys(t.value).sort((e, t) => Number(t) - Number(e));
		for (const e of s) {
			const s = t.value[Number(e)];
			void 0 === s
				? i.splice(Number(e), 1)
				: s instanceof rt
					? i.splice(Number(e), 0, s.value)
					: (i[Number(e)] = nt(i[e], s));
		}
		return Object.freeze(i);
	}
	if ("object" == typeof t && !Array.isArray(t)) {
		it(!Array.isArray(e), "Expected an object. Got an array.");
		const i = { ...e },
			s = Object.keys(t);
		for (const e of s) {
			const s = t[e];
			void 0 === s ? delete i[e] : (i[e] = nt(i[e], s));
		}
		return Object.freeze(i);
	}
	return t;
...
const { html: lt, Decorators: dt, Directives: ct, LitElement: pt } = e,
	{ customElement: ht, property: ut, state: gt } = dt,
	{ live: mt } = ct,
	vt = Object.freeze({
		string: (e) => e.trim(),
		number: (e) => {
	...
	yt = st({
		...
		[a.Schema.StepType.Hover]: {
			required: ["selectors"],
			optional: ["assertedEvents", "frame", "target", "timeout"],
		},
		[a.Schema.StepType.Change]: {
			required: ["selectors", "value"],
			optional: ["assertedEvents", "frame", "target", "timeout"],
		},
		[a.Schema.StepType.KeyDown]: {
			required: ["key"],
			optional: ["assertedEvents", "target", "timeout"],
		},
		[a.Schema.StepType.KeyUp]: {
			required: ["key"],
			optional: ["assertedEvents", "target", "timeout"],
		},
		[a.Schema.StepType.Scroll]: {
			required: [],
			optional: ["assertedEvents", "frame", "target", "timeout", "x", "y"],
		},
		[a.Schema.StepType.Close]: {
			required: [],
			optional: ["assertedEvents", "target", "timeout"],
		},
		[a.Schema.StepType.Navigate]: {
			required: ["url"],
			optional: ["assertedEvents", "target", "timeout"],
		},
		[a.Schema.StepType.WaitForElement]: {
			required: ["selectors"],
			optional: [
				"assertedEvents",
				"attributes",
		...
		[a.Schema.StepType.WaitForExpression]: {
			required: ["expression"],
			optional: ["assertedEvents", "frame", "target", "timeout"],
		},
		[a.Schema.StepType.CustomStep]: {
			required: ["name", "parameters"],
			optional: ["assertedEvents", "target", "timeout"],
		},
		[a.Schema.StepType.EmulateNetworkConditions]: {
			required: ["download", "latency", "upload"],
			optional: ["assertedEvents", "target", "timeout"],
		},
		[a.Schema.StepType.SetViewport]: {
			required: [
				"deviceScaleFactor",
				"hasTouch",
				"height",
	...
	St = i.i18n.registerUIStrings("panels/recorder/components/StepEditor.ts", wt),
	xt = i.i18n.getLocalizedString.bind(void 0, St);
class $t extends Event {
	static eventName = "stepedited";
	data;
	constructor(e) {
		(super($t.eventName, { bubbles: !0, composed: !0 }), (this.data = e));
	}
}
class kt {
	static #lt = new S.SharedObject.SharedObject(
		() => a.RecordingPlayer.RecordingPlayer.connectPuppeteer(),
		({ browser: e }) => a.RecordingPlayer.RecordingPlayer.disconnectPuppeteer(e)
	);
	static async default(e) {
		...
		for (const e of i.required)
			s = Promise.all([
				s,
				(async () =>
					Object.assign(t, { [e]: await this.defaultByAttribute(t, e) }))(),
			]);
		return (await s, Object.freeze(t));
	}
	static async defaultByAttribute(e, t) {
		return this.#lt.run((e) => {
			switch (t) {
				case "assertedEvents":
					return nt(
						ft.assertedEvents,
						new ot({ 0: { url: e.page.url() || ft.assertedEvents[0].url } })
					);
				case "url":
					return e.page.url() || ft.url;
				case "height":
					return e.page.evaluate(() => visualViewport.height) || ft.height;
				case "width":
					return e.page.evaluate(() => visualViewport.width) || ft.width;
				default:
					return ft[t];
			}
		});
	...
	static fromStep(e) {
		...
		for (const i of ["parameters", "properties"])
			i in e && void 0 !== e[i] && (t[i] = JSON.stringify(e[i]));
		if ("attributes" in e && e.attributes) {
			t.attributes = [];
			for (const [i, s] of Object.entries(e.attributes))
				t.attributes.push({ name: i, value: s });
		}
		return (
			"selectors" in e &&
				(t.selectors = e.selectors.map((e) =>
					"string" == typeof e ? [e] : [...e]
				)),
			st(t)
		);
	...
	static toStep(e) {
		...
		for (const i of ["parameters", "properties"]) {
			const s = e[i];
			s && Object.assign(t, { [i]: JSON.parse(s) });
		}
		if (e.attributes)
			if (0 !== e.attributes.length) {
				...
				for (const { name: t, value: s } of e.attributes)
					Object.assign(i, { [t]: s });
				Object.assign(t, { attributes: i });
			} else "attributes" in t && delete t.attributes;
		if (e.selectors) {
			const i = e.selectors
				.filter((e) => e.length > 0)
				.map((e) => (1 === e.length ? e[0] : [...e]));
			0 !== i.length
				? Object.assign(t, { selectors: i })
				: "selectors" in t && delete t.selectors;
		}
		return (
			e.frame && 0 === e.frame.length && "frame" in t && delete t.frame,
			(i = a.SchemaUtils.parseStep(t)),
...
let Tt = class extends pt {
	static styles = [tt];
	#dt = new w.SelectorPicker.SelectorPicker(this);
	constructor() {
		(super(), (this.disabled = !1));
	}
	#e = (e) => {
		(e.preventDefault(), e.stopPropagation(), this.#dt.toggle());
	};
	disconnectedCallback() {
		(super.disconnectedCallback(), this.#dt.stop());
	}
	render() {
		if (!this.disabled)
			return lt`<devtools-button
      @click=${this.#e}
...
(at([ut()], Tt.prototype, "disabled", void 0),
	(Tt = at([ht("devtools-recorder-selector-picker-button")], Tt)));
let Et = class extends pt {
	static styles = [tt];
	#ct = new Set();
	constructor() {
		(super(),
			(this.state = { type: a.Schema.StepType.WaitForElement }),
			(this.isTypeEditable = !0),
			(this.disabled = !1));
	}
	createRenderRoot() {
		const e = super.createRenderRoot();
		return (e.addEventListener("keydown", this.#pt), e);
	}
	set step(e) {
		((this.state = st(kt.fromStep(e))), (this.error = void 0));
	}
	#ht(e) {
		try {
			(this.dispatchEvent(new $t(kt.toStep(e))), (this.state = e));
		} catch (e) {
		...
		(e.preventDefault(),
			...
			this.#ht(
				nt(this.state, {
					...
					frame: e.data.frame,
					selectors: e.data.selectors.map((e) =>
						"string" == typeof e ? [e] : e
					),
	...
	#gt = (e, t, i) => (s) => {
		(s.preventDefault(),
			s.stopPropagation(),
			this.#ht(nt(this.state, e)),
			this.#mt(t),
			i && l.userMetrics.recordingEdited(i));
	};
	#pt = (e) => {
		if (
			(it(e instanceof KeyboardEvent),
			e.target instanceof y.SuggestionInput.SuggestionInput &&
				"Enter" === e.key)
		) {
			(e.preventDefault(), e.stopPropagation());
			const t = this.renderRoot.querySelectorAll("devtools-suggestion-input"),
				i = [...t].findIndex((t) => t === e.target);
			i >= 0 && i + 1 < t.length ? t[i + 1].focus() : e.target.blur();
		}
	};
	#vt = (e) => (t) => {
		if (
			(it(t.target instanceof y.SuggestionInput.SuggestionInput),
			t.target.disabled)
		)
			return;
		const i = bt[e.attribute],
			s = vt[i](t.target.value),
			r = e.from.bind(this)(s);
		r &&
			(this.#ht(nt(this.state, r)),
			e.metric && l.userMetrics.recordingEdited(e.metric));
	};
	#bt = async (e) => {
		if (
			(it(e.target instanceof y.SuggestionInput.SuggestionInput),
			e.target.disabled)
		)
			return;
		const t = e.target.value;
		t !== this.state.type &&
			(Object.values(a.Schema.StepType).includes(t)
				? (this.#ht(await kt.default(t)),
					l.userMetrics.recordingEdited(
						l.UserMetrics.RecordingEdited.TypeChanged
					))
				: (this.error = xt(wt.unknownActionType)));
	};
	#ft = async (e) => {
		(e.preventDefault(), e.stopPropagation());
		const t = e.target.dataset.attribute;
		(this.#ht(
			nt(this.state, { [t]: await kt.defaultByAttribute(this.state, t) })
		),
			this.#mt(`[data-attribute=${t}].attribute devtools-suggestion-input`));
	};
	#yt(e) {
		if (!this.disabled)
			return lt`
	...
	#wt(e) {
		...
		return [...yt[this.state.type].optional].includes(e) && !this.disabled
			? lt`<devtools-button
      .size=${"SMALL"}
      .iconName=${"bin"}
      .variant=${"secondary"}
      ...
      @click=${(t) => {
				(t.preventDefault(),
					t.stopPropagation(),
					this.#ht(nt(this.state, { [e]: void 0 })));
			}}
    ></devtools-button>`
			: void 0;
	}
	#St(e) {
		return (
			this.#ct.add("type"),
			lt`<div class="row attribute" data-attribute="type">
	...
	#xt(e) {
		this.#ct.add(e);
		const t = this.state[e]?.toString();
		if (void 0 !== t)
			return lt`<div class="row attribute" data-attribute=${e}>
        ...
        @blur=${this.#vt({
					...
					from(t) {
						if (void 0 !== this.state[e]) {
							if ("properties" === e)
								l.userMetrics.recordingAssertion(
									l.UserMetrics.RecordingAssertion.PropertyAssertionEdited
								);
							return { [e]: t };
						}
					},
					metric: l.UserMetrics.RecordingEdited.OtherEditing,
				})}
      ></devtools-suggestion-input>
      ${this.#wt(e)}
    </div>`;
	...
	#$t() {
		if ((this.#ct.add("frame"), void 0 !== this.state.frame))
			return lt`
            ...
            <div class="padded row">
              <devtools-suggestion-input
                ...
                @blur=${this.#vt({
									attribute: "frame",
									from(e) {
										if (void 0 !== this.state.frame?.[t])
											return { frame: new ot({ [t]: e }) };
									},
									metric: l.UserMetrics.RecordingEdited.OtherEditing,
								})}
              ></devtools-suggestion-input>
	...
	#kt() {
		if ((this.#ct.add("selectors"), void 0 !== this.state.selectors))
			return lt`<div class="attribute" data-attribute="selectors">
      <div class="row">
      ...
      ${this.state.selectors.map(
				(e, t, i) => lt`<div class="padded row" data-selector-path=${t}>
            ...
            >
              <devtools-suggestion-input
                ...
                @blur=${this.#vt({
									attribute: "selectors",
									from(e) {
										if (void 0 !== this.state.selectors?.[t]?.[i])
											return { selectors: new ot({ [t]: new ot({ [i]: e }) }) };
									},
									metric: l.UserMetrics.RecordingEdited.SelectorPartEdited,
								})}
              ></devtools-suggestion-input>
	...
	#Tt() {
		if ((this.#ct.add("assertedEvents"), void 0 !== this.state.assertedEvents))
			return lt`<div class="attribute" data-attribute="assertedEvents">
      <div class="row">
        <div>asserted events<span class="separator">:</span></div>
        ${this.#wt("assertedEvents")}
      </div>
      ${this.state.assertedEvents.map(
				(e, t) => lt` <div class="padded row">
            <div>type<span class="separator">:</span></div>
            <div>${e.type}</div>
          </div>
          <div class="padded row">
            ...
            <devtools-suggestion-input
              ...
              @blur=${this.#vt({
								...
								from(e) {
									if (void 0 !== this.state.assertedEvents?.[t]?.title)
										return { assertedEvents: new ot({ [t]: { title: e } }) };
								},
								metric: l.UserMetrics.RecordingEdited.OtherEditing,
							})}
            ></devtools-suggestion-input>
          ...
          <div class="padded row">
            ...
            <devtools-suggestion-input
              ...
              @blur=${this.#vt({
								attribute: "url",
								from(e) {
									if (void 0 !== this.state.assertedEvents?.[t]?.url)
										return { assertedEvents: new ot({ [t]: { url: e } }) };
								},
								metric: l.UserMetrics.RecordingEdited.OtherEditing,
							})}
            ></devtools-suggestion-input>
	...
	#Et() {
		if ((this.#ct.add("attributes"), void 0 !== this.state.attributes))
			return lt`<div class="attribute" data-attribute="attributes">
      <div class="row">
        <div>attributes<span class="separator">:</span></div>
        ${this.#wt("attributes")}
      </div>
      ${this.state.attributes.map(
				({ name: e, value: t }, i, s) => lt`<div class="padded row">
          <devtools-suggestion-input
            .disabled=${this.disabled}
            ...
            @blur=${this.#vt({
							...
							from(e) {
								if (void 0 !== this.state.attributes?.[i]?.name)
									return (
										l.userMetrics.recordingAssertion(
											l.UserMetrics.RecordingAssertion.AttributeAssertionEdited
										),
										{ attributes: new ot({ [i]: { name: e } }) }
									);
							},
							metric: l.UserMetrics.RecordingEdited.OtherEditing,
						})}
          ...
          <devtools-suggestion-input
            ...
            @blur=${this.#vt({
							attribute: "attributes",
							from(e) {
								if (void 0 !== this.state.attributes?.[i]?.value)
									return (
										l.userMetrics.recordingAssertion(
											l.UserMetrics.RecordingAssertion.AttributeAssertionEdited
										),
										{ attributes: new ot({ [i]: { value: e } }) }
									);
							},
							metric: l.UserMetrics.RecordingEdited.OtherEditing,
						})}
						...
						onClick: this.#gt(
							{
								attributes: new ot({
									[i + 1]: new rt(
										(() => {
											{
												const e = new Set(s.map(({ name: e }) => e)),
													t = ft.attributes[0];
												let i = t.name,
													r = 0;
												for (; e.has(i); ) (++r, (i = `${t.name}-${r}`));
												return { ...t, name: i };
											}
										})()
									),
							...
							`devtools-suggestion-input[data-path="attributes.${i + 1}.name"]`,
							l.UserMetrics.RecordingEdited.OtherEditing
						),
					})}
	...
	#Rt() {
		return [...yt[this.state.type].optional]
			.filter((e) => void 0 === this.state[e])
			.map(
				(e) => lt`<devtools-button
          .variant=${"secondary"}
          class="add-row"
          data-attribute=${e}
          @click=${this.#ft}
...
(at([gt()], Et.prototype, "state", void 0),
	at([gt()], Et.prototype, "error", void 0),
	at([ut()], Et.prototype, "isTypeEditable", void 0),
	at([ut()], Et.prototype, "disabled", void 0),
	(Et = at([ht("devtools-recorder-step-editor")], Et)));
...
export {
	I as ControlButton,
	j as CreateRecordingView,
	W as RecordingListView,
	Ke as RecordingView,
	ge as ReplayButton,
	oe as SelectButton,
	et as StartView,
