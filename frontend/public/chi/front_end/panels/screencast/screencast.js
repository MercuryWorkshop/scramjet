...
const a = ["left", "middle", "right", "back", "forward"],
	r = {
		mousedown: "mousePressed",
		mouseup: "mouseReleased",
		mousemove: "mouseMoved",
	};
class l extends e.SDKModel.SDKModel {
	inputAgent;
	...
	emitKeyEvent(e) {
		...
		const i = "keypress" === e.type ? String.fromCharCode(e.charCode) : void 0;
		this.inputAgent.invoke_dispatchKeyEvent({
			...
			autoRepeat: e.repeat,
			isKeypad: 3 === e.location,
			isSystemKey: !1,
			location: 3 !== e.location ? e.location : void 0,
		});
	}
	emitMouseEvent(e, t, i) {
		e.type in r &&
			(("mousedown" !== e.type && null !== this.activeMouseOffsetTop) ||
			...
			"mouseup" === e.type && (this.activeMouseOffsetTop = null));
	}
	emitWheelEvent(e, t, i) {
		(null === this.activeMouseOffsetTop && (this.activeMouseOffsetTop = t),
			this.inputAgent.invoke_dispatchMouseEvent({
				type: "mouseWheel",
				x: Math.round(e.offsetX / i),
				y: Math.round(e.offsetY / i - this.activeMouseOffsetTop),
...
e.SDKModel.SDKModel.register(l, {
	capabilities: e.Target.Capability.Input,
	autostart: !1,
});
var h = Object.freeze({ __proto__: null, InputModel: l });
...
const c = {
	...
	g = i.i18n.registerUIStrings("panels/screencast/ScreencastView.ts", c),
	u = i.i18n.getLocalizedString.bind(void 0, g);
class m extends s.Widget.VBox {
	screenCaptureModel;
	domModel;
	overlayModel;
	resourceTreeModel;
	...
	constructor(t) {
		(super(),
			...
			(this.inputModel = t.target().model(l)),
	...
	initialize() {
		(this.element.classList.add("screencast"),
			...
			s.ARIAUtils.setLabel(
				this.canvasElement,
				u(c.screencastViewOfDebugTarget)
			),
	...
	wasShown() {
		(this.startCasting(), this.registerCSSFiles([d]));
	}
	willHide() {
		this.stopCasting();
	}
	...
	updateGlasspane() {
		this.targetInactive
			? ((this.glassPaneElement.textContent = u(c.theTabIsInactive)),
				this.glassPaneElement.classList.remove("hidden"))
			: e.TargetManager.TargetManager.instance().allTargetsSuspended()
				? ((this.glassPaneElement.textContent = u(c.profilingInProgress)),
					this.glassPaneElement.classList.remove("hidden"))
	...
	drawElementTitle() {
		...
		(d + s > e && (d = e - s - 2),
			...
			l &&
				(this.context.lineTo(d + 14, r),
				this.context.lineTo(d + 21, r - 7),
				this.context.lineTo(d + 28, r)),
			this.context.lineTo(d + s, r),
			this.context.lineTo(d + s, r + n),
			h &&
				(this.context.lineTo(d + 28, r + n),
				this.context.lineTo(d + 21, r + n + 7),
				this.context.lineTo(d + 14, r + n)),
			this.context.lineTo(d, r + n),
			this.context.closePath(),
	...
	viewportDimensions() {
		...
		return {
			...
			height: this.element.offsetHeight - e - 30 - v,
		};
	}
	setInspectMode(e, t) {
		return (
			(this.inspectModeConfig = "none" !== e ? t : null),
			Promise.resolve()
		);
	}
	highlightFrame(e) {}
	...
	createNavigationBar() {
		...
		this.navigationBack.appendChild(new o.Icon.Icon()).data = {
			color: "var(--icon-default)",
			iconName: "arrow-back",
		};
		((this.navigationBack.disabled = !0),
			s.ARIAUtils.setLabel(this.navigationBack, u(c.back)),
			(this.navigationForward = this.navigationBar.createChild(
				"button",
				"navigation"
			)));
		this.navigationForward.appendChild(new o.Icon.Icon()).data = {
			color: "var(--icon-default)",
			iconName: "arrow-forward",
		};
		((this.navigationForward.disabled = !0),
			s.ARIAUtils.setLabel(this.navigationForward, u(c.forward)),
			(this.navigationReload = this.navigationBar.createChild(
				"button",
				"navigation"
			)));
		this.navigationReload.appendChild(new o.Icon.Icon()).data = {
			color: "var(--icon-default)",
			iconName: "refresh",
		};
		(s.ARIAUtils.setLabel(this.navigationReload, u(c.reload)),
			(this.navigationUrl = this.navigationBar.appendChild(
				s.UIUtils.createInput()
			)),
			(this.navigationUrl.type = "text"),
			s.ARIAUtils.setLabel(this.navigationUrl, u(c.addressBar)),
			...
			s.ARIAUtils.setLabel(this.mouseInputToggle, u(c.mouseInput)),
			...
			s.ARIAUtils.setLabel(this.touchInputToggle, u(c.touchInput)),
			(this.navigationProgressBar = new b(
	...
	#e(t) {
		if (
			...
			return;
		const i = e.TargetManager.TargetManager.instance().models(
			e.EmulationModel.EmulationModel
		);
		for (const e of i) e.overrideEmulateTouch(t);
		((this.mouseInputToggle.disabled = !t),
			(this.touchInputToggle.disabled = t),
			(this.mouseInputToggleIcon.data = {
				...this.mouseInputToggleIcon.data,
			...
			this.canvasContainerElement.classList.toggle("touchable", t));
	}
...
const M = { toggleScreencast: "Toggle screencast" },
	w = i.i18n.registerUIStrings("panels/screencast/ScreencastApp.ts", M),
	T = i.i18n.getLocalizedString.bind(void 0, w);
let C, x, I;
class S {
	enabledSetting;
	toggleButton;
	...
	presentUI(e) {
		((this.rootView = new s.RootView.RootView()),
...
class k {
	static instance(e = { forceNew: !1 }) {
		const { forceNew: t } = e;
		return ((x && !t) || (x = new k()), x);
	}
	item() {
		return S.instance().toggleButton;
	}
}
class R {
	static instance(e = { forceNew: !1 }) {
		const { forceNew: t } = e;
		return ((I && !t) || (I = new R()), I);
	}
	createApp() {
		return S.instance();
	}
...
var P = Object.freeze({
	__proto__: null,
	ScreencastApp: S,
	ToolbarButtonProvider: k,
	ScreencastAppProvider: R,
});
export { h as InputModel, P as ScreencastApp, y as ScreencastView };
