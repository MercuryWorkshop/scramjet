...
import * as t from "../../../../ui/components/buttons/buttons.js";
import * as a from "../../../../ui/components/helpers/helpers.js";
import * as n from "../../../../ui/components/icon_button/icon_button.js";
import * as r from "../../../../ui/components/input/input.js";
import * as i from "../../../../ui/lit-html/lit-html.js";
...
const d = {
	...
	h = e.i18n.getLocalizedString.bind(void 0, c);
...
class p extends Event {
	...
	detail;
	constructor(e) {
		(super(p.eventName), (this.detail = { value: e }));
	}
}
const m = {
...
class b extends HTMLElement {
	static litTagName = i.literal`devtools-user-agent-client-hints-form`;
	...
	#s = "";
	connectedCallback() {
		this.#e.adoptedStyleSheets = [r.checkboxStyles, o];
	}
	set value(e) {
		const {
			metaData: t = m,
			showMobileCheckbox: a = !1,
			showSubmitButton: n = !1,
		} = e;
		((this.#n = { ...this.#n, ...t }), (this.#r = a), (this.#i = n), this.#l());
	}
	get value() {
		return { metaData: this.#n };
	}
	set disabled(e) {
		((this.#a = e), (this.#t = !1), this.#l());
	}
	get disabled() {
		return this.#a;
	}
	#o = (e) => {
		("Space" !== e.code &&
			...
			"ArrowLeft" !== e.code &&
			"ArrowRight" !== e.code) ||
			(e.stopPropagation(), this.#d(e.code));
	};
	#d = (e) => {
		this.#a ||
			("ArrowLeft" === e && !this.#t) ||
	...
	#c = (e, t, a) => {
		const n = this.#n.brands?.map((n, r) => {
			if (r === t) {
				const { brand: t, version: r } = n;
				return "brandName" === a
					? { brand: e, version: r }
					: { brand: t, version: e };
			}
		...
		((this.#n = { ...this.#n, brands: n }),
			this.dispatchEvent(new u()),
			this.#l());
	};
	#h = (e, t, a) => {
		const n = this.#n.fullVersionList?.map((n, r) => {
			if (r === t) {
				const { brand: t, version: r } = n;
				return "brandName" === a
					? { brand: e, version: r }
		...
		((this.#n = { ...this.#n, fullVersionList: n }),
			this.dispatchEvent(new u()),
			this.#l());
	};
	#u = (e) => {
		const { brands: t = [] } = this.#n;
		(t.splice(e, 1),
			(this.#n = { ...this.#n, brands: t }),
			this.dispatchEvent(new u()),
			(this.#s = h(d.deletedBrand)),
			this.#l());
		let a = this.shadowRoot?.getElementById(`ua-brand-${e + 1}-input`);
		(a || (a = this.shadowRoot?.getElementById("add-brand-button")),
			a?.focus());
	};
	#p = (e) => {
		const { fullVersionList: t = [] } = this.#n;
		(t.splice(e, 1),
			(this.#n = { ...this.#n, fullVersionList: t }),
			this.dispatchEvent(new u()),
			(this.#s = h(d.deletedBrand)),
			this.#l());
		let a = this.shadowRoot?.getElementById(`fvl-brand-${e + 1}-input`);
		(a || (a = this.shadowRoot?.getElementById("add-fvl-brand-button")),
			a?.focus());
	};
	#m = () => {
		const { brands: e } = this.#n;
		((this.#n = {
			...this.#n,
			brands: [...(Array.isArray(e) ? e : []), { brand: "", version: "" }],
		}),
			this.dispatchEvent(new u()),
			(this.#s = h(d.addedBrand)),
			this.#l());
		const t = this.shadowRoot?.querySelectorAll(".ua-brand-name-input");
		if (t) {
			const e = Array.from(t).pop();
			e && e.focus();
		}
	};
	#b = (e) => {
	...
	#g = () => {
		const { fullVersionList: e } = this.#n;
		((this.#n = {
			...this.#n,
			fullVersionList: [
				...(Array.isArray(e) ? e : []),
			...
			(this.#s = h(d.addedBrand)),
			this.#l());
		const t = this.shadowRoot?.querySelectorAll(".fvl-brand-name-input");
		if (t) {
			const e = Array.from(t).pop();
			e && e.focus();
		}
	...
	#v = (e, t) => {
		(e in this.#n && ((this.#n = { ...this.#n, [e]: t }), this.#l()),
			this.dispatchEvent(new u()));
	};
	#$ = (e) => {
		("Space" !== e.code && "Enter" !== e.code) ||
	...
	#w = (e) => {
		(e.preventDefault(),
			this.#i && (this.dispatchEvent(new p(this.#n)), this.#l()));
	};
	#x(e, t, a, n) {
		return i.html`
	...
	#k() {
		const { platform: e, platformVersion: t } = this.#n;
		return i.html`
	...
	#D() {
		const { brands: e = [{ brand: "", version: "" }] } = this.#n,
			t = e.map((e, t) => {
				const { brand: a, version: r } = e,
					l = () => {
						this.#u(t);
					};
				return i.html`
	...
	#A() {
		const { fullVersionList: e = [{ brand: "", version: "" }] } = this.#n,
			t = e.map((e, t) => {
				const { brand: a, version: r } = e,
					l = () => {
						this.#p(t);
	...
	#l() {
		const { fullVersion: e, architecture: a } = this.#n,
			r = this.#D(),
			l = this.#A(),
			o = this.#x(
				h(d.fullBrowserVersion),
				h(d.fullBrowserVersionPlaceholder),
				e || "",
				"fullVersion"
			),
			c = this.#k(),
    ...
    `;
		i.render(b, this.#e, { host: this });
	}
	validate = () => {
		for (const [e, t] of Object.entries(this.#n))
			if ("brands" === e || "fullVersionList" === e) {
				const e = this.#n.brands?.every(({ brand: e, version: t }) => {
					const a = l.UserAgentMetadata.validateAsStructuredHeadersString(
							e,
							h(d.notRepresentable)
						),
						n = l.UserAgentMetadata.validateAsStructuredHeadersString(
							t,
							h(d.notRepresentable)
						);
					return a.valid && n.valid;
				});
				if (!e) return { valid: !1, errorMessage: h(d.notRepresentable) };
			} else {
				const e = l.UserAgentMetadata.validateAsStructuredHeadersString(
					t,
					h(d.notRepresentable)
				);
				if (!e.valid) return e;
...
a.CustomElements.defineComponent("devtools-user-agent-client-hints-form", b);
var g = Object.freeze({
	__proto__: null,
	ClientHintsChangeEvent: u,
	ClientHintsSubmitEvent: p,
	UserAgentClientHintsForm: b,
});
export { g as UserAgentClientHintsForm };
