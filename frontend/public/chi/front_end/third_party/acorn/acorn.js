				...
				(!(i < 65) &&
					(i < 91 ||
						(i < 97
							...
							: i < 123 ||
								(i <= 65535
									? i >= 170 && h.test(String.fromCharCode(i))
									: !1 !== s && (p(i, e) || p(i, t))))));
}
var u = function (t, e) {
	(void 0 === e && (e = {}),
...
var f = { beforeExpr: !0 },
	...
	x = {};
function g(t, e) {
	return (void 0 === e && (e = {}), (e.keyword = t), (x[t] = new u(t, e)));
}
var v = {
		num: new u("num", m),
...
function M(t) {
	var e = {};
	for (var i in O) e[i] = t && A(t, i) ? t[i] : O[i];
	if (
		...
		(t && null != t.allowHashBang) || (e.allowHashBang = e.ecmaVersion >= 14),
		P(e.onToken))
	) {
		var s = e.onToken;
		...
		P(e.onComment) &&
			(e.onComment = (function (t, e) {
				return function (i, s, r, a, n, o) {
					...
					(t.locations && (h.loc = new R(this, n, o)),
...
var q = function (t, e, i) {
		((this.options = t = M(t)),
			...
			(this.keywords = N(
				a[t.ecmaVersion >= 6 ? 6 : "module" === t.sourceType ? "5module" : 5]
			)));
		var r = "";
		(!0 !== t.allowReserved &&
			((r = s[t.ecmaVersion >= 6 ? 6 : 5 === t.ecmaVersion ? 5 : 3]),
			"module" === t.sourceType && (r += " await")),
			(this.reservedWords = N(r)));
		var n = (r ? r + " " : "") + s.strict;
		((this.reservedWordsStrict = N(n)),
			(this.reservedWordsStrictBind = N(n + " " + s.strictBind)),
			(this.input = String(e)),
			(this.containsEsc = !1),
			i
				? ((this.pos = i),
					(this.lineStart = this.input.lastIndexOf("\n", i - 1) + 1),
					(this.curLine = this.input.slice(0, this.lineStart).split(y).length))
...
((q.prototype.parse = function () {
	...
	(j.canAwait.get = function () {
		for (var t = this.scopeStack.length - 1; t >= 0; t--) {
			...
			if (e.inClassFieldInit || e.flags & F) return !1;
...
((z.isLet = function (t) {
	...
	if (c(s, !0)) {
		for (var r = i + 1; l((s = this.input.charCodeAt(r)), !0); ) ++r;
	...
	(z.isAsyncFunction = function () {
		...
		var t,
			...
			i = this.pos + e[0].length;
		return !(
			y.test(this.input.slice(this.pos, i)) ||
			"function" !== this.input.slice(i, i + 8) ||
			(i + 8 !== this.input.length &&
				(l((t = this.input.charCodeAt(i + 8))) || (t > 55295 && t < 56320)))
		);
	}),
	(z.parseStatement = function (t, e, i) {
		var s,
			r = this.type,
			a = this.startNode();
		switch ((this.isLet(t) && ((r = v._var), (s = "let")), r)) {
			case v._break:
			case v._continue:
			...
			case v._import:
				...
				return (
					...
					r === v._import ? this.parseImport(a) : this.parseExport(a, i)
				);
			default:
				...
				return r === v.name && "Identifier" === c.type && this.eat(v.colon)
	...
	(z.parseIfStatement = function (t) {
		return (
			...
			(t.alternate = this.eat(v._else) ? this.parseStatement("if") : null),
	...
	(z.parseSwitchStatement = function (t) {
		...
		(this.next(),
			...
			this.labels.push(K),
			this.enterScope(0));
		for (var i = !1; this.type !== v.braceR; )
			if (this.type === v._case || this.type === v._default) {
				var s = this.type === v._case;
...
((z.parseCatchClauseParam = function () {
	...
	(z.parseTryStatement = function (t) {
		...
		return (
			(t.finalizer = this.eat(v._finally) ? this.parseBlock() : null),
			t.handler ||
				t.finalizer ||
	...
	(z.parseFor = function (t, e) {
		return (
			(t.init = e),
			this.expect(v.semi),
			(t.test = this.type === v.semi ? null : this.parseExpression()),
			this.expect(v.semi),
			(t.update = this.type === v.parenR ? null : this.parseExpression()),
			this.expect(v.parenR),
				...
				this.eat(v.eq)
					...
					: s ||
						...
						? s ||
							...
							(e && (this.type === v._in || this.isContextual("of")))
							? (r.init = null)
							: this.raise(
									this.lastTokEnd,
...
((z.parseFunction = function (t, e, i, s, r) {
	(this.initFunction(t),
		(this.options.ecmaVersion >= 9 || (this.options.ecmaVersion >= 6 && !s)) &&
			(this.type === v.star && e & Z && this.unexpected(),
			(t.generator = this.eat(v.star))),
		this.options.ecmaVersion >= 8 && (t.async = !!s),
		e & X &&
			((t.id = 4 & e && this.type !== v.name ? null : this.parseIdent()),
			!t.id ||
				e & Z ||
	...
	return (
		...
		e & X || (t.id = this.type === v.name ? this.parseIdent() : null),
		this.parseFunctionParams(t),
		this.parseFunctionBody(t, i, !1, r),
		(this.yieldPos = a),
		(this.awaitPos = n),
		(this.awaitIdentPos = o),
		this.finishNode(t, e & X ? "FunctionDeclaration" : "FunctionExpression")
	);
}),
	...
	(z.parseClassElement = function (t) {
		...
		if (
			...
			e < 13 || this.type === v.parenL || "method" !== n || r || a)
		) {
			var p = !i.static && J(i, "constructor"),
				c = p && t;
			(p &&
				"method" !== n &&
	...
	(z.parseClassSuper = function (t) {
		t.superClass = this.eat(v._extends)
	...
	(z.checkExport = function (t, e, i) {
		t &&
			("string" != typeof e && (e = "Identifier" === e.type ? e.name : e.value),
			A(t, e) && this.raiseRecoverable(i, "Duplicate export '" + e + "'"),
			(t[e] = !0));
	}),
	...
	(z.parseImport = function (t) {
		return (
			this.next(),
			this.type === v.string
				? ((t.specifiers = Y), (t.source = this.parseExprAtom()))
				: ((t.specifiers = this.parseImportSpecifiers()),
					this.expectContextual("from"),
					(t.source =
						this.type === v.string ? this.parseExprAtom() : this.unexpected())),
			this.semicolon(),
	...
	(z.parseImportSpecifiers = function () {
		...
		if (
			this.type === v.name &&
			(t.push(this.parseImportDefaultSpecifier()), !this.eat(v.comma))
		)
			return t;
		if (this.type === v.star)
...
((tt.toAssignable = function (t, e, i) {
		...
		if (this.options.ecmaVersion >= 6)
			switch (this.type) {
				case v.bracketL:
					...
					return (
						this.next(),
						(t.elements = this.parseBindingList(v.bracketR, !0, !0)),
	...
	(tt.parseBindingList = function (t, e, i, s) {
		for (var r = [], a = !0; !this.eat(t); )
			if ((a ? (a = !1) : this.expect(v.comma), e && this.type === v.comma))
				r.push(null);
			else {
	...
	(tt.parseMaybeDefault = function (t, e, i) {
		if (
			...
			this.options.ecmaVersion < 6 || !this.eat(v.eq))
		)
			return i;
	...
	(tt.checkLValSimple = function (t, e, i) {
		...
		switch (t.type) {
			...
			case "ParenthesizedExpression":
				return (
					s &&
						this.raiseRecoverable(t.start, "Binding parenthesized expression"),
...
((st.initialContext = function () {
	...
	(st.braceIsBlock = function (t) {
		...
		return (
			e === it.f_expr ||
			e === it.f_stat ||
			(t !== v.colon || (e !== it.b_stat && e !== it.b_expr)
				? t === v._return || (t === v.name && this.exprAllowed)
					? y.test(this.input.slice(this.lastTokEnd, this.start))
					: t === v._else ||
						t === v.semi ||
						t === v.eof ||
	...
	(v._function.updateContext = v._class.updateContext =
		function (t) {
			(!t.beforeExpr ||
			t === v._else ||
			(t === v.semi && this.curContext() !== it.p_stat) ||
			(t === v._return &&
				y.test(this.input.slice(this.lastTokEnd, this.start))) ||
			((t === v.colon || t === v.braceL) && this.curContext() === it.b_stat)
...
function at(t) {
	return (
		...
		("ChainExpression" === t.type && at(t.expression))
	);
}
((rt.checkPropClash = function (t, e, i) {
	...
	(rt.parseMaybeAssign = function (t, e, i) {
		...
		if ((i && (p = i.call(this, p, o, h)), this.type.isAssign)) {
			...
			return (
				...
				this.type === v.eq && (p = this.toAssignable(p, !1, e)),
				s || (e.parenthesizedAssign = e.trailingComma = e.doubleProto = -1),
				e.shorthandAssign >= p.start && (e.shorthandAssign = -1),
				this.type === v.eq ? this.checkLValPattern(p) : this.checkLValSimple(p),
				(c.left = p),
				this.next(),
	...
	(rt.parseExprOp = function (t, e, i, s, r) {
		...
		if (null != a && (!r || this.type !== v._in) && a > s) {
	...
	(rt.buildBinary = function (t, e, i, s, r, a) {
	...
	(rt.parseMaybeUnary = function (t, e, i, s) {
		var r,
			a = this.start,
			n = this.startLoc;
		if (this.isContextual("await") && this.canAwait)
			...
			((o.operator = this.value),
				...
				h
					...
					: this.strict &&
						...
						: "delete" === o.operator && at(o.argument)
							? this.raiseRecoverable(
									o.start,
			...
			if (((r = this.parseExprSubscripts(t, s)), this.checkExpressionErrors(t)))
				return r;
	...
	(rt.parseSubscripts = function (t, e, i, s, r) {
		for (
			var a =
					this.options.ecmaVersion >= 8 &&
					"Identifier" === t.type &&
	...
	(rt.parseSubscriptAsyncArrow = function (t, e, i, s) {
		return this.parseArrowExpression(this.startNodeAt(t, e), i, !0, s);
	}),
	(rt.parseSubscript = function (t, e, i, s, r, a, n) {
		...
		if (
			p ||
			(h && this.type !== v.parenL && this.type !== v.backQuote) ||
			this.eat(v.dot)
		) {
			...
			((c.object = t),
				p
					...
					: this.type === v.privateId && "Super" !== t.type
			...
			if (r && !h && this.shouldParseAsyncArrow())
				return (
					...
					this.parseSubscriptAsyncArrow(e, i, m, n)
				);
	...
	(rt.parseExprAtom = function (t, e, i) {
		...
		switch (this.type) {
			...
			case v.name:
				...
				if (
					...
					return (
						this.overrideContext(it.f_expr),
				...
				if (r && !this.canInsertSemicolon()) {
					...
					if (
						...
						return (
							(h = this.parseIdent(!1)),
							(!this.canInsertSemicolon() && this.eat(v.arrow)) ||
								this.unexpected(),
							this.parseArrowExpression(this.startNodeAt(a, n), [h], !0, e)
						);
				}
				return h;
			case v.regexp:
			...
			case v.bracketL:
				return (
					(s = this.startNode()),
					this.next(),
					(s.elements = this.parseExprList(v.bracketR, !0, !0, t)),
	...
	(rt.parseExprImport = function (t) {
		...
		return this.type !== v.parenL || t
			? this.type === v.dot
				? ((e.meta = i), this.parseImportMeta(e))
				: void this.unexpected()
			: this.parseDynamicImport(e);
	}),
	(rt.parseDynamicImport = function (t) {
		if (
			(this.next(), (t.source = this.parseMaybeAssign()), !this.eat(v.parenR))
		) {
			var e = this.start;
	...
	(rt.parseParenAndDistinguishExpression = function (t, e) {
		...
		if (this.options.ecmaVersion >= 6) {
			...
			for (this.yieldPos = 0, this.awaitPos = 0; this.type !== v.parenR; ) {
				if (
					(c ? (c = !1) : this.expect(v.comma),
					a && this.afterTrailingComma(v.parenR, !0))
				) {
					l = !0;
					break;
				}
			...
			if (
				(this.expect(v.parenR),
				t && this.shouldParseArrow(p) && this.eat(v.arrow))
			)
				return (
			...
			((p.length && !l) || this.unexpected(this.lastTokStart),
				...
				p.length > 1
					? (((i = this.startNodeAt(o, h)).expressions = p),
						this.finishNodeAt(i, "SequenceExpression", m, x))
					: (i = p[0]));
	...
	(rt.parseParenArrowList = function (t, e, i, s) {
...
((rt.parseNew = function () {
	...
	(rt.parsePropertyValue = function (t, e, i, s, r, a, n, o) {
		((i || s) && this.type === v.colon && this.unexpected(),
			this.eat(v.colon)
				? ((t.value = e
	...
	(rt.parseArrowExpression = function (t, e, i, s) {
		var r = this.yieldPos,
			a = this.awaitPos,
			n = this.awaitIdentPos;
		return (
	...
	(rt.parseFunctionBody = function (t, e, i, s) {
	...
	(rt.parsePrivateIdent = function () {
		var t = this.startNode();
		return (
			this.type === v.privateId ? (t.name = this.value) : this.unexpected(),
			this.next(),
...
var dt =
	...
	mt = ft + " EBase EComp EMod EPres ExtPict",
	xt = { 9: dt, 10: ft, 11: ft, 12: mt, 13: mt, 14: mt },
	gt = {
		9: "",
		10: "",
		11: "",
	...
	St = kt + " Cypro_Minoan Cpmn Old_Uyghur Ougr Tangsa Tnsa Toto Vithkuqi Vith",
	Ct = {
		9: yt,
		10: bt,
		11: _t,
		12: kt,
		13: St,
		14: St + " Hrkt Katakana_Or_Hiragana Kawi Nag_Mundari Nagm Unknown Zzzz",
	},
	wt = {};
function Et(t) {
	var e = (wt[t] = {
		binary: N(xt[t] + " " + vt),
		binaryOfStrings: N(gt[t]),
		nonBinary: { General_Category: N(vt), Script: N(Ct[t]) },
	});
...
for (var It = 0, At = [9, 10, 11, 12, 13, 14]; It < At.length; It += 1) {
	Et(At[It]);
}
var Pt = q.prototype,
	Nt = function (t) {
		((this.parser = t),
			...
			(this.unicodeProperties =
				wt[t.options.ecmaVersion >= 14 ? 14 : t.options.ecmaVersion]),
			(this.source = ""),
			(this.flags = ""),
			(this.start = 0),
			(this.switchU = !1),
...
((Nt.prototype.reset = function (t, e, i) {
	...
	(Nt.prototype.raise = function (t) {
		this.parser.raiseRecoverable(
			this.start,
			"Invalid regular expression: /" + this.source + "/: " + t
		);
	}),
	(Nt.prototype.at = function (t, e) {
		void 0 === e && (e = !1);
		var i = this.source,
			s = i.length;
		if (t >= s) return -1;
	...
	(Nt.prototype.nextIndex = function (t, e) {
		void 0 === e && (e = !1);
		var i = this.source,
			s = i.length;
		if (t >= s) return s;
	...
	(Nt.prototype.current = function (t) {
	...
	(Nt.prototype.eat = function (t, e) {
		return (
			void 0 === e && (e = !1),
			this.current(e) === t && (this.advance(e), !0)
		);
	}),
	(Nt.prototype.eatChars = function (t, e) {
		void 0 === e && (e = !1);
	...
	(Pt.regexp_eatRegExpIdentifierName = function (t) {
		if (((t.lastStringValue = ""), this.regexp_eatRegExpIdentifierStart(t))) {
			for (
				t.lastStringValue += V(t.lastIntValue);
			...
			if (this.regexp_eatFixedHexDigits(t, 4)) {
				...
				if (r && a >= 55296 && a <= 56319) {
					...
					if (t.eat(92) && t.eat(117) && this.regexp_eatFixedHexDigits(t, 4)) {
						...
						if (o >= 56320 && o <= 57343)
							return (
								(t.lastIntValue = 1024 * (a - 55296) + (o - 56320) + 65536),
								!0
							);
					}
...
((Pt.regexp_eatCharacterClassEscape = function (t) {
	...
	if (
		(function (t) {
			return (
				100 === t || 68 === t || 115 === t || 83 === t || 119 === t || 87 === t
			);
		})(e)
	)
	...
	(Pt.regexp_validateUnicodePropertyNameAndValue = function (t, e, i) {
		(A(t.unicodeProperties.nonBinary, e) || t.raise("Invalid property name"),
	...
	(Pt.regexp_eatUnicodePropertyName = function (t) {
		...
		for (t.lastStringValue = ""; Lt((e = t.current())); )
			((t.lastStringValue += V(e)), t.advance());
	...
	(Pt.regexp_eatClassAtom = function (t) {
		...
		if (t.eat(92)) {
			...
			if (t.switchU) {
				var i = t.current();
				((99 === i || Mt(i)) && t.raise("Invalid class escape"),
					t.raise("Invalid escape"));
			}
			t.pos = e;
		}
	...
	(Pt.regexp_eatClassControlLetter = function (t) {
		var e = t.current();
		return (
			!(!Dt(e) && 95 !== e) && ((t.lastIntValue = e % 32), t.advance(), !0)
		);
	}),
	...
	(Pt.regexp_eatHexDigits = function (t) {
		...
		for (t.lastIntValue = 0; Ot((i = t.current())); )
			((t.lastIntValue = 16 * t.lastIntValue + Bt(i)), t.advance());
		return t.pos !== e;
	}),
	...
	(Pt.regexp_eatFixedHexDigits = function (t, e) {
		...
		for (var s = 0; s < e; ++s) {
			...
			((t.lastIntValue = 16 * t.lastIntValue + Bt(r)), t.advance());
		}
		return !0;
	}));
var Ft = function (t) {
...
((Ut.next = function (t) {
	...
	(Ut.getToken = function () {
		return (this.next(), new Ft(this));
	}),
	"undefined" != typeof Symbol &&
		(Ut[Symbol.iterator] = function () {
			var t = this;
			return {
				next: function () {
					var e = t.getToken();
					return { done: e.type === v.eof, value: e };
				},
			};
		}),
	(Ut.nextToken = function () {
	...
	(Ut.readToken = function (t) {
		return c(t, this.options.ecmaVersion >= 6) || 92 === t
			? this.readWord()
			: this.getTokenFromCode(t);
	}),
	(Ut.fullCharCodeAtPos = function () {
		var t = this.input.charCodeAt(this.pos);
		if (t <= 55295 || t >= 56320) return t;
		var e = this.input.charCodeAt(this.pos + 1);
		return e <= 56319 || e >= 57344 ? t : (t << 10) + e - 56613888;
	}),
	(Ut.skipBlockComment = function () {
		...
		if (
			...
			for (var s = void 0, r = e; (s = k(this.input, r, this.pos)) > -1; )
	...
	(Ut.readToken_mult_modulo_exp = function (t) {
		var e = this.input.charCodeAt(this.pos + 1),
			i = 1,
			s = 42 === t ? v.star : v.modulo;
		return (
			this.options.ecmaVersion >= 7 &&
				42 === t &&
				42 === e &&
				(++i, (s = v.starstar), (e = this.input.charCodeAt(this.pos + 2))),
			61 === e ? this.finishOp(v.assign, i + 1) : this.finishOp(s, i)
		);
	}),
	(Ut.readToken_pipe_amp = function (t) {
		var e = this.input.charCodeAt(this.pos + 1);
		if (e === t) {
			if (this.options.ecmaVersion >= 12)
				if (61 === this.input.charCodeAt(this.pos + 2))
					return this.finishOp(v.assign, 3);
			return this.finishOp(124 === t ? v.logicalOR : v.logicalAND, 2);
		}
		return 61 === e
			? this.finishOp(v.assign, 2)
			: this.finishOp(124 === t ? v.bitwiseOR : v.bitwiseAND, 1);
	}),
	(Ut.readToken_caret = function () {
		return 61 === this.input.charCodeAt(this.pos + 1)
			? this.finishOp(v.assign, 2)
			: this.finishOp(v.bitwiseXOR, 1);
	}),
	(Ut.readToken_plus_min = function (t) {
		var e = this.input.charCodeAt(this.pos + 1);
		return e === t
			? 45 !== e ||
				this.inModule ||
	...
	(Ut.readToken_eq_excl = function (t) {
		...
		return 61 === e
			...
			: 61 === t && 62 === e && this.options.ecmaVersion >= 6
				...
				: this.finishOp(61 === t ? v.eq : v.prefix, 1);
	}),
	(Ut.readToken_question = function () {
		var t = this.options.ecmaVersion;
		if (t >= 11) {
			var e = this.input.charCodeAt(this.pos + 1);
			if (46 === e) {
				var i = this.input.charCodeAt(this.pos + 2);
				if (i < 48 || i > 57) return this.finishOp(v.questionDot, 2);
			}
			if (63 === e) {
				if (t >= 12)
					if (61 === this.input.charCodeAt(this.pos + 2))
	...
	(Ut.readRegexp = function () {
		...
		return this.finishToken(v.regexp, { pattern: r, flags: n, value: h });
	}),
	(Ut.readInt = function (t, e, i) {
		for (
	...
	(Ut.readRadixNumber = function (t) {
		...
		return (
			...
			this.options.ecmaVersion >= 11 && 110 === this.input.charCodeAt(this.pos)
				? ((i = qt(this.input.slice(e, this.pos))), ++this.pos)
				: c(this.fullCharCodeAtPos()) &&
					this.raise(this.pos, "Identifier directly after number"),
			this.finishToken(v.num, i)
		);
	}),
	(Ut.readNumber = function (t) {
		var e = this.pos;
		t ||
		...
		if (!i && !t && this.options.ecmaVersion >= 11 && 110 === s) {
			var r = qt(this.input.slice(e, this.pos));
			return (
				++this.pos,
				c(this.fullCharCodeAtPos()) &&
					this.raise(this.pos, "Identifier directly after number"),
			...
			92 === s
				...
				: 8232 === s || 8233 === s
					? (this.options.ecmaVersion < 10 &&
						...
						this.options.locations &&
							(this.curLine++, (this.lineStart = this.pos)))
					: (_(s) && this.raise(this.start, "Unterminated string constant"),
						++this.pos);
		}
		return (
			(e += this.input.slice(i, this.pos++)),
			this.finishToken(v.string, e)
		);
	}));
var jt = {};
((Ut.tryReadTemplateToken = function () {
	...
	(Ut.invalidStringToken = function (t, e) {
		if (this.inTemplateElement && this.options.ecmaVersion >= 9) throw jt;
		this.raise(t, e);
	}),
	(Ut.readTmplToken = function () {
	...
	(Ut.readInvalidTemplateToken = function () {
	...
	(Ut.readHexChar = function (t) {
		var e = this.pos,
			i = this.readInt(16, t);
		return (
			null === i && this.invalidStringToken(e, "Bad character escape sequence"),
			i
		);
	}),
	(Ut.readWord1 = function () {
		this.containsEsc = !1;
		for (
