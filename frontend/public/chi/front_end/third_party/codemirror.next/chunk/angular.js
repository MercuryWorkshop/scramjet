...
const S = Q(39, 33, !1),
	...
	R = e.parser.configure({ top: "SingleExpression" }),
	$ = P.configure({
		props: [
			O({
				Text: r.content,
	...
	b = {
		parser: $.configure({
			wrap: t((O, r) => ("InterpolationContent" == O.name ? g : null)),
		}),
	},
	m = {
		parser: $.configure({
...
function C(O = {}) {
	let r = v;
	if (O.base) {
		if ("html" != O.base.language.name || !(O.base.language instanceof n))
			throw new RangeError(
				"The base option must be the result of calling html(...)"
			);
		r = O.base;
	}
	return new a(r.language == v.language ? d : W(r.language), [
		r.support,
		r.language.data.of({
