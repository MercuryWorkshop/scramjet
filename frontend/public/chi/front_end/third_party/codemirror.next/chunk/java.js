...
const l = O({
		...
		"requires exports opens uses provides public private protected static transitive abstract final strictfp synchronized native transient volatile throws":
			Q.modifier,
		IntegerLiteral: Q.integer,
		FloatingPointLiteral: Q.float,
		"StringLiteral TextBlock": Q.string,
		CharacterLiteral: Q.character,
		LineComment: Q.lineComment,
		BlockComment: Q.blockComment,
		BooleanLiteral: Q.bool,
		PrimitiveType: Q.standard(Q.typeName),
		TypeName: Q.typeName,
		Identifier: Q.variableName,
		"MethodName/Identifier": Q.function(Q.variableName),
		Definition: Q.definition(Q.variableName),
	...
	Y = $.deserialize({
		...
		specialized: [{ term: 230, get: (O) => Z[O] || -1 }],
		tokenPrec: 6995,
	}),
	t = P.define({
		name: "java",
		parser: Y.configure({
			props: [
				a.add({
					IfStatement: i({ except: /^\s*({|else\b)/ }),
					TryStatement: i({ except: /^\s*({|catch|finally)\b/ }),
					LabeledStatement: X,
					SwitchBlock: (O) => {
						let Q = O.textAfter,
							$ = /^\s*\}/.test(Q),
...
export { n as java, t as javaLanguage };
