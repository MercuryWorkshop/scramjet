...
function xhrRequest(url, responseType) {
	return new Promise(function (resolve, reject) {
		...
		xhr.addEventListener("load", function () {
			if (xhr.status != 200) reject(Error(xhr.statusText));
...
function bindEvents(element, resolveEventName, rejectEventName) {
	element.eventPromise = bindEvents2(
		element,
		resolveEventName,
		element,
		rejectEventName
...
function bindEvents2(
	...
	return new Promise(function (resolve, reject) {
		...
		const cleanup = function () {
			...
			if (rejectObject2) {
				rejectObject2.removeEventListener(
...
function createElement(tagName, attrs, parentNode, doBindEvents) {
	...
	if (!isImg) setAttributes(element, attrs);
...
function requestViaIframe(url, additionalAttributes) {
	...
	const resultPromise = promiseWithTimeout(
		...
		5000,
		"Timed out waiting for iframe-tag result"
	).then((event) => {
		if (event.source !== iframe.contentWindow)
			return Promise.reject(new Error("Unexpected event.source"));
		return event.data;
	});
	// Attach the message listener before starting the navigation, otherwise a
	// fast local iframe can postMessage() before bindEvents2() is listening.
...
function workerUrlThatImports(url, additionalAttributes) {
	...
	return (
		`/common/security-features/subresource/static-import.py` +
...
function requestViaDedicatedWorker(url, options) {
	...
	worker.postMessage("");
	return bindEvents2(worker, "message", worker, "error").then((event) =>
		wrapResult(event.data)
	);
}
...
function requestViaSharedWorker(url, options) {
	...
	const promise = bindEvents2(worker.port, "message", worker, "error").then(
		(event) => wrapResult(event.data)
	);
	worker.port.start();
	return promise;
}
...
function requestViaNavigable(
	...
	const promise = promiseWithTimeout(
		bindEvents2(window, "message", iframe, "error", window, "error"),
		5000,
		`Timed out waiting for ${navigableElementName} navigation result`
	).then((event) => {
		if (event.source !== iframe.contentWindow)
			return Promise.reject(new Error("Unexpected event.source"));
		return event.data;
...
function requestViaAnchor(url, additionalAttributes) {
	return requestViaNavigable(
		"a",
		Object.assign(
			{ href: url, innerHTML: "Link to resource" },
			additionalAttributes
		),
		document.body,
		(a) => a.click()
...
function requestViaScript(url, additionalAttributes) {
	...
	return bindEvents2(window, "message", script, "error", window, "error").then(
		(event) => wrapResult(event.data)
	);
}
...
function requestViaLinkStylesheet(url) {
	return createRequestViaElement(
		"link",
...
function requestViaVideo(url) {
	return createMediaElement("video", {}, { src: url }).eventPromise;
}
...
function requestViaAudio(url) {
	return createMediaElement("audio", {}, { type: "audio/wav", src: url })
		.eventPromise;
}
...
function requestViaPicture(url) {
	var picture = createMediaElement(
		"picture",
		{},
		{ srcset: url, type: "image/png" }
	);
...
function requestViaSVGAnchor(url, additionalAttributes) {
	...
	const link_attributes = Object.assign(
		{ href: url, target: name },
		additionalAttributes
	);
	...
	const promise = promiseWithTimeout(
		bindEvents2(window, "message", iframe, "error", window, "error"),
		5000,
		"Timed out waiting for SVG anchor navigation result"
	).then((event) => {
		if (event.source !== iframe.contentWindow)
			return Promise.reject(new Error("Unexpected event.source"));
		return event.data;
...
const subresourceMap = {
	...
	"worker-import": {
		...
		invoker: (url, additionalAttributes) =>
			requestViaDedicatedWorker(
				workerUrlThatImports(url, additionalAttributes),
				{ type: "module" }
			),
	},
	"worker-import-data": {
		path: "/common/security-features/subresource/worker.py",
		invoker: (url) =>
			requestViaDedicatedWorker(workerDataUrlThatImports(url), {
				type: "module",
			}),
	},
	...
	"sharedworker-import": {
		path: "/common/security-features/subresource/shared-worker.py",
		invoker: (url, additionalAttributes) =>
			requestViaSharedWorker(workerUrlThatImports(url, additionalAttributes), {
				type: "module",
			}),
	},
...
function getSubresourceOrigin(originType) {
  ...
  */
	const originMap = {
		...
		"same-ws-downgrade": wsProtocol + "://" + sameOriginHost + ":" + wssRawPort,
		"cross-ws-downgrade":
...
function getRequestURLs(subresourceType, originType, redirectionType) {
	...
	const stashEndpoint =
		"/common/security-features/subresource/xhr.py?key=" +
		key +
		"&path=" +
		stashPath;
	return {
		testUrl:
			...
			"?redirection=" +
			encodeURIComponent(redirectionType) +
			"&action=purge&key=" +
			key +
			"&path=" +
			stashPath,
...
function invokeRequest(subresource, sourceContextList) {
	if (sourceContextList.length === 0) {
		...
		return subresourceMap[subresource.subresourceType]
			.invoker(subresource.url, additionalAttributes)
			.then((result) =>
				Object.assign({ sourceContextUrl: location.toString() }, result)
			);
	}
	...
	const sourceContextMap = {
		...
		iframe: {
			// <iframe src="same-origin-URL"></iframe>
			invoker: invokeFromIframe,
		},
		"iframe-blank": {
			// <iframe></iframe>
...
function invokeFromWorker(
	...
	let workerUrl =
		...
		encodeURIComponent(
			JSON.stringify(currentSourceContext.policyDeliveries || [])
		);
	...
	return promise
		...
		.then((event) => {
			if (event.data.error) return Promise.reject(event.data.error);
			return event.data;
		});
}
	...
	return promise
		.then(() => {
			const promise = promiseWithTimeout(
				bindEvents2(window, "message", iframe, "error", window, "error"),
				5000,
				`Timed out waiting for ${currentSourceContext.sourceContextType} result`
			);
