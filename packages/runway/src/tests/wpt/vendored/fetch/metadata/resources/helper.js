function validate_expectations(key, expected, tag) {
	return fetch(
		...
		.then((text) => {
			...
			test(
				(t) => assert_equals(value.dest, expected.dest),
				`${tag}: sec-fetch-dest`
			);
			test(
				(t) => assert_equals(value.mode, expected.mode),
				`${tag}: sec-fetch-mode`
			);
			test(
				(t) => assert_equals(value.site, expected.site),
				`${tag}: sec-fetch-site`
			);
			test(
				(t) => assert_equals(value.user, expected.user),
				`${tag}: sec-fetch-user`
			);
		});
}
