...
function MultipleRedirectTo(partialPath) {
	let finalURL =
		insecureRedirectURL + encodeURIComponent(secureTestURL + partialPath);
...
function RunCommonRedirectTests(
	testNamePrefix,
	urlHelperMethod,
	expectedResults
) {
	async_test((t) => {
		let testWindow = window.open(
			urlHelperMethod("resources/post-to-owner.py?top-level-navigation" + nonce)
		);
		...
		window.addEventListener(
			...
			t.step_func((e) => {
				...
				if (expectation["mode"] != "") expectation["mode"] = "navigate";
				if (expectation["dest"] == "font") expectation["dest"] = "document";
	...
	promise_test((t) => {
		return new Promise((resolve, reject) => {
			...
			e.onload = (e) => {
				let expectation = { ...expectedResults };
				if (expectation["mode"] != "") expectation["mode"] = "navigate";
				if (expectation["dest"] == "font") expectation["dest"] = "embed";
				fetch(
					"/fetch/metadata/resources/record-header.py?retrieve=true&file=" + key
				)
					.then((response) => response.text())
					.then(
						t.step_func((text) =>
							assert_header_equals(text, expectation, testNamePrefix + " embed")
						)
					)
					.then(resolve)
					.catch((e) => reject(e));
	...
	promise_test((t) => {
		return new Promise((resolve, reject) => {
			...
			e.onload = (e) => {
				let expectation = { ...expectedResults };
				if (expectation["mode"] != "") expectation["mode"] = "navigate";
				if (expectation["dest"] == "font") expectation["dest"] = "object";
				fetch(
					"/fetch/metadata/resources/record-header.py?retrieve=true&file=" + key
				)
					.then((response) => response.text())
	...
	if (document.createElement("link").relList.supports("preload")) {
		async_test((t) => {
			...
			e.onload = e.onerror = t.step_func_done((e) => {
				let expectation = { ...expectedResults };
				if (expectation["mode"] != "") expectation["mode"] = "cors";
				fetch(
					"/fetch/metadata/resources/record-header.py?retrieve=true&file=" + key
				)
	...
	promise_test((t) => {
		return new Promise((resolve, reject) => {
			...
			e.onload = (e) => {
				let expectation = { ...expectedResults };
				if (expectation["mode"] != "") expectation["mode"] = "no-cors";
				if (expectation["dest"] == "font") expectation["dest"] = "style";
				fetch(
					"/fetch/metadata/resources/record-header.py?retrieve=true&file=" + key
				)
					.then((response) => response.text())
	...
	promise_test((t) => {
		return new Promise((resolve, reject) => {
			...
			el.onload = t.step_func((_) => {
				let expectation = { ...expectedResults };
				if (expectation["mode"] != "") expectation["mode"] = "cors";
				if (expectation["dest"] == "font") expectation["dest"] = "track";
				fetch(
					"/fetch/metadata/resources/record-header.py?retrieve=true&file=" + key
				)
					.then((response) => response.text())
					.then(
						t.step_func((text) =>
							assert_header_equals(text, expectation, testNamePrefix + " track")
						)
					)
					.then(resolve);
			});
